...
def score_memory_entropy(forecasts: List[Dict], key: str = "arc_label") -> float:
...
def compare_against_memory(new_batch: List[Dict], memory: List[Dict], key: str = "arc_label") -> float:
...
def flag_memory_duplication(new_batch: List[Dict], memory: List[Dict], key: str = "arc_label") -> List[Dict]:
    if not isinstance(new_batch, list) or not isinstance(memory, list):
        raise ValueError("Inputs must be lists")
    """
...
def generate_entropy_report(forecasts: List[Dict], memory: List[Dict]) -> Dict:
    """
