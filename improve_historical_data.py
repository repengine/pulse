...
def modify_historical_ingestion_plugin():
    ...
    try:
        ...
        modified_content = content.replace("RETRODICTION_TIMELINE_YEARS = 5", 
                                           "RETRODICTION_TIMELINE_YEARS = 5")
        
        # If the parameter was 1 year, change it to 5
        modified_content = modified_content.replace("RETRODICTION_TIMELINE_YEARS = 1", 
                                                   "RETRODICTION_TIMELINE_YEARS = 5")
...
def clean_and_impute_data(data: dict) -> dict:
    ...
    try:
        ...
        if missing_count > 0:
            ...
            logger.info(f"After imputation: {df['value'].isna().sum()} missing values remain")
        ...
        # Update statistics
        non_nan_values = [item['value'] for item in data['values'] if not pd.isna(item['value'])]
        if non_nan_values:
            data['metadata']['retrieval_stats'].update({
                ...
                'completeness_pct': 100.0 * len(non_nan_values) / len(data['values']),
...
def update_verification_report(variable_results: Dict[str, Dict[str, Any]]):
    ...
    try:
        ...
        report["successful_retrievals"] = sum(1 for v in variable_results.values() if v.get("success", False))
        report["failed_retrievals"] = sum(1 for v in variable_results.values() if not v.get("success", False))
        ...
        for var_name, var_report in variable_results.items():
            if var_report.get("success", False) and var_report.get("stats"):
                report["variable_reports"][var_name] = {
                    ...
                    "anomalies_detected": var_report["stats"].get("anomalies_count", 0) > 0
                }
        
        # Update overall metrics
        completeness_values = [v.get("completeness_pct", 0) for v in report["variable_reports"].values()]
        report["overall_metrics"]["average_completeness"] = sum(completeness_values) / len(completeness_values) if completeness_values else 0
        report["overall_metrics"]["variables_with_gaps"] = sum(1 for v in report["variable_reports"].values() if v.get("gaps_detected", False))
        report["overall_metrics"]["variables_with_anomalies"] = sum(1 for v in report["variable_reports"].values() if v.get("anomalies_detected", False))
...
def execute_data_ingestion():
    ...
    try:
        ...
        # Import the plugin
        from iris.iris_plugins_variable_ingestion.historical_ingestion_plugin import historical_ingestion_plugin
        ...
        for variable_name in PRIORITY_VARIABLES:
            ...
            if variable_data:
                ...
                for item in variable_data:
                    ...
                    if value_val is not None and not (isinstance(value_val, float) and np.isnan(value_val)):
                        valid_values += 1
                ...
                processed_file = f"{processed_dir}/{variable_name}_processed_{timestamp}.json"
    ...
    for variable_name in PRIORITY_VARIABLES:
        ...
        try:
            ...
            if not processed_path:
                ...
                results[variable_name] = {"success": False, "error": "No processed data found"}
                continue
            ...
            if not data:
                logger.warning(f"Failed to load processed data for {variable_name}, skipping")
                results[variable_name] = {"success": False, "error": "Failed to load processed data"}
                continue
...
def main():
    ...
    try:
        ...
        logger.info(f"Completed historical data improvement: {successful}/{len(results)} variables successfully processed")
