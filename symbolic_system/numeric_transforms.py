...
class SymbolicNumericTransformer:
    ...
    def __init__(self):
        ...
        self.threshold_file = os.path.join(os.path.dirname(__file__), 
        ...
        if os.path.exists(self.threshold_file):
            try:
                with open(self.threshold_file, 'r') as f:
                    ...
                    logger.info(f"Loaded adaptive thresholds from {self.threshold_file}")
    ...
    def numeric_to_symbolic(self, indicator_name: str, value: float, 
            ...
            if "high_impact" in variable_mapping and "threshold" in variable_mapping:
                if value > variable_mapping["threshold"]:
                    ...
                    if impact_overlays:
                        ...
                        overlay_value = 0.5 + min(0.5, (value - variable_mapping["threshold"]) 
                                                 / variable_mapping["threshold"] * 0.5)
    ...
    def symbolic_to_numeric(self, overlay_name: str, value: float) -> Dict[str, float]:
        ...
        if overlay_name == "hope":
            return {
                "price_momentum": normalized_impact * 0.5,  # Positive hope -> positive momentum
                "trading_volume": value * 0.3 + 0.2,  # More hope -> slightly higher volume
            ...
            return {
                "price_momentum": -normalized_impact * 0.5,  # More despair -> negative momentum
                "trading_volume": value * 0.5,  # More despair -> can increase volume (panic)
            ...
            return {
                ...
                "price_momentum_stability": value * 0.4  # Trust -> more stable momentum
            }
    ...
    def update_adaptive_thresholds(self, historical_data: Dict[str, List[float]]):
        ...
        for indicator_name, values in historical_data.items():
            ...
            self.adaptive_thresholds[f"{indicator_name}_high"] = float(np.percentile(values_array, 80))
            self.adaptive_thresholds[f"{indicator_name}_low"] = float(np.percentile(values_array, 20))
            self.adaptive_thresholds[f"{indicator_name}_median"] = float(np.median(values_array))
            
            logger.info(f"Updated adaptive thresholds for {indicator_name}: "
    ...
    def _record_transformation(self, indicator_name: str, indicator_value: float,
                              overlay_name: str, overlay_value: float, confidence: float):
        """
        ...
        if key not in self.confidence_scores:
            ...
            self.confidence_scores[key] = 0.9 * self.confidence_scores[key] + 0.1 * confidence
    ...
    def apply_numeric_to_state(self, state: WorldState, indicators: Dict[str, float]):
        ...
        for indicator_name, value in indicators.items():
            ...
            overlay_name, overlay_value, confidence = self.numeric_to_symbolic(indicator_name, value)
            ...
            if not state.overlays.has_overlay(overlay_name):
                ...
                if overlay_name not in ["hope", "despair", "rage", "fatigue", "trust"] and confidence > 0.7:
            ...
            new_value = current_value * (1 - influence_weight) + overlay_value * influence_weight
            ...
            self._record_transformation(indicator_name, value, overlay_name, new_value, confidence)
    ...
    def get_confidence(self, overlay_name: str) -> float:
        ...
        related_scores = [v for k, v in self.confidence_scores.items() if k.endswith(f"_{overlay_name}")]
        if related_scores:
