...
from symbolic_system.gravity.integration import (
    initialize_gravity_system, enable_gravity_system, 
    pre_simulation_hook, post_simulation_hook,
    record_prediction_residual, apply_gravity_correction,
    get_gravity_fabric, get_pillar_system
)
...
def example_with_simulation() -> None:
    ...
    print(f"SPX Improvement: {report['fabric_stats']['variable_improvements'].get('SPX', {}).get('percentage_improvement', 0):.2f}%")
    print(f"VIX Improvement: {report['fabric_stats']['variable_improvements'].get('VIX', {}).get('percentage_improvement', 0):.2f}%")
...
def example_standalone_usage() -> None:
    ...
    for point in recent_residuals:
        print(f"  Predicted: {point.predicted:.2f}, Actual: {point.actual:.2f}, "
