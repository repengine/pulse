...
class SymbolicGravityFabric:
    ...
    def __init__(
        ...
        self.pillar_system = pillar_system or SymbolicPillarSystem(config=config)
    ...
    def bulk_apply_correction(
        self, sim_vars: Dict[str, float], symbol_vec: Optional[Dict[str, float]] = None
    ) -> Dict[str, float]:
        """
    ...
    def update_weights(
        self, residual: Dict[str, float], symbol_vec: Optional[Dict[str, float]] = None
    ) -> None:
        """
    ...
    def step(self, state: Any) -> None:
        ...
        try:
            ...
            overlay_dict = (
                overlays.as_dict() if hasattr(overlays, "as_dict") else overlays
            )  # type: ignore
            ...
            for name, intensity in overlay_dict.items():
                if self.pillar_system.has_pillar(name):
                    ...
                    self.pillar_system.register_pillar(
                        name, initial_intensity=intensity
                    )
    ...
    def get_metrics(self) -> Dict[str, Any]:
        ...
        if total_corr > 0:
            metrics["mean_correction"] = (
                sum(metrics["correction_magnitudes"]) / total_corr
            )
...
def create_default_fabric(config=None) -> SymbolicGravityFabric:
    ...
    fabric = SymbolicGravityFabric(
        gravity_engine=gravity_engine, pillar_system=pillar_system, config=config
    )
...
if __name__ == "__main__":
    ...
    simulated = {"market_price": 100.0, "volatility": 0.2, "sentiment": -0.3}
