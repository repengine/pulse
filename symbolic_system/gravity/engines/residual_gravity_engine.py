...
class GravityEngineConfig:
    ...
    def __init__(
        ...
        self.regularization_strength = regularization_strength if regularization_strength is not None else grav_cfg.DEFAULT_REGULARIZATION
        self.learning_rate = learning_rate if learning_rate is not None else grav_cfg.DEFAULT_LEARNING_RATE
        self.momentum_factor = momentum_factor if momentum_factor is not None else grav_cfg.DEFAULT_MOMENTUM
        self.circuit_breaker_threshold = circuit_breaker_threshold if circuit_breaker_threshold is not None else grav_cfg.DEFAULT_CIRCUIT_BREAKER_THRESHOLD
        self.max_correction = max_correction if max_correction is not None else grav_cfg.DEFAULT_MAX_CORRECTION
        self.enable_adaptive_lambda = enable_adaptive_lambda if enable_adaptive_lambda is not None else grav_cfg.DEFAULT_ENABLE_ADAPTIVE_LAMBDA
        self.enable_weight_pruning = enable_weight_pruning if enable_weight_pruning is not None else grav_cfg.DEFAULT_ENABLE_WEIGHT_PRUNING
        self.weight_pruning_threshold = weight_pruning_threshold if weight_pruning_threshold is not None else grav_cfg.DEFAULT_WEIGHT_PRUNING_THRESHOLD
        self.fragility_threshold = fragility_threshold if fragility_threshold is not None else grav_cfg.DEFAULT_FRAGILITY_THRESHOLD
        self.ewma_span = ewma_span if ewma_span is not None else grav_cfg.DEFAULT_EWMA_SPAN
        
        # Initialize adaptive lambda parameters
        self.adaptive_lambda_min = adaptive_lambda_min if adaptive_lambda_min is not None else grav_cfg.DEFAULT_ADAPTIVE_LAMBDA_MIN
        self.adaptive_lambda_max = adaptive_lambda_max if adaptive_lambda_max is not None else grav_cfg.DEFAULT_ADAPTIVE_LAMBDA_MAX
        self.adaptive_lambda_window_size = adaptive_lambda_window_size if adaptive_lambda_window_size is not None else grav_cfg.DEFAULT_ADAPTIVE_LAMBDA_WINDOW_SIZE
        self.adaptive_lambda_residual_threshold_high = adaptive_lambda_residual_threshold_high if adaptive_lambda_residual_threshold_high is not None else grav_cfg.DEFAULT_ADAPTIVE_LAMBDA_RESIDUAL_THRESHOLD_HIGH
        self.adaptive_lambda_residual_threshold_low = adaptive_lambda_residual_threshold_low if adaptive_lambda_residual_threshold_low is not None else grav_cfg.DEFAULT_ADAPTIVE_LAMBDA_RESIDUAL_THRESHOLD_LOW
        self.adaptive_lambda_increase_factor = adaptive_lambda_increase_factor if adaptive_lambda_increase_factor is not None else grav_cfg.DEFAULT_ADAPTIVE_LAMBDA_INCREASE_FACTOR
        self.adaptive_lambda_decrease_factor = adaptive_lambda_decrease_factor if adaptive_lambda_decrease_factor is not None else grav_cfg.DEFAULT_ADAPTIVE_LAMBDA_DECREASE_FACTOR
        
        # Logging level
        self.logging_level = logging_level if logging_level is not None else grav_cfg.DEFAULT_LOGGING_LEVEL
        
        # Shadow Model Trigger parameters
        self.enable_shadow_model_trigger = grav_cfg.DEFAULT_ENABLE_SHADOW_MODEL_TRIGGER
        self.shadow_model_variance_threshold = grav_cfg.DEFAULT_SHADOW_MODEL_VARIANCE_THRESHOLD
        self.shadow_model_window_size = grav_cfg.DEFAULT_SHADOW_MODEL_WINDOW_SIZE
        self.shadow_model_min_trigger_samples = grav_cfg.DEFAULT_SHADOW_MODEL_MIN_TRIGGER_SAMPLES
        self.shadow_model_min_causal_variance = grav_cfg.DEFAULT_SHADOW_MODEL_MIN_CAUSAL_VARIANCE
...
class ResidualGravityEngine:
    ...
    def __init__(
        ...
        if self.enable_shadow_model_trigger:
            self._recent_causal_residuals = deque(maxlen=config.shadow_model_window_size)
            self._recent_gravity_residuals = deque(maxlen=config.shadow_model_window_size)
    ...
    def _dict_to_ordered_array(self, symbol_vec: Dict[str, float]) -> np.ndarray:
        ...
        symbol_array = np.array([symbol_vec.get(name, 0.0) for name in self.pillar_names])
    ...
    def compute_gravity(self, symbol_vec: Dict[str, float]) -> Union[float, np.ndarray]:
        ...
        gravity_vector = self.impact_matrix_B @ symbol_array  # Shape: (state_dimensionality, 1)
        ...
        if self.state_dimensionality == 1:
            ...
            self._stats["last_gravity_contributors"] = {
                ...
                if k in symbol_vec and abs(self.weights[k] * symbol_vec[k]) > grav_cfg.SIGNIFICANT_CONTRIBUTOR_THRESHOLD
            }
    ...
    def update_impact_matrix(self, residual: Union[float, np.ndarray], symbol_vec: Dict[str, float]) -> None:
        """
        ...
        if self.state_dimensionality == 1:
            ...
            for i, pillar_name in enumerate(self.pillar_names):
                if pillar_name in symbol_vec:
                    ...
                    grad = float(residual_array[0, 0]) * s_k - self.reg * self.impact_matrix_B[0, i]
                    
                    # Update momentum
                    self._v_matrix[0, i] = self.β * self._v_matrix[0, i] + (1 - self.β) * grad
            ...
            for i, pillar_name in enumerate(self.pillar_names):
                if pillar_name in symbol_vec:
                    ...
                    grad = s_k * residual_array - self.reg * self.impact_matrix_B[:, i:i+1]
                    
                    # Update momentum for the entire column
                    self._v_matrix[:, i:i+1] = self.β * self._v_matrix[:, i:i+1] + (1 - self.β) * grad
                    
                    # Update impact matrix column
                    self.impact_matrix_B[:, i:i+1] += self.η * self._v_matrix[:, i:i+1]
        ...
        if apply_ewma and impact_matrix_B_old_ewma is not None:
            self.impact_matrix_B = self.ewma_alpha * self.impact_matrix_B + (1 - self.ewma_alpha) * impact_matrix_B_old_ewma
    ...
    def apply_gravity_correction(self,
        ...
        if self.enable_adaptive_lambda:
            ...
            effective_lambda = self.lambda_ * max(grav_cfg.MIN_LAMBDA_SCALE_FACTOR, 1.0 - fragility)
    ...
    def _update_adaptive_lambda(self, current_residual_magnitude: float) -> None:
        ...
        if len(self.recent_residuals) == self.config.adaptive_lambda_window_size:
            ...
            if avg_recent_residual > self.config.adaptive_lambda_residual_threshold_high:
                # Increase lambda when residuals are high
                new_lambda = self.lambda_ * self.config.adaptive_lambda_increase_factor
            elif avg_recent_residual < self.config.adaptive_lambda_residual_threshold_low:
            ...
            self._stats["adaptive_lambda_adjustment"] = self._stats.get("adaptive_lambda_adjustment", 0) + 1
            self._stats["current_lambda"] = float(self.lambda_)
            self._stats["avg_recent_residual"] = float(avg_recent_residual)
    
    def _calculate_fragility(self, gravity_correction: Union[float, np.ndarray]) -> float:
        """
        ...
        fragility = (grav_cfg.FRAGILITY_RMS_WEIGHT * rms_factor +
    ...
    def process_and_correct(self,
        ...
        correction, corrected_prediction = self.apply_gravity_correction(sim_vec, symbol_vec)
        ...
        if self.enable_shadow_model_trigger:
            ...
            if len(self._recent_causal_residuals) >= self.config.shadow_model_min_trigger_samples:
    ...
    def check_health(self) -> Dict[str, Any]:
        ...
        elif self._stats.get("updates", 0) > grav_cfg.STAGNANT_WEIGHT_UPDATES and health["rms_weight"] < grav_cfg.STAGNANT_WEIGHT_RMS_THRESHOLD:
            health["status"] = "caution"
            health["warnings"].append("Weights remain very small after many updates, possibly not learning")
        
        # 6. Low correction efficiency
        elif health["correction_efficiency"] < grav_cfg.LOW_CORRECTION_EFFICIENCY_THRESHOLD and self._stats.get("updates", 0) > grav_cfg.MIN_UPDATES_FOR_EFFICIENCY_CHECK:
            health["status"] = "caution"
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "ResidualGravityEngine":
        ...
        config = GravityEngineConfig(
            ...
            circuit_breaker_threshold=data.get("circuit_breaker_threshold", grav_cfg.DEFAULT_CIRCUIT_BREAKER_THRESHOLD),
            ...
            enable_adaptive_lambda=data.get("enable_adaptive_lambda", grav_cfg.DEFAULT_ENABLE_ADAPTIVE_LAMBDA),
            enable_weight_pruning=data.get("enable_weight_pruning", grav_cfg.DEFAULT_ENABLE_WEIGHT_PRUNING),
            weight_pruning_threshold=data.get("weight_pruning_threshold", grav_cfg.DEFAULT_WEIGHT_PRUNING_THRESHOLD),
            fragility_threshold=data.get("fragility_threshold", grav_cfg.DEFAULT_FRAGILITY_THRESHOLD)
        )
