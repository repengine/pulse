...
SIMULATED_VARIABLES = [
    ...
    "housing_starts", "consumer_confidence", "industrial_production",
    "retail_sales", "commodity_prices", "market_volatility",
    "geopolitical_tension", "innovation_rate", "hope", "rage",
    "social_cohesion", "trust_in_institutions"
]
...
def stop_autopilot():
    ...
    if autopilot_status == "stopped":
        return jsonify({
            "status": "error",
            "error": "Autopilot is not running"
        })
...
def run_learning_audit():
    ...
    if not params.get("previous_batch_id") or not params.get("current_batch_id"):
        return jsonify({
            "status": "error",
            "error": "Both previous_batch_id and current_batch_id are required"
        })
...
def simulate_retrodiction_run(run_id):
    ...
    try:
        ...
        variables = params.get("variables_of_interest", ["inflation", "unemployment", "gdp_growth", "interest_rate"])
        ...
        for var in variables:
            ...
            for i in range(params.get("days", 30)):
                ...
                value = base_value + (i * random.uniform(-0.1, 0.1)) + random.uniform(-0.5, 0.5)
