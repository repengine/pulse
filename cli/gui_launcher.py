...
def check_dependencies():
    ...
    required_packages = [
        'flask', 'requests', 'matplotlib', 'numpy', 'pandas'
    ]
...
def start_api_server():
    ...
    api_server_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "../api/server.py")
    
    try:
        if os.name == 'nt':  # Windows
            # Use pythonw to avoid a console window
            process = subprocess.Popen([sys.executable, api_server_path],
...
def start_gui():
    """Start the Pulse GUI."""
    gui_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "../pulse_desktop/tkinter_ui.py")
...
def main():
    ...
    if not deps_ok:
        ...
        if install:
            ...
            if not install_dependencies(missing_packages):
                show_error_dialog("Failed to install dependencies. Please install them manually and try again.")
                return
        else:
    ...
    if not api_process:
        show_error_dialog("Failed to start API server. Please check the logs for details.")
        return
    
    # Wait for the API server to become available
    if not wait_for_api_server(timeout=10):
        logger.warning("API server may not be fully operational, but will continue starting the GUI")
