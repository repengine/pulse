...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Pulse Simulation and Data Management Engine"
    )
    ...
    simulate_parser.add_argument(
        "--turns", type=int, default=5, help="Number of simulation turns to run"
    )
    simulate_parser.add_argument(
        "--output", type=str, default=None, help="Optional output file for digest"
    )
    simulate_parser.add_argument(
        "--auto-upgrade",
        action="store_true",
        help="Automatically generate epistemic upgrade plan after run",
    )

    # Repair command
    repair_parser = subparsers.add_parser(
        "repair", help="Address identified data quality issues"
    )
    repair_parser.add_argument(
        "variable_name", type=str, help="Name of the variable to repair"
    )
    repair_parser.add_argument(
        "--variable-type",
        type=str,
    ...
    repair_parser.add_argument(
        "--skip-smoothing", action="store_true", help="Skip the smoothing step"
    )
    repair_parser.add_argument(
        "--skip-cross-source",
        action="store_true",
    ...
    simulate_repair_parser = subparsers.add_parser(
        "simulate-repair", help="Preview repairs without applying them"
    )
    simulate_repair_parser.add_argument(
        "variable_name", type=str, help="Name of the variable to simulate repairs for"
    )
    simulate_repair_parser.add_argument(
        "--variable-type",
        type=str,
    ...
    repair_report_parser.add_argument(
        "variable_name", type=str, help="Name of the variable"
    )
    repair_report_parser.add_argument(
        "--version",
        type=str,
        help="Optional Version ID (if None, gets the latest version)",
    )

    # Revert command
    revert_parser = subparsers.add_parser(
        "revert", help="Revert a variable to its original state"
    )
    revert_parser.add_argument("variable_name", type=str, help="Name of the variable")
    revert_parser.add_argument(
        "--version",
        type=str,
        help="Optional Version ID to revert to (if None, reverts to the original)",
    )

    # Compare Versions command
    compare_parser = subparsers.add_parser(
        "compare-versions", help="Compare two versions of a variable"
    )
    ...
    compare_parser.add_argument(
        "--version2",
        type=str,
        help="Optional Second version ID (if None, uses the latest repaired version)",
    )

    # List Versions command
    versions_parser = subparsers.add_parser(
        "list-versions", help="List all repair versions for a variable"
    )
    ...
    if args.command == "simulate":
        ...
        if args.output:
            try:
                with open(args.output, "w", encoding="utf-8") as f:
                    ...
                    f.write(
                        str(
                            generate_strategos_digest(
                                ForecastMemory(), n=min(args.turns, 5)
                            )
                        )
                    )
        ...
        if getattr(args, "auto_upgrade", False):
            try:
                ...
                upgrade_path = CONFIG.get(
                    "upgrade_plan_path", "plans/epistemic_upgrade_plan.json"
                )
                batch_path = CONFIG.get(
                    "batch_output_path", "logs/strategic_batch_output.jsonl"
                )
                revised_path = CONFIG.get(
                    "revised_forecasts_path", "logs/revised_forecasts.jsonl"
                )
        ...
        result = simulate_repair(args.variable_name, variable_type=args.variable_type)
        print(json.dumps(result.to_dict(), indent=2))

    elif args.command == "repair-report":
        result = get_repair_report(args.variable_name, version_id=args.version)
        print(json.dumps(result, indent=2))

    elif args.command == "revert":
        result = revert_to_original(args.variable_name, version_id=args.version)
        print(json.dumps(result, indent=2))

    elif args.command == "compare-versions":
        result = compare_versions(
            args.variable_name, args.version_id1, version_id2=args.version2
        )
        print(json.dumps(result, indent=2))

    elif args.command == "list-versions":
        result = get_all_versions(args.variable_name)
        print(json.dumps(result, indent=2))
