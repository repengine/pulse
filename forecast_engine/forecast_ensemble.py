...
def ensemble_forecast(simulation_forecast: Dict, ai_forecast: Dict) -> Dict:
    ...
    logger.info(
        ...
        {k: simulation_forecast[k] for k in list(simulation_forecast.keys())[:3]} if hasattr(simulation_forecast, "keys") else str(simulation_forecast)[:100],
        type(ai_forecast),
    ...
    import re
    uuid_like = isinstance(sim_val_raw, str) and re.match(r"^[0-9a-fA-F-]{32,36}$", sim_val_raw)
    try:
        if uuid_like:
            logger.error(f"Simulation forecast 'value' is a UUID string (forecast_id?): {sim_val_raw}. This is a bug upstream. Using default 0.0")
            sim_value = 0.0
        else:
            sim_value = float(sim_val_raw)
    except (ValueError, TypeError):
        logger.warning(f"Invalid simulation forecast value: {sim_val_raw} (type={type(sim_val_raw)}). Using default 0.0")
        sim_value = 0.0
        
    ai_adj_raw = ai_forecast.get("adjustment", 0.0)
    # Also check AI adjustment for UUID-like strings
    uuid_like_ai = isinstance(ai_adj_raw, str) and re.match(r"^[0-9a-fA-F-]{32,36}$", ai_adj_raw)
    try:
        if uuid_like_ai:
            logger.error(f"AI forecast 'adjustment' is a UUID string (forecast_id?): {ai_adj_raw}. This is a bug upstream. Using default 0.0")
            ai_adjustment = 0.0
        else:
            ai_adjustment = float(ai_adj_raw)
    except (ValueError, TypeError):
        logger.warning(f"Invalid AI adjustment value: {ai_adj_raw} (type={type(ai_adj_raw)}). Using default 0.0")
        ai_adjustment = 0.0
