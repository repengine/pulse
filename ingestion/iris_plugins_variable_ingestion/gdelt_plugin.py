...
class GdeltPlugin(IrisPluginManager):
    ...
    def _safe_get(
        self, url: str, params: dict = None, dataset_id: str = "unknown"
    ) -> Optional[str]:
        ...
        save_request_metadata(
            dataset_id, params or {}, source_name=_SOURCE_NAME, url=url
        )
        ...
        for attempt in range(self.MAX_RETRIES + 1):
            try:
                ...
                save_api_response(
                    ...
                    },  # Save first 10K chars (can be large)
                ...
                logger.warning(
                    f"GDELT request failed ({attempt + 1}/{self.MAX_RETRIES}): {exc}"
                )
        ...
        logger.error(
            f"Failed to fetch data from GDELT after {self.MAX_RETRIES} attempts"
        )
        return None

    def _fetch_events_data(
        self, region_codes: Union[str, List[str]]
        ...
        location_filter = " OR ".join(
            [f"ActionGeo_CountryCode:{code}" for code in region_codes]
        )
        ...
        region_str = (
            ...
            else f"{len(region_codes)}_regions"
        )
    ...
    def _fetch_gkg_data(
        ...
        # Create dataset ID for persistence
        theme_str = (
            "_".join(theme_filters)
            if len(theme_filters) <= 3
            else f"{len(theme_filters)}_themes"
        )
        ...
        try:
            ...
            if not isinstance(gkg_records, list):
                logger.error(
                    f"Unexpected GDELT GKG response format: {type(gkg_records)}"
                )
                return None
    ...
    def _process_events_data(
        self, events: List[Dict[str, Any]], region_key: str, timestamp: dt.datetime
    ) -> List[Dict[str, Any]]:
    ...
    def _process_gkg_data(
        self, gkg_records: List[Dict[str, Any]], theme_key: str, timestamp: dt.datetime
    ) -> List[Dict[str, Any]]:
