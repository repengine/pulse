    ...
    try:
        if args.variable:
            ...
            result = retrieve_historical_data(
                var_info,
                years=args.years,
                end_date=None if not args.end_date else datetime.strptime(args.end_date, "%Y-%m-%d")
            )
            
            if "stats" in result:
                logger.info(f"Successfully retrieved {result['stats']['data_point_count']} data points")
            ...
            logger.info(f"Retrieved data for {success_count + failure_count} variables:")
    ...
    try:
        if args.variable:
            ...
            if result.status == "success":
                logger.info(f"Successfully transformed and stored {result.item_count} records")
                logger.info(f"Dataset ID: {result.data_store_id}")
                if result.start_date and result.end_date:
                    logger.info(f"Date range: {result.start_date.date()} to {result.end_date.date()}")
            else:
            ...
            logger.info(f"Transforming and storing data for all priority {args.priority} variables")
            ...
            for var_info in catalog["variables"]:
                try:
                    ...
                    if result.status == "success":
                        ...
                        failure_count += 1
                        logger.warning(f"Failed to transform {var_name}: {result.error}")
    ...
    try:
        if args.variable:
            ...
            if verification["status"] == "success":
                ...
                logger.warning(f"Verification completed with warnings for {args.variable}")
                logger.warning(f"Found {verification['error_count']} data type errors")
                
            else:
                logger.error(f"Verification failed for {args.variable}: {verification.get('error', '')}")
                return 1
            ...
            for var_info in catalog["variables"]:
                try:
                    ...
                    if verification["status"] == "success":
                        ...
                        warning_count += 1
                        logger.warning(f"Warnings for {var_name}: {verification['error_count']} errors")
                    else:
                        failure_count += 1
                        logger.error(f"Failed verification for {var_name}: {verification.get('error', '')}")
                        
                except Exception as e:
                    logger.error(f"Error verifying {var_info['variable_name']}: {e}")
                    failure_count += 1
            
            logger.info(f"Verified {success_count + warning_count + failure_count} variables:")
    ...
    try:
        if args.coverage:
            ...
            if "overall_metrics" in report:
                ...
                logger.info(f"Fully complete variables (≥99%): {metrics['fully_complete_variables']}")
                logger.info(f"Partially complete variables (80-99%): {metrics['partially_complete_variables']}")
                logger.info(f"Incomplete variables (<80%): {metrics['incomplete_variables']}")
            else:
            ...
            logger.info(f"Historical datasets by priority:")
...
def handle_quality_check_command(args):
    ...
    try:
        if args.variable:
            ...
            logger.info(f"Overall quality score: {result.quality_score.overall_score:.2f}")
            ...
            logger.info(f"Performing quality check for all priority {args.priority} variables")
            ...
            logger.info(f"Variables with anomalies: {stats['variables_with_anomalies']}")
            
            # Save report to file
            output_file = args.output if args.output else f"quality_report_priority_{args.priority}.json"
            with open(output_file, "w") as f:
            ...
            logger.info(f"Average overall score: {stats['average_overall_score']:.2f}")
            logger.info(f"Variables analyzed: {report['variables_analyzed']}")
            logger.info(f"Variables with issues: {stats['variables_with_gaps'] + stats['variables_with_anomalies']}")
...
def handle_anomaly_detect_command(args):
    ...
    try:
        ...
        logger.info(f"Detecting anomalies in {args.variable} using {args.method} method")
        ...
        if anomalies:
            ...
            for i, anomaly in enumerate(sorted(anomalies, key=lambda a: a.severity, reverse=True)):
                if i < 10:  # Show top 10 anomalies by severity
                    logger.info(f"  - {anomaly.timestamp.date()}: value={anomaly.value:.4f}, " +
        ...
        if args.visualize:
            logger.info(f"Generating visualization for {args.variable} with anomalies highlighted")
            vis_paths = visualize_data_quality(args.variable, show_gaps=False, show_trend_breaks=False)
            logger.info(f"Visualization saved to: {vis_paths.get('time_series', 'unknown')}")
...
def handle_cross_validate_command(args):
    ...
    try:
        ...
        logger.info(f"Cross-validating data for {args.variable} across different sources")
        ...
        if result.sources:
            ...
            logger.info(f"Found {len(result.sources)} sources: {', '.join(result.sources)}")
        ...
        if args.visualize and result.sources:
            logger.info(f"Generating cross-source comparison visualization for {args.variable}")
            # The visualization function will automatically create a cross-source chart if multiple sources are available
            vis_paths = visualize_data_quality(args.variable, show_anomalies=False, show_gaps=False, show_trend_breaks=False)
            logger.info(f"Cross-source visualization saved to: {vis_paths.get('cross_source', 'not created')}")
    ...
    try:
        if args.variable:
            ...
            if result.status == "success":
                ...
                logger.info(f"- Cross-source reconciliations: {result.cross_source_actions}")
            ...
            logger.info(f"Repairing data quality issues for all priority {args.priority} variables")
            ...
            no_repair_count = sum(1 for r in results.values() if r.status == "no_repairs_needed")
            ...
            # Total repairs
            total_repairs = sum(r.total_repairs for r in results.values() if r.status == "success")
...
def handle_simulate_repair_command(args):
    ...
    try:
        ...
        if result.status == "success":
            ...
            logger.info(f"- Smoothing actions: {result.smoothing_actions}")
            logger.info(f"- Cross-source reconciliations: {result.cross_source_actions}")
            logger.info(f"Potential quality improvement: {result.quality_improvement:.3f}")
            ...
            logger.error(f"Failed to simulate repairs for {args.variable}: {result.error}")
            return 1
    ...
    try:
        if args.variable:
            ...
            if versions["status"] == "success":
                logger.info(f"Found {len(versions['versions'])} versions for {variable_name}:")
                
                for version in versions["versions"]:
                    logger.info(f"- {version['version_id']}: {version['timestamp']} ({version['version_type']})")
            ...
            if result["status"] == "success":
                ...
                logger.error(f"Failed to revert: {result.get('message', 'Unknown error')}")
                return 1
                
        elif args.compare:
            # Compare versions
            variable_name, version1, version2 = args.compare, args.version, args.version2
            ...
            if result["status"] == "success":
                ...
                logger.info(f"Version 1: {result['version_id1']} ({result['version1_type']})")
                logger.info(f"Version 2: {result['version_id2']} ({result['version2_type']})")
                ...
                if diff_stats:
                    ...
                    logger.info(f"  Mean difference: {diff_stats.get('mean_difference', 0):.4f}")
                    logger.info(f"  Median difference: {diff_stats.get('median_difference', 0):.4f}")
                    logger.info(f"  Standard deviation: {diff_stats.get('std_difference', 0):.4f}")
                    logger.info(f"  Number of differences: {diff_stats.get('num_differences', 0)}")
                ...
                logger.error(f"Failed to compare versions: {result.get('message', 'Unknown error')}")
                return 1
                
        else:
            logger.error("Missing required arguments. Use --variable, --list-versions, --revert, or --compare.")
            return 1
...
def handle_gap_analysis_command(args):
    ...
    try:
        ...
        result = analyze_gaps(args.variable, freq=args.frequency, min_gap_size=args.min_gap_size)
        ...
        if "error" not in result:
            ...
            if result['gaps']:
                ...
                sorted_gaps = sorted(result['gaps'], key=lambda g: g['gap_duration_days'], reverse=True)
                for i, gap in enumerate(sorted_gaps[:5]):
                    logger.info(f"  - {gap['start_time']} to {gap['end_time']}: {gap['gap_duration_days']} days")
        ...
        if args.visualize and "error" not in result:
            ...
            vis_paths = visualize_data_quality(args.variable, show_anomalies=False, show_trend_breaks=False)
            logger.info(f"Visualization with gaps highlighted saved to: {vis_paths.get('time_series', 'unknown')}")
            logger.info(f"Completeness visualization saved to: {vis_paths.get('completeness', 'not created')}")
...
def main():
    ...
    # 'retrieve' command
    retrieve_parser = subparsers.add_parser("retrieve", help="Retrieve raw historical data")
    retrieve_group = retrieve_parser.add_mutually_exclusive_group(required=True)
    retrieve_group.add_argument("--variable", type=str, help="Name of variable to retrieve")
    retrieve_group.add_argument("--priority", type=int, help="Retrieve variables with this priority")
    retrieve_group.add_argument("--all", action="store_true", help="Retrieve all variables")
    retrieve_parser.add_argument("--years", type=int, default=5, help="Number of years of data to retrieve")
    retrieve_parser.add_argument("--end-date", type=str, help="End date (YYYY-MM-DD)")
    
    # 'transform' command
    transform_parser = subparsers.add_parser("transform", help="Transform and store data")
    transform_group = transform_parser.add_mutually_exclusive_group(required=True)
    transform_group.add_argument("--variable", type=str, help="Name of variable to transform")
    transform_group.add_argument("--priority", type=int, help="Transform variables with this priority")
    transform_group.add_argument("--all", action="store_true", help="Transform all variables")
    ...
    report_group.add_argument("--coverage", action="store_true", help="Generate data coverage report")
    report_group.add_argument("--summary", action="store_true", help="Generate summary report")
    
    # 'quality-check' command - NEW COMMAND
    quality_parser = subparsers.add_parser("quality-check", help="Perform comprehensive quality assessment")
    ...
    quality_group.add_argument("--priority", type=int, help="Check variables with this priority")
    quality_group.add_argument("--all", action="store_true", help="Check all variables")
    quality_parser.add_argument("--anomaly-method", type=str, default="zscore",
                              choices=["zscore", "iqr", "isolation_forest"],
                              help="Method to use for anomaly detection")
    quality_parser.add_argument("--variable-type", type=str, default="raw",
                              choices=["raw", "price", "percentage", "rate", "index", "count", "temperature"],
                              help="Type of variable (for range validation)")
    quality_parser.add_argument("--output", type=str, help="Output file for results (JSON)")
    quality_parser.add_argument("--visualize", action="store_true", help="Generate visualizations")
    
    # 'anomaly-detect' command - NEW COMMAND
    anomaly_parser = subparsers.add_parser("anomaly-detect", help="Identify and report anomalies")
    anomaly_parser.add_argument("--variable", type=str, required=True, help="Name of variable to analyze")
    anomaly_parser.add_argument("--method", type=str, default="zscore",
                              choices=["zscore", "iqr", "isolation_forest"],
                              help="Anomaly detection method")
    anomaly_parser.add_argument("--threshold", type=float, default=3.0,
                              help="Threshold for anomaly detection")
    anomaly_parser.add_argument("--window-size", type=int, help="Window size for contextual anomaly detection")
    anomaly_parser.add_argument("--variable-type", type=str, default="raw",
                              choices=["raw", "price", "percentage", "rate", "index", "count", "temperature"],
                              help="Type of variable (for range validation)")
    anomaly_parser.add_argument("--output", type=str, help="Output file for results (JSON)")
    anomaly_parser.add_argument("--visualize", action="store_true", help="Generate visualization")
    
    # 'cross-validate' command - NEW COMMAND
    cross_parser = subparsers.add_parser("cross-validate", help="Compare data from different sources")
    cross_parser.add_argument("--variable", type=str, required=True, help="Name of variable to cross-validate")
    cross_parser.add_argument("--output", type=str, help="Output file for results (JSON)")
    cross_parser.add_argument("--visualize", action="store_true", help="Generate visualization")
    
    # 'gap-analysis' command - NEW COMMAND
    gap_parser = subparsers.add_parser("gap-analysis", help="Identify and report on gaps in time series")
    gap_parser.add_argument("--variable", type=str, required=True, help="Name of variable to analyze")
    gap_parser.add_argument("--frequency", type=str, default="auto",
                           choices=["auto", "D", "B", "W", "M"],
                           help="Expected frequency of the data (D=daily, B=business, W=weekly, M=monthly)")
    gap_parser.add_argument("--min-gap-size", type=int, default=1,
    ...
    gap_parser.add_argument("--visualize", action="store_true", help="Generate visualization")
    ...
    repair_group.add_argument("--priority", type=int, help="Repair variables with this priority")
    repair_group.add_argument("--all", action="store_true", help="Repair all variables")
    repair_parser.add_argument("--variable-type", type=str, default="raw",
                            choices=["raw", "price", "percentage", "rate", "index", "count", "temperature"],
                            help="Type of variable (for strategy selection)")
    repair_parser.add_argument("--skip-smoothing", action="store_true",
                            help="Skip data smoothing step")
    repair_parser.add_argument("--skip-cross-source", action="store_true",
                            help="Skip cross-source reconciliation")
    
    # 'simulate-repair' command
    simulate_parser = subparsers.add_parser("simulate-repair", help="Simulate repairs without applying them")
    simulate_parser.add_argument("--variable", type=str, required=True,
                              help="Name of variable to simulate repairs on")
    simulate_parser.add_argument("--variable-type", type=str, default="raw",
                              choices=["raw", "price", "percentage", "rate", "index", "count", "temperature"],
                              help="Type of variable (for strategy selection)")
    simulate_parser.add_argument("--output", type=str, help="Output file for detailed simulation results (JSON)")
    
    # 'repair-report' command
    report_parser = subparsers.add_parser("repair-report", help="Generate a report of repairs made")
    report_group = report_parser.add_mutually_exclusive_group(required=True)
    report_group.add_argument("--variable", type=str, help="Name of variable to get repair report for")
    report_group.add_argument("--list-versions", type=str, metavar="VARIABLE",
                           help="List all repair versions for a variable")
    report_group.add_argument("--revert", type=str, metavar="VARIABLE",
                           help="Revert a variable to its original (pre-repair) state")
    report_group.add_argument("--compare", type=str, metavar="VARIABLE",
                           help="Compare two versions of a variable")
    report_parser.add_argument("--version", type=str, help="Version ID for operations that need a version")
    report_parser.add_argument("--version2", type=str,
