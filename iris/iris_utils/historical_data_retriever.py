"""iris.iris_utils.historical_data_retriever
===========================================
...
```
python -m iris.iris_utils.historical_data_retriever --variable spx_close
python -m iris.iris_utils.historical_data_retriever --priority 1
python -m iris.iris_utils.historical_data_retriever --all
```
"""
...
def get_priority_variables(priority_level: int = 1) -> List[Dict[str, Any]]:
    ...
    return [var for var in catalog["variables"] if var.get("priority") == priority_level]
...
def fetch_fred_data(
    ...
    if _FRED is None:
        raise ValueError("FRED API key not set. Set the FRED_API_KEY environment variable.")
    
    logger.info(f"Fetching FRED data for series {series_id} from {start_date.date()} to {end_date.date()}")
...
def fetch_yahoo_finance_data(
    ...
    logger.info(f"Fetching Yahoo Finance data for ticker {ticker} from {start_date.date()} to {end_date.date()}")
...
def retrieve_historical_data(
    ...
    data_dir = ensure_data_directory(
        f"historical_timeline/{variable_name}", 
        base_dir="data"
    )
    ...
    metadata_file = save_request_metadata(
        ...
        request_params,
        source_name=source,
        base_dir=f"data/historical_timeline"
    )
    ...
    try:
        ...
        if source_type == "YahooFinance":
            logger.info(f"Processing Yahoo Finance data with {len(data)} rows for {variable_name}")
            ...
            for date_idx, value in data_items:
                try:
                    ...
                    if pd.notna(value):  # Check for NaN
                        ...
                        logger.warning(f"Skipping NaN value for {variable_name} at {date_str}")
        ...
        # Handle FRED data
            ...
            if isinstance(data, pd.Series):
                ...
                for date_idx, value in data_items:
                    try:
                        # Format the date
                        date_str = date_idx.isoformat() if hasattr(date_idx, 'isoformat') else str(date_idx)
                ...
                logger.warning(f"FRED data for {variable_name} is not a pandas Series as expected")
        
        # Fallback for other data types
        else:
            logger.warning(f"Unsupported data format for {variable_name}. No values processed.")
        ...
        logger.info(f"Data points: {stats.data_point_count}, Completeness: {stats.completeness_pct:.2f}%")
        
        if stats.gaps:
            logger.warning(f"Found {len(stats.gaps)} gaps in data for {variable_name}")
        
        if stats.anomalies:
            logger.warning(f"Found {len(stats.anomalies)} anomalies in data for {variable_name}")
    ...
    if not data.empty:
        ...
        for date_idx in dates:
            try:
                ...
                current_date = pd.Timestamp(date_idx).to_pydatetime() if not isinstance(date_idx, dt.datetime) else date_idx
    ...
    if not data.empty and len(data) > 1:
        ...
        for idx, value in data.items():
            try:
                ...
                logger.warning(f"Could not process value {value} for anomaly detection: {e}")
    
    # Create and return statistics
    return RetrievalStats(
        variable_name=variable_name,
        source=str(data.name) if hasattr(data, "name") and data.name is not None else "unknown",
        ...
        mean_value=float(data.iloc[0]) if not data.empty and len(data) == 1 else float(data.mean()) if not data.empty else 0,
        median_value=float(data.iloc[0]) if not data.empty and len(data) == 1 else float(data.median()) if not data.empty else 0,
...
def retrieve_priority_variables(
    ...
    for var_info in priority_vars:
        try:
            ...
            logger.error(f"Failed to retrieve data for {var_info['variable_name']}: {e}")
    ...
    if report["successful_retrievals"] > 0:
        ...
        report["overall_metrics"] = {
            ...
            "variables_with_gaps": sum(
                1 for var_report in report["variable_reports"].values()
...
def main():
    ...
    parser.add_argument(
        "--end-date", 
        type=str, 
        help="End date for data retrieval (format: YYYY-MM-DD)"
    )
    ...
    if args.end_date:
        try:
            ...
            logger.error(f"Invalid end date format. Use YYYY-MM-DD.")
            return 1
    ...
    try:
        ...
        if "overall_metrics" in report:
            ...
            logger.info(f"Variables with gaps: {report['overall_metrics']['variables_with_gaps']}")
            logger.info(f"Variables with anomalies: {report['overall_metrics']['variables_with_anomalies']}")
