"""iris.iris_utils.world_bank_integration
======================================
...
```
python -m iris.iris_utils.world_bank_integration --file path/to/wb_data.csv
python -m iris.iris_utils.world_bank_integration --extract-zip path/to/wb_data.zip
```
"""
...
class PathSanitizingDataStore:
    ...
    def _store_dataset_fallback(self, dataset_name, data_items, metadata):
        ...
        logger.info(f"Stored dataset {dataset_name} using fallback method with {len(countries)} countries")
...
def process_world_bank_data(csv_path: str) -> Dict[str, Dict[str, List[Dict[str, Any]]]]:
    """
    ...
    try:
        ...
        try:
            ...
            logger.warning(f"Failed to process with pandas: {e}. Falling back to CSV reader.")
...
def transform_world_bank_data(
    ...
    for indicator, countries_data in wb_data.items():
        ...
        logger.info(f"Transformed {len(transformed_data[variable_name])} records for {variable_name}")
...
def load_world_bank_data_to_store(
    ...
    for variable_name, data_records in transformed_data.items():
        ...
        if variable_info is None:
            ...
            indicator_code = next(
                (code for code, info in WORLD_BANK_INDICATORS.items() 
                 if info["variable_name"] == variable_name),
                None
            )
            
            if indicator_code is None:
                logger.error(f"Variable {variable_name} not found in World Bank indicators")
                continue
        ...
        data_items = [
            {
                ...
                "metadata": {
                    ...
                    "tags": [variable_name, f"priority_{variable_info.get('priority', 3)}", "world_bank"]
                }
            }
        ...
        dataset_metadata = {
            ...
            "description": WORLD_BANK_INDICATORS.get(variable_info["api_endpoint"], {}).get("description", ""),
        ...
        try:
            ...
            start_date = datetime.fromisoformat(data_records[0]["timestamp"]) if data_records else None
            end_date = datetime.fromisoformat(data_records[-1]["timestamp"]) if data_records else None
            ...
            logger.info(f"Successfully stored {len(data_records)} records for {variable_name}")
    ...
    if success_count > 0:
        logger.info(f"World Bank data integration partially successful: {success_count} variables with {total_records} records")
        logger.info("Despite RecursiveDataStore storage errors, data is available in the historical_timeline directory")
...
def integrate_world_bank_data(
    ...
    if csv_path is None and zip_path is None:
        # Use default paths
        zip_path = os.path.join(MANUAL_DATA_DIR, "WB_DATA_d950d0cd269a601150c0afd03b234ee2.zip")
        if os.path.exists(zip_path):
            ...
            if csv_files:
                ...
                raise FileNotFoundError(f"World Bank data file not found")
    ...
    if success_count > 0:
        logger.info(f"Data successfully stored in historical_timeline directory structure")
        logger.info(f"Integration of World Bank bulk data completed")
...
def main():
    ...
    input_group.add_argument(
        "--file",
        type=str,
        help="Path to World Bank CSV file"
    )
    input_group.add_argument(
        "--zip",
        type=str,
        help="Path to World Bank ZIP file"
    )
