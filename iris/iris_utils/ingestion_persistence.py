...
DEFAULT_SENSITIVE_PARAMS: Set[str] = {
    "api_key", "key", "secret", "token", "password", "auth", "apikey", 
    "access_token", "client_secret", "app_secret", "private_key"
}


def ensure_data_directory(source_name: str, base_dir: Optional[str] = None) -> pathlib.Path:
...
def mask_sensitive_data(
    ...
    for param_name in sensitive_params:
        if param_name in masked_params and masked_params[param_name]:
            ...
            if len(value) > 4:
                masked_params[param_name] = value[0] + mask_char * (len(value) - 2) + value[-1]
            else:
