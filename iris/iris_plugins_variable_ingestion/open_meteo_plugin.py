...
class OpenMeteoPlugin(IrisPluginManager):
    ...
    def _safe_get(self, url: str, params: dict) -> Optional[dict]:
        ...
        for attempt in range(self.MAX_RETRIES + 1):
            try:
                ...
                logger.warning(f"Open-Meteo request failed ({attempt+1}/{self.MAX_RETRIES}): {exc}")
                if attempt < self.MAX_RETRIES:
                    time.sleep(self.RETRY_WAIT * (attempt + 1))
        return None

    def _fetch_current_weather(self, city_name: str, lat: float, lon: float, 
    ...
    def _fetch_daily_forecast(self, city_name: str, lat: float, lon: float,
                             timezone: str) -> List[Dict[str, Any]]:
    ...
    def _fetch_historical_monthly(self, city_name: str, lat: float, lon: float,
                                 timezone: str, year: int, month: int) -> List[Dict[str, Any]]:
        ...
        if "temperature_2m_mean" in daily_data and daily_data["temperature_2m_mean"]:
            ...
            if temps:
                ...
                signals.append({
                    ...
                    "timestamp": self._to_timestamp(f"{year}-{month:02d}-15")  # Middle of month
                })
