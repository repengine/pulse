...
def nasdaq_plugin() -> List[Dict[str, Any]]:
    ...
    if not signals and "WIKI/" not in "".join(dataset_list):  # Don't fallback if using old WIKI datasets
        logger.warning("[nasdaq_plugin] No data retrieved from specified datasets. Trying fallback datasets.")
    ...
    if accessible_datasets:
        logger.info("[nasdaq_plugin] Successfully accessed datasets: %s", accessible_datasets)
    else:
        logger.error("[nasdaq_plugin] Could not access any datasets. Check API key permissions.")
        logger.warning("[nasdaq_plugin] Falling back to mock data for demonstration purposes.")
...
def _fetch_latest(code: str) -> Optional[Dict[str, Any]]:
    ...
    try:
        ...
        if resp.status_code == 410:
            logger.error("[nasdaq_plugin] %s endpoint is no longer available (410 Gone). "
                         "This dataset has been deprecated. Try using an alternative dataset.", code)
            return None
        elif resp.status_code == 403:
            logger.error("[nasdaq_plugin] %s endpoint returned 403 Forbidden. "
                         "Your API key may not have access to this dataset.", code)
            return None
        elif resp.status_code == 404:
            logger.error("[nasdaq_plugin] %s endpoint returned 404 Not Found. "
                         "This dataset may not exist or has been renamed.", code)
            return None
        ...
        progress_file = os.path.join(ingest_dir, f"{code}_progress_{_dt.datetime.now().strftime('%Y%m%d_%H%M%S')}_start.json")
        ...
        progress_file = os.path.join(ingest_dir, f"{code}_progress_{_dt.datetime.now().strftime('%Y%m%d_%H%M%S')}_raw_extracted.json")
        with open(progress_file, 'w') as f:
            json.dump({
                ...
                "dataset_info": {
                    ...
                    "latest_date": latest[0] if latest and len(latest) > 0 else None
                }
            }, f, indent=2)
            
        # Get dataset type and code to determine structure
        dataset_type = code.split('/')[0].upper() if '/' in code else ""
        dataset_code = code.split('/')[1] if '/' in code and len(code.split('/')) > 1 else ""
        ...
        progress_file = os.path.join(ingest_dir, f"{code}_progress_{_dt.datetime.now().strftime('%Y%m%d_%H%M%S')}_basic_fields.json")
        with open(progress_file, 'w') as f:
            json.dump({
        ...
        if dataset_type == "EOD" or dataset_type == "WIKI":
            ...
            if len(latest) >= 6:
                ...
                progress_file = os.path.join(ingest_dir, f"{code}_progress_{_dt.datetime.now().strftime('%Y%m%d_%H%M%S')}_parse_ohlcv_start.json")
                with open(progress_file, 'w') as f:
                    json.dump({
                ...
                signal["meta"] = {
                    ...
                    "volume": float(latest[5]) if latest[5] is not None and len(latest) > 5 else None,
                }
                
                # Save progress after processing OHLCV data
                progress_file = os.path.join(ingest_dir, f"{code}_progress_{_dt.datetime.now().strftime('%Y%m%d_%H%M%S')}_parse_ohlcv_complete.json")
                with open(progress_file, 'w') as f:
                    json.dump({
                ...
                # Shorter data format
                progress_file = os.path.join(ingest_dir, f"{code}_progress_{_dt.datetime.now().strftime('%Y%m%d_%H%M%S')}_parse_short_start.json")
                with open(progress_file, 'w') as f:
                    json.dump({
                ...
                progress_file = os.path.join(ingest_dir, f"{code}_progress_{_dt.datetime.now().strftime('%Y%m%d_%H%M%S')}_parse_short_complete.json")
                with open(progress_file, 'w') as f:
                    json.dump({
            ...
            progress_file = os.path.join(ingest_dir, f"{code}_progress_{_dt.datetime.now().strftime('%Y%m%d_%H%M%S')}_parse_generic_start.json")
            with open(progress_file, 'w') as f:
                json.dump({
            ...
            signal["meta"] = {
                f"column_{i}": float(val) if val is not None and isinstance(val, (int, float, str)) and val != "" else val
                for i, val in enumerate(latest[1:], 1)
                if i < len(latest)
            }
            
            progress_file = os.path.join(ingest_dir, f"{code}_progress_{_dt.datetime.now().strftime('%Y%m%d_%H%M%S')}_parse_generic_complete.json")
            with open(progress_file, 'w') as f:
                json.dump({
        ...
        progress_file = os.path.join(ingest_dir, f"{code}_progress_{_dt.datetime.now().strftime('%Y%m%d_%H%M%S')}_processing_complete.json")
        with open(progress_file, 'w') as f:
            json.dump({
        ...
        progress_file = os.path.join(ingest_dir, f"{code}_progress_{_dt.datetime.now().strftime('%Y%m%d_%H%M%S')}_final.json")
        with open(progress_file, 'w') as f:
            json.dump({
        ...
        error_file = os.path.join(error_dir, f"{code}_error_{_dt.datetime.now().strftime('%Y%m%d_%H%M%S')}.json")
        ...
        if "Wiki" in str(exc) or "WIKI" in str(exc):
            ...
            logger.warning("[nasdaq_plugin] WIKI dataset is deprecated. Try using alternative datasets like LBMA/GOLD or FRED/GDP.")
        elif "403" in str(exc) or "Forbidden" in str(exc):
            error_type = "access_forbidden"
            logger.warning("[nasdaq_plugin] Access forbidden to %s. Your subscription may not include this dataset.", code)
        elif "404" in str(exc) or "Not Found" in str(exc):
            error_type = "not_found"
            logger.warning("[nasdaq_plugin] Dataset %s not found. It may have been renamed or removed.", code)
        else:
            error_type = "general_error"
        
        # Save categorized error information
        error_summary_file = os.path.join(error_dir, f"{code}_error_summary_{_dt.datetime.now().strftime('%Y%m%d_%H%M%S')}.json")
