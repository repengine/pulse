...
class GithubPlugin(IrisPluginManager):
    ...
    TECH_DOMAINS = {
        "ai": ["machine-learning", "artificial-intelligence", "deep-learning", "nlp", "tensorflow", "pytorch"],
        "web": ["javascript", "typescript", "react", "vue", "angular", "node", "frontend"],
        "backend": ["java", "python", "go", "rust", "c-sharp", "microservices", "api"],
        "mobile": ["android", "ios", "flutter", "react-native", "kotlin", "swift"],
        "cloud": ["kubernetes", "docker", "aws", "azure", "gcp", "devops", "serverless"],
        "blockchain": ["blockchain", "web3", "ethereum", "solidity", "nft", "cryptocurrency"],
    ...
    def __init__(self):
        ...
        if not self.token:
            logger.warning("GITHUB_TOKEN environment variable not set - plugin disabled")
    ...
    def _safe_get(self, endpoint: str, params: dict = None) -> Optional[Any]:
            ...
            try:
                ...
                if resp.status_code == 403 and "X-RateLimit-Remaining" in resp.headers:
                    if int(resp.headers["X-RateLimit-Remaining"]) == 0:
                        ...
                        logger.warning(f"GitHub API rate limit reached. Sleeping for {sleep_time:.1f} seconds.")
                ...
                logger.warning(f"GitHub API request failed ({attempt+1}/{self.MAX_RETRIES}): {exc}")
    ...
    def _calculate_domain_popularity(self, repositories: List[Dict[str, Any]]) -> float:
        ...
        score = total_stars + (total_forks * 2)  # Forks indicate higher engagement than stars
    ...
    def _track_specific_repos(self, repos: List[str]) -> List[Dict[str, Any]]:
        ...
        for repo_path in repos[:3]:  # Limit to 3 repos per run
            ...
            commits_result, _ = self._safe_get(f"repos/{repo_path}/commits", {"per_page": 30})
            if commits_result and isinstance(commits_result, list):
    ...
    def _analyze_activity_trends(self, domain_name: str, topics: List[str]) -> List[Dict[str, Any]]:
