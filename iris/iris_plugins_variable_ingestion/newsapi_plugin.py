...
class NewsapiPlugin(IrisPluginManager):
    ...
    TOPICS = {
        "finance": ["economy", "finance", "stock market", "recession", "inflation", "crypto", "dollar", "fed"],
        "geopolitics": ["diplomacy", "conflict", "war", "election", "summit", "treaty", "sanctions", "trade war"],
        "technology": ["AI", "artificial intelligence", "innovation", "tech", "programming", "cybersecurity", "startup"],
        "climate": ["climate change", "global warming", "environment", "renewable energy", "carbon", "pollution"],
        "health": ["healthcare", "pandemic", "virus", "medicine", "vaccine", "disease", "epidemic", "public health"]
    }
    ...
    POSITIVE_KEYWORDS = [
        "growth", "recovery", "gain", "boost", "success", "positive", "improvement",
        "breakthrough", "advance", "progress", "optimism", "opportunity", "promising",
        "benefit", "prosper", "thrive", "rebound", "triumph"
    ]
    
    NEGATIVE_KEYWORDS = [
        "decline", "fall", "drop", "loss", "crisis", "downturn", "concern", "risk",
        "warning", "threat", "danger", "failure", "struggle", "conflict", "crash",
        "recession", "problem", "fear", "tension", "collapse", "disaster"
    ]
    ...
    def _safe_get(self, endpoint: str, params: dict) -> Optional[dict]:
        ...
        for attempt in range(self.MAX_RETRIES + 1):
            try:
                ...
                logger.warning(f"NewsAPI request failed ({attempt+1}/{self.MAX_RETRIES}): {exc}")
                if attempt < self.MAX_RETRIES:
                    time.sleep(self.RETRY_WAIT * (attempt + 1))
        return None

    def _fetch_topic_headlines(self, topic_name: str, keywords: List[str]) -> List[Dict[str, Any]]:
        ...
        if len(all_articles) < 50 and len(self.NEWS_SOURCES) > 5:
            params["sources"] = ",".join(self.NEWS_SOURCES[5:10])  # Next batch of sources
            response = self._safe_get("top-headlines", params)
            if response and "articles" in response:
                all_articles.extend(response["articles"])
    ...
    def _calculate_coverage_intensity(self, articles: List[Dict[str, Any]]) -> Optional[float]:
        ...
        for article in articles:
            if "publishedAt" in article and article["publishedAt"]:
                try:
                    pub_time = dt.datetime.fromisoformat(article["publishedAt"].replace('Z', '+00:00'))
    ...
    def _extract_subtopic_trends(self, articles: List[Dict[str, Any]], topic_keywords: List[str]) -> Dict[str, float]:
        ...
        stopwords = {"the", "and", "of", "to", "in", "a", "for", "on", "is", "that", "with", "said", "by", "as", "at", "from", "it", "was"}
        for word in words:
        ...
        for word, count in word_counter.most_common(10):
            normalized_score = min(100, count * 5)  # Adjust multiplier based on typical counts
            combined_trends[word] = normalized_score
            
        # Add top bigrams
        for bigram, count in bigram_counter.most_common(5):
            normalized_score = min(100, count * 10)  # Bigrams typically have lower counts
