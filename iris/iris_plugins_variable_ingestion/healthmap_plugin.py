...
class HealthmapPlugin(IrisPluginManager):
    ...
    def _safe_get(self, url: str, dataset_id: str = "unknown") -> Optional[str]:
        ...
        save_request_metadata(
            dataset_id, {"url": url}, source_name=_SOURCE_NAME, url=url
        )
        ...
        for attempt in range(self.MAX_RETRIES + 1):
            try:
                ...
                logger.warning(
                    f"HealthMap request failed ({attempt + 1}/{self.MAX_RETRIES}): {exc}"
                )
        ...
        logger.error(
            f"Failed to fetch data from HealthMap after {self.MAX_RETRIES} attempts"
        )
        return None

    def _fetch_health_events(
        self, feed_url: str, region_key: str
        ...
        try:
            ...
            for item in channel.findall("item"):
                ...
                event["location"] = self._extract_location(
                    event["title"], event["description"]
                )
                event["category"] = self._extract_category(
                    event["title"], event["description"]
                )
