...
class NasaPowerPlugin(IrisPluginManager):
    ...
    def fetch_signals(self) -> List[Dict[str, Any]]:
        ...
        location_signals = self._fetch_location_data(location_name, location["lat"], location["lon"])
        signals.extend(location_signals)
        
        return signals
    
    def _fetch_location_data(self, location_name: str, lat: float, lon: float) -> List[Dict[str, Any]]:
        ...
        end_date = dt.datetime.now() - dt.timedelta(days=1)  # Yesterday (NASA data has a delay)
        ...
        for attempt in range(self.MAX_RETRIES + 1):
            try:
                ...
                logger.warning(f"[nasa_power_plugin] Request attempt {attempt+1} failed: {e}")
                if attempt < self.MAX_RETRIES:
                    time.sleep(self.RETRY_WAIT)
        
        # If all attempts failed, return empty list
        if not response_data:
            logger.error(f"[nasa_power_plugin] Failed to fetch data for {location_name} after {self.MAX_RETRIES + 1} attempts")
            return signals
        ...
        try:
            ...
            logger.info(f"[nasa_power_plugin] Successfully received data for {location_name}")
            ...
            if not most_recent:
                logger.warning(f"[nasa_power_plugin] No valid data found for {location_name}")
                return signals
            
            # Generate signals for each parameter
            for nasa_param, param_name in _PARAMETERS.items():
                if nasa_param in parameter_data and most_recent in parameter_data[nasa_param]:
            ...
            logger.error(f"[nasa_power_plugin] Error processing data for {location_name}: {e}")
    ...
    def _get_most_recent_date_with_data(self, parameter_data: Dict[str, Dict[str, float]]) -> Optional[str]:
