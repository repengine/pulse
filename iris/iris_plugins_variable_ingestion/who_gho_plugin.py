...
class WhoGhoPlugin(IrisPluginManager):
    ...
    INDICATORS = {
        ...
        "maternal_mortality": ("MDG_0000000004", "Maternal mortality ratio (per 100 000 live births)"),
        "tuberculosis_incidence": ("MDG_0000000025", "Tuberculosis incidence (per 100 000 population)"),
        "malaria_incidence": ("MALARIA_EST_CASES", "Malaria incidence (per 1 000 population)"),
        "hiv_prevalence": ("HIV_0000000001", "HIV prevalence among adults aged 15 to 49 (%)"),
        "overweight_prevalence": ("NCD_BMI_25A", "Prevalence of overweight among adults (%)"),
        "obesity_prevalence": ("NCD_BMI_30A", "Prevalence of obesity among adults (%)"),
        "alcohol_consumption": ("SA_0000001462", "Total alcohol consumption per capita (liters)"),
        "tobacco_prevalence": ("M_Est_smk_curr_std", "Age-standardized prevalence of tobacco smoking among persons aged 15+ years"),
        "uhc_coverage_index": ("UHC_INDEX_REPORTED", "Universal health coverage (UHC) service coverage index"),
        "medical_doctors_density": ("HWF_0001", "Medical doctors (per 10,000 population)"),
        "hospital_beds_density": ("HFAC_0078", "Hospital bed density (per 10,000 population)")
    }
    ...
    def fetch_signals(self) -> List[Dict[str, Any]]:
        ...
        for var_name, (indicator_code, _) in today_indicators.items():
            ...
            for country_code in self.COUNTRIES:
                country_data = self._fetch_indicator_data(indicator_code, country_code, latest_available_year)
                if country_data:
    ...
    def _safe_get(self, url: str, params: dict = None) -> Optional[dict]:
        ...
        for attempt in range(self.MAX_RETRIES + 1):
            try:
                ...
                logger.warning(f"WHO GHO API request failed ({attempt+1}/{self.MAX_RETRIES}): {exc}")
                if attempt < self.MAX_RETRIES:
                    time.sleep(self.RETRY_WAIT * (attempt + 1))
        return None

    def _fetch_indicator_metadata(self, indicator_code: str) -> Optional[Dict[str, Any]]:
    ...
    def _fetch_indicator_data(self, indicator_code: str, country_code: str, year: int) -> List[Dict[str, Any]]:
        ...
        for entry in indicator_data:
            ...
            try:
                ...
                # Create signal name
                var_base_name = next((k for k, v in self.INDICATORS.items() if v[0] == indicator_code), indicator_code.lower())
                ...
                signals.append({
                    ...
                    "metadata": {
                        ...
                        "indicator": entry.get("IndicatorName", self.INDICATORS.get(var_base_name, (indicator_code, ""))[1]),
                        "year": year,
                ...
                logger.error(f"Error processing WHO data for {indicator_code}/{country_code}: {e}")
