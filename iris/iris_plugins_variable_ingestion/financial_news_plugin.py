...
class FinancialNewsPlugin(IrisPluginManager):
    ...
    def __init__(self):
        ...
        if self.alphavantage_api_key or self.newsapi_key:
            self.enabled = True
            logger.info(f"Financial News plugin enabled with {sum(bool(k) for k in [self.alphavantage_api_key, self.newsapi_key])} API key(s)")
        else:
    ...
    def fetch_signals(self) -> List[Dict[str, Any]]:
        ...
        if hour_of_day in [4, 16] and now.minute < 15:  # Only in the first 15 min of those hours
    ...
    def _safe_get_alphavantage(self, function: str, params: dict, dataset_id: str) -> Optional[dict]:
        ...
        logger.error(f"Failed to fetch data from Alpha Vantage after {self.MAX_RETRIES} attempts")
        return None
    
    def _safe_get_newsapi(self, endpoint: str, params: dict, dataset_id: str) -> Optional[dict]:
        ...
        # If all attempts failed, log the error
        logger.error(f"Failed to fetch data from NewsAPI after {self.MAX_RETRIES} attempts")
        return None
    ...
    def _fetch_company_news(self, symbol: str) -> Optional[Dict[str, Any]]:
        ...
        if self.alphavantage_api_key:
            ...
            response_data = self._safe_get_alphavantage("NEWS_SENTIMENT", params, dataset_id)
    ...
    def _calculate_sentiment_score(self, text: str) -> float:
        ...
        positive_terms = [
            "growth", "profit", "surge", "gain", "boost", "rally", "bullish", 
            "outperform", "beat", "upgrade", "rise", "record", "jump", "recovery",
            "strong", "positive", "optimistic", "confidence", "upside"
        ]
        
        negative_terms = [
            "decline", "drop", "fall", "loss", "plunge", "bearish", "underperform",
            "miss", "downgrade", "decrease", "weak", "negative", "pessimistic", 
            "concern", "risk", "warning", "crash", "crisis", "trouble", "downturn"
        ]
        ...
        # Count occurrences
        positive_count = sum(1 for term in positive_terms if re.search(rf'\b{term}\b', text_lower))
        negative_count = sum(1 for term in negative_terms if re.search(rf'\b{term}\b', text_lower))
    ...
    def _process_company_news(self, symbol: str, news_data: Dict[str, Any], 
    ...
    def _process_sector_news(self, sector: str, news_data: Dict[str, Any], 
                           timestamp: dt.datetime) -> List[Dict[str, Any]]:
    ...
    def _process_topic_news(self, topic: str, news_data: Dict[str, Any], 
                          timestamp: dt.datetime) -> List[Dict[str, Any]]:
    ...
    def _process_market_news(self, news_data: Dict[str, Any], 
                           timestamp: dt.datetime) -> List[Dict[str, Any]]:
