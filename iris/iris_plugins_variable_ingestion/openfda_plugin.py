...
class OpenfdaPlugin(IrisPluginManager):
    ...
    def fetch_signals(self) -> List[Dict[str, Any]]:
        ...
        endpoint_signals = self._fetch_endpoint_data(endpoint_name, endpoint_path, time_window)
        signals.extend(endpoint_signals)
        
        return signals
    
    def _fetch_endpoint_data(self, endpoint_name: str, endpoint_path: str, time_window: str) -> List[Dict[str, Any]]:
        ...
        search_query = self._get_search_query(endpoint_name, start_date_str, end_date_str)
        ...
        for attempt in range(self.MAX_RETRIES + 1):
            try:
                ...
                logger.warning(f"[openfda_plugin] Request attempt {attempt+1} failed: {e}")
                if attempt < self.MAX_RETRIES:
                    time.sleep(self.RETRY_WAIT)
        
        # If all attempts failed, return empty list
        if not response_data:
            logger.error(f"[openfda_plugin] Failed to fetch data for {endpoint_name} after {self.MAX_RETRIES + 1} attempts")
            return signals
        ...
        try:
            if "results" in response_data:
                ...
                if count_field and "count" in response_data:
                    ...
                    signals.extend(self._process_count_data(endpoint_name, count_field, count_results, time_window))
                # For regular queries
                elif "results" in response_data:
                    results = response_data.get("results", [])
                    signals.extend(self._process_regular_data(endpoint_name, results, time_window))
                    
                logger.info(f"[openfda_plugin] Successfully processed {len(signals)} signals for {endpoint_name}")
            else:
            ...
            logger.error(f"[openfda_plugin] Error processing data for {endpoint_name}: {e}")
    ...
    def _get_search_query(self, endpoint_name: str, start_date: str, end_date: str) -> str:
    ...
    def _process_count_data(self, endpoint_name: str, count_field: str, count_results: List[Dict], time_window: str) -> List[Dict[str, Any]]:
    ...
    def _process_regular_data(self, endpoint_name: str, results: List[Dict], time_window: str) -> List[Dict[str, Any]]:
        ...
        if endpoint_name == "drug_events":
            ...
            for event in results:
                ...
                if "reaction" in patient:
                    for reaction in patient.get("reaction", []):
                        if reaction.get("reactionoutcome") in ["1", "2", "3"]:  # Serious outcomes
                            serious_count += 1
            ...
            if results:
                serious_signal = {
                    ...
                    "value": float(serious_count) / len(results) * 100 if results else 0,
                    "source": "openfda",
                    "timestamp": dt.datetime.now(dt.timezone.utc).isoformat(),
                    "metadata": {
                        "endpoint": endpoint_name,
            ...
            if results:
                voluntary_signal = {
                    ...
                    "value": float(voluntary_count) / len(results) * 100 if results else 0,
                    "source": "openfda",
                    "timestamp": dt.datetime.now(dt.timezone.utc).isoformat(),
                    "metadata": {
                        "endpoint": endpoint_name,
