...
class CensusPlugin:
    def __init__(self):
        ...
        self.api_key = os.environ.get("CENSUS_API_KEY") # TODO: Add CENSUS_API_KEY configuration
    ...
    def fetch_data(self, endpoint, params):
        ...
        max_results = params.get('max_results', 50000)  # Safety limit to prevent infinite loops
        ...
        while url and len(all_observations) < max_results:
            try:
                logging.info(f"Fetching Census data from {url} with params {current_params}")
                ...
                if data and len(data) > 1:
                    ...
                    # Log progress
                    logging.info(f"Retrieved {batch_size} records, total: {len(all_observations)}")
                    
                    # Check if we received fewer records than requested - indicates end of data
                    if batch_size < limit:
                        logging.info("Received fewer records than requested, ending pagination")
                        break
                    ...
                    logging.warning(f"No data found for Census endpoint {endpoint} with params {current_params}.")
                    break
        ...
        logging.info(f"Completed Census data retrieval with {len(all_observations)} total records")
    ...
    def ingest_retail_sales(self):
        ...
        endpoint = "2023/eits/resretaill"  # Monthly Retail Trade Survey - U.S. Retail Sales
        params = {
            ...
            for obs in observations:
                ...
                if timestamp_str and value is not None:
                    try:
                        ...
                        if date_obj:
                            timestamp = date_obj.isoformat()  # Convert to ISO format for consistency
                        else:
                            logging.warning(f"Could not parse date format for '{timestamp_str}'. Skipping observation.")
                            continue
                        ...
                        logging.debug(f"Saved retail sales data point for {timestamp}: {value} {unit}")
                    except ValueError as e:
                        logging.error(f"Could not convert value '{value}' to float for Retail Sales on date {timestamp_str}: {str(e)}")
                        continue
            print(f"Successfully ingested {len(observations)} observations for Retail Sales.")
        else:
            ...
            for obs in observations:
                ...
                if timestamp_str and value is not None:
                    try:
                        ...
                        save_data_point_incremental(data_point, "economic_indicators")
                        logging.debug(f"Saved {data_type} data point for {timestamp}: {value} {unit}")
                    except ValueError as e:
                        logging.error(f"Could not convert value '{value}' to float for {data_type} on date {timestamp_str}: {str(e)}")
                        continue
            print(f"Successfully ingested {len(observations)} observations for Housing Starts & Building Permits.")
        else:
            print("No data or error fetching data for Housing Starts & Building Permits.")
