...
class WorldBankPlugin(IrisPluginManager):
    ...
    COUNTRIES = {
        ...
        "africa": ["ZA", "NG", "EG", "ET", "KE"],                  # South Africa, Nigeria, Egypt, Ethiopia, Kenya
        "middle_east": ["SA", "AE", "IR", "IL", "TR"],             # Saudi Arabia, UAE, Iran, Israel, Turkey
    ...
    def fetch_signals(self) -> List[Dict[str, Any]]:
        ...
        if not indicator_data:
            logger.warning(f"No data found for indicator {indicator_key} in region {region_key}")
            return signals
        
        # Process data into signals
        signals.extend(self._process_indicator_data(
            indicator_data, 
            indicator_key, 
            region_key,
            now
    ...
    def _safe_get(self, url: str, params: dict, dataset_id: str) -> Optional[dict]:
        ...
        for attempt in range(self.MAX_RETRIES + 1):
            try:
                ...
                logger.warning(f"World Bank request failed ({attempt+1}/{self.MAX_RETRIES}): {exc}")
        ...
        logger.error(f"Failed to fetch data from World Bank after {self.MAX_RETRIES} attempts")
        return None
    
    def _fetch_indicator_data(self, country_codes: List[str], indicator_code: str) -> Optional[Dict[str, Any]]:
        ...
        if not response_data or not isinstance(response_data, list) or len(response_data) < 2:
            logger.error(f"Invalid response format from World Bank API for indicator {indicator_code}")
            return None
        ...
        try:
            ...
            for point in data_points:
                if not all(k in point for k in ["country", "countryiso3code", "value", "date"]):
                    continue
                ...
                if country_code not in country_data or date > country_data[country_code]["date"]:
    ...
    def _process_indicator_data(self, indicator_data: Dict[str, Any], indicator_key: str,
                              region_key: str, timestamp: dt.datetime) -> List[Dict[str, Any]]:
