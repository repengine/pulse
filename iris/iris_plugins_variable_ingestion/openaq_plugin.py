...
from iris.iris_plugins_variable_ingestion.openaq_plugin import OpenaqPlugin
...
_CITIES = [
    ...
    "Paris", "Los Angeles", "Mexico City", "Cairo", "Mumbai"
]
    ...
    def _fetch_city_data(self, city: str) -> List[Dict[str, Any]]:
        ...
        for attempt in range(self.MAX_RETRIES + 1):
            try:
                ...
                logger.warning(f"[openaq_plugin] Request attempt {attempt+1} failed: {e}")
                if attempt < self.MAX_RETRIES:
                    time.sleep(self.RETRY_WAIT)
        
        # If all attempts failed, return empty list
        if not response_data:
            logger.error(f"[openaq_plugin] Failed to fetch data for {city} after {self.MAX_RETRIES + 1} attempts")
            return signals
        ...
        try:
            ...
            logger.info(f"[openaq_plugin] Found {len(results)} monitoring stations for {city}")
                ...
                for measurement in measurements:
                    ...
                    if parameter and value is not None:
                        ...
                        signal = {
                            ...
                            "value": float(value),
                            "source": "openaq",
                            "timestamp": last_updated or dt.datetime.now(dt.timezone.utc).isoformat(),
                            "metadata": {
                                "city": city,
