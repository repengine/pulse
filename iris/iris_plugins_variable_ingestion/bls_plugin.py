...
class BLSPlugin:
    ...
    def fetch_series_data(self, series_id, start_year=None, end_year=None):
        ...
        try:
            ...
            if result and result['status'] == 'REQUEST_SUCCEEDED':
                ...
                print(f"BLS API request failed for {series_id}: {result.get('message', 'Unknown error')}")
                return None
    ...
    def ingest_inflation_measures(self):
        ...
        for name, series_id in series_ids.items():
            ...
            observations = self.fetch_series_data(series_id, start_year=1900, end_year=datetime.now().year)
            ...
            if observations:
                for obs in observations:
                    ...
                    year = int(obs['year'])
                    period = obs['period']
                    periodicity = obs['periodicity'] # 'M' for monthly, 'Q' for quarterly, 'A' for annual
                    ...
                    if periodicity == 'M':
                        ...
                        print(f"Unknown periodicity {periodicity} for series {series_id}. Skipping observation.")
                        continue
                    ...
                    try:
                        ...
                        save_data_point_incremental(name, date_str, value, source_name="BLS", metadata={"series_id": series_id})
                    except ValueError:
                         print(f"Could not convert value '{obs['value']}' to float for series {series_id} on date {date_str}. Skipping observation.")
                         continue
