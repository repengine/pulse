    ...
    for step_idx, step in enumerate(trace_data):
        ...
        if dominant_pillars:
            ...
            for pillar_info in dominant_pillars:
                ...
                print(f"    {pillar_name}: weight={weight:+.4f} (contribution: {contribution_pct:.1f}%)")
                ...
                if source_points:
                    ...
                    if len(source_points) > 3:
                        print(f"      ... and {len(source_points) - 3} more data points")
        
        print("\n" + "-" * 50)
    
    if not found_data:
        print(f"No gravity correction data found for variable '{variable_name}'")
        print("Make sure simulation was run with '--explain-gravity' flag and variable exists in the simulation.")
...
def plot_gravity_correction_details_html(
    ...
    if not PLOTLY_AVAILABLE:
        return _plot_gravity_correction_details_matplotlib(trace_data, variable_name, output_path, max_steps)
...
def extract_trace_data_for_variable(trace_data: List[Dict], variable_name: str) -> List[Dict]:
    """
...
def export_gravity_explanation_json(trace_data: List[Dict], variable_name: str, output_path: str) -> str:
    """
    ...
    if not variable_data:
        logger.warning(f"No gravity correction data found for variable '{variable_name}'")
        return ""
...
def get_top_source_data_points_for_pillar(trace_data: List[Dict], variable_name: str, pillar_name: str, top_n: int = 5) -> List[Dict]:
    """
