...
class ShadowModelMonitor:
    ...
    """
    def __init__(self, threshold: float, window_steps: int, critical_variables: list[str]):
        """
        ...
        if not critical_variables:
            logger.warning("ShadowModelMonitor initialized with no critical variables to monitor.")
        ...
        self.delta_window: deque[tuple[dict[str, float], dict[str, float]]] = deque(maxlen=window_steps)
        self.current_step_internal = 0 # For internal tracking if needed, though `current_step` is passed in `record_step`

    def record_step(self, causal_deltas: dict[str, float], gravity_deltas: dict[str, float], current_step: int):
        """
        ...
        causal_deltas_critical = {var: causal_deltas.get(var, 0.0) for var in self.critical_variables if var in causal_deltas}
        gravity_deltas_critical = {var: gravity_deltas.get(var, 0.0) for var in self.critical_variables if var in gravity_deltas}
    ...
    def calculate_variance_explained(self, variable: str) -> float:
        ...
        if variable not in self.critical_variables:
            logger.warning(f"Variable '{variable}' not in critical_variables list for ShadowModelMonitor.")
        ...
        for causal_step_deltas, gravity_step_deltas in self.delta_window:
            causal_deltas_for_var_in_window.append(causal_step_deltas.get(variable, 0.0))
            gravity_deltas_for_var_in_window.append(gravity_step_deltas.get(variable, 0.0))
...
if __name__ == '__main__':
    ...
    steps_data = [
        # Step 0
        ({"var1": 1.0, "var2": 2.0, "var3": 0.5, "var4_ignored": 10.0}, # Causal deltas
         {"var1": 0.1, "var2": 0.3, "var3": 0.05, "var4_ignored": 1.0}), # Gravity deltas
        # Step 1
        ({"var1": 1.2, "var2": 2.1, "var3": 0.6},
         {"var1": 0.8, "var2": 0.5, "var3": 0.1}), # var1 gravity is higher
        # Step 2 - Window is now full
        ({"var1": 0.9, "var2": 1.9, "var3": 0.4},
         {"var1": 0.7, "var2": 1.5, "var3": 0.05}), # var2 gravity is higher
        # Step 3 - Window rolls
        ({"var1": 1.1, "var2": 0.5, "var3": 0.5},
         {"var1": 0.1, "var2": 1.8, "var3": 0.1}), # var2 gravity remains high
    ]
    ...
    for i, (cd, gd) in enumerate(steps_data):
        ...
        if len(monitor.delta_window) >= monitor.window_steps:
            for var_to_check in monitor.critical_variables:
                ...
                logger.info(f"Variance explained by gravity for '{var_to_check}': {ve*100:.2f}%")
            
            triggered, problematic_vars = monitor.check_trigger()
            if triggered:
                logger.warning(f"TRIGGERED at step {i}! Problematic variables: {problematic_vars}")
            else:
                logger.info(f"No trigger at step {i}.")
        else:
            logger.info(f"Window not full yet ({len(monitor.delta_window)}/{monitor.window_steps} steps).")
    ...
    logger.info(f"Variance explained for v_zero (all zeros): {ve_zero}") # Should be 0.0
    triggered_zero, _ = monitor_edge.check_trigger()
    logger.info(f"Triggered for v_zero (all zeros): {triggered_zero}") # Should be False
    ...
    logger.info(f"Variance explained for v_zero (causal only): {ve_causal_only}") # Should be 0.0
    triggered_causal_only, _ = monitor_edge.check_trigger()
    logger.info(f"Triggered for v_zero (causal only): {triggered_causal_only}") # Should be False
    ...
    logger.info(f"Variance explained for v_zero (gravity only): {ve_gravity_only}") # Should be 1.0
    triggered_gravity_only, _ = monitor_edge.check_trigger() # Threshold is 0.5
    logger.info(f"Triggered for v_zero (gravity only): {triggered_gravity_only}") # Should be True