...
def test_nasdaq_incremental():
    ...
    if not os.environ.get("NASDAQ_API_KEY"):
        print("WARNING: NASDAQ_API_KEY environment variable not set.")
        print("Set this variable to test the NASDAQ plugin.")
        return False
...
def test_high_frequency_store():
    ...
    file_path = os.path.join("data/high_frequency_data_test", f"{test_var}.jsonl")
    if os.path.exists(file_path):
        ...
        with open(file_path, 'r') as f:
            ...
            if data.get("timestamp") == timestamp and data.get("value") == 123.45:
                print("SUCCESS: Data was written correctly")
                return True
            else:
                print("ERROR: Data in file doesn't match what was written")
                return False
    else:
