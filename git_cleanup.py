...
def suggest_lfs_setup():
    ...
    print("\n--- GIT LFS SUGGESTIONS ---")
    print("If your repository contains large binary files that you need to track, consider using Git LFS.")
...
def cleanup_instructions():
    ...
    print("2. Remove files from Git's history: Run this command to purge large files based on .gitignore:")
    print("   git filter-branch --index-filter 'git rm -rf --cached --ignore-unmatch $(git ls-files -ci --exclude-standard)' --prune-empty --tag-name-filter cat -- --all")
    ...
    print("   cd yourrepo.git && git reflog expire --expire=now --all && git gc --prune=now --aggressive")
...
def main():
    ...
    if repo_size > 1000:  # More than 1GB
        ...
        print("Your repository size is reasonable, but cleaning can still be beneficial.")
