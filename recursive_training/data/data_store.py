...
class RecursiveDataStore:
    ...
    @classmethod
    def get_instance(cls, config: Optional[Dict[str, Any]] = None) -> 'RecursiveDataStore':
        """
    ...
    def _get_storage_path(self, item_id: str, version: Optional[int] = None) -> Path:
        ...
        """
        # Use first few chars of the ID for directory structure to avoid too many files in one directory
        prefix = item_id[:2]
    ...
    def _update_indices(self, item_id: str, metadata: Dict[str, Any]) -> None:
        ...
        timestamp = metadata.get("ingestion_timestamp", datetime.now(timezone.utc).isoformat())
    ...
    def _get_current_version(self, item_id: str) -> int:
        ...
        versions = [
            int(f.name[1:-5]) for f in item_dir.glob("v*.data")
    ...
    def _update_storage_stats(self, item_id: str, size: int, metadata: Dict[str, Any]) -> None:
        """
    ...
    def _cleanup_old_versions(self, item_id: str) -> None:
        ...
        version_files = [
            (int(f.name[1:-5]), f) for f in item_dir.glob("v*.data")
            if f.name.startswith("v") and f.name.endswith(".data")
        ]
        ...
        for version, file_path in version_files[self.max_versions:]:
            try:
                ...
                self.logger.error(f"Failed to remove old version {version} of {item_id}: {e}")
    ...
    def store_dataset(self, dataset_name: str, data_items: List[Dict[str, Any]], metadata: Optional[Dict[str, Any]] = None) -> str:
        """
        ...
        Returns:
            Dataset ID
        """
        dataset_id = f"dataset_{dataset_name}_{datetime.now(timezone.utc).strftime('%Y%m%d%H%M%S')}"
    ...
    def retrieve_dataset(self, dataset_name: str, dataset_id: Optional[str] = None) -> Tuple[List[Any], Dict[str, Any]]:
        """
        Retrieve a dataset.
                    ...
                    if item_dir.is_dir():
                        ...
                        if metadata_path.exists():
                            try:
                                ...
                                self.logger.error(f"Failed to update storage stats for {item_dir}: {e}")
