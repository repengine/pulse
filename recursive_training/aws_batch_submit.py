...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description="Submit Pulse retrodiction training jobs to AWS Batch"
    )
    ...
    parser.add_argument(
        "--job-queue",
        type=str,
        default="pulse-retrodiction-job-queue",
        help="AWS Batch job queue name",
    )
    parser.add_argument(
        "--job-definition",
        type=str,
        default="pulse-retrodiction-training",
    ...
    parser.add_argument(
        "--vcpus", type=int, default=4, help="Number of vCPUs to allocate to the job"
    )
    parser.add_argument(
        "--memory",
        type=int,
    ...
    parser.add_argument(
        "--dask-threads", type=int, default=2, help="Number of threads per Dask worker"
    )
    parser.add_argument(
        "--job-timeout",
        type=int,
    ...
    parser.add_argument(
        "--monitor", action="store_true", help="Monitor job status after submission"
    )
    parser.add_argument(
        "--poll-interval",
        type=int,
...
def submit_batch_job(
    ...
    try:
        ...
        _timeout_minutes = max(1, timeout_seconds // 60)
...
def monitor_job(
    batch_client: boto3.client, job_id: str, poll_interval: int = 30
) -> Dict[str, Any]:
    """
    ...
    while status not in terminal_states:
        ...
        if (
            "container" in job_info
            and "log_stream_name" in job_info["container"]
            and job_info["container"]["log_stream_name"]
        ):
...
def build_environment(args) -> List[Dict[str, str]]:
    ...
    """
    environment = [
        {"name": "S3_DATA_BUCKET", "value": args.s3_data_bucket},
        {"name": "S3_RESULTS_BUCKET", "value": args.s3_results_bucket},
    ...
    if args.use_dask:
        environment.extend(
            [
                {"name": "USE_DASK", "value": "true"},
                {"name": "DASK_THREADS", "value": str(args.dask_threads)},
            ]
        )
...
def main():
    ...
    try:
        ...
        if args.monitor:
            ...
            logger.info("Monitor the job status in the AWS Management Console or run:")
            logger.info(
                f"python -m recursive_training.aws_batch_submit_status --job-id {job_details['job_id']}"
            )
