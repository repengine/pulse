...
class CostController:
    ...
    def _load_historical_data(self):
        ...
        try:
            ...
            self.logger.info(f"Loaded historical cost data: daily=${self.current_day_cost:.2f}, "
    ...
    def _update_status(self):
        ...
        daily_percentage = (self.current_day_cost / self.daily_limit) * 100 if self.daily_limit > 0 else 0
        monthly_percentage = (self.current_month_cost / self.monthly_limit) * 100 if self.monthly_limit > 0 else 0
        
        if daily_percentage >= 100 or monthly_percentage >= 100:
            self.current_status = CostStatus.SHUTDOWN
        elif daily_percentage >= self.critical_threshold or monthly_percentage >= self.critical_threshold:
    ...
    def _check_day_change(self):
        ...
        if current_date != self.current_day:
            ...
            if current_month != self.current_month:
                self.logger.info(f"Month changed from {self.current_month} to {current_month}")
    ...
    def track_cost(self, api_calls: int = 0, token_usage: int = 0, 
                  direct_cost: Optional[float] = None) -> Dict[str, Any]:
        """
        ...
        return {
            ...
            "daily_limit_percentage": (self.current_day_cost / self.daily_limit) * 100 if self.daily_limit > 0 else 0,
            "monthly_limit_percentage": (self.current_month_cost / self.monthly_limit) * 100 if self.monthly_limit > 0 else 0
        }
    
    def track_operation(self, operation_type: str, data_size: int = 0, duration: float = 0.0, cost: float = 0.0):
        """
        ...
        self.logger.debug(f"Tracking operation: {operation_type}, data_size: {data_size}, duration: {duration}, cost: {cost}")
    ...
    def can_make_api_call(self, count: int = 1, token_estimate: int = 0, 
                         check_cost: bool = True) -> bool:
        """
    ...
    def get_cost_summary(self) -> Dict[str, Any]:
        ...
        daily_percentage = (self.current_day_cost / self.daily_limit) * 100 if self.daily_limit > 0 else 0
        monthly_percentage = (self.current_month_cost / self.monthly_limit) * 100 if self.monthly_limit > 0 else 0
        total_percentage = (self.total_cost / self.total_limit) * 100 if self.total_limit > 0 else 0
    ...
    def get_cost_forecast(self, days_ahead: int = 30) -> Dict[str, Any]:
        ...
        forecast = {
            ...
            "days_until_daily_limit": (self.daily_limit / daily_average) if daily_average > 0 else float('inf'),
            "days_until_monthly_limit": ((self.monthly_limit - self.current_month_cost) / daily_average) if daily_average > 0 else float('inf'),
            "days_until_total_limit": ((self.total_limit - self.total_cost) / daily_average) if daily_average > 0 else float('inf')
        }
