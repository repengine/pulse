...
class HybridRuleAdapter:
    ...
    @classmethod
    def get_instance(cls, config: Optional[Dict[str, Any]] = None) -> 'HybridRuleAdapter':
        """
    ...
    def register_rule_class(self, rule_type: str, rule_class: Type[Any]) -> None:
        ...
        self.logger.debug(f"Registered rule class {rule_class.__name__} for type {rule_type}")
    
    def register_condition_class(self, condition_type: str, condition_class: Type[Any]) -> None:
        """
        ...
        if not is_dataclass(condition_class):
            raise ValueError(f"Condition class {condition_class.__name__} must be a dataclass")
        
        self.condition_classes[condition_type] = condition_class
        self.logger.debug(f"Registered condition class {condition_class.__name__} for type {condition_type}")
    ...
    def register_action_class(self, action_type: str, action_class: Type[Any]) -> None:
        ...
        if not is_dataclass(action_class):
            raise ValueError(f"Action class {action_class.__name__} must be a dataclass")
        
        self.action_classes[action_type] = action_class
        self.logger.debug(f"Registered action class {action_class.__name__} for type {action_type}")
    
    def to_object(self, rule_dict: Dict[str, Any], rule_class: Optional[Type[T]] = None) -> T:
        """
    ...
    def _dict_to_dataclass(self, data_dict: Dict[str, Any], target_class: Type[T]) -> T:
        ...
        for field_name, field_type in type_hints.items():
            if field_name in data_dict:
                ...
                if (inspect.isclass(field_type) and is_dataclass(field_type) and 
                    isinstance(field_value, dict)):
                    constructor_args[field_name] = self._dict_to_dataclass(field_value, field_type)
                
                # Handle lists of dataclasses
                elif (getattr(field_type, "__origin__", None) is list and 
                     inspect.isclass(getattr(field_type, "__args__", [None])[0]) and
                     is_dataclass(getattr(field_type, "__args__", [None])[0]) and
    ...
    def _track_conversion_cost(self, operation: str, data: Dict[str, Any], start_time: datetime) -> None:
        """
    ...
    def adapt_rule(self, rule: Union[Dict[str, Any], Any]) -> Union[Dict[str, Any], Any]:
        """
