...
class RecursiveRuleGenerator:
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        ...
        self.logger.info(f"RecursiveRuleGenerator initialized with {self.max_workers} worker processes")
    ...
    def _compute_context_hash(self, context: Dict[str, Any], rule_type: str) -> str:
        ...
        hash_input = json.dumps({"context": context, "rule_type": rule_type}, sort_keys=True)
    ...
    def generate_rules_batch(self,
        ...
        if len(contexts) == 1:
            return [self.generate_rule(contexts[0], rule_types[0], method, max_iterations, cost_limit)]
            
        # Set default values if not provided
        method = method or self.default_method
        max_iterations = max_iterations or self.max_iterations
        cost_limit = cost_limit or self.cost_config.daily_cost_threshold_usd / 5  # 20% of daily budget for batch
        ...
        self.logger.info(f"Starting batch rule generation (ID: {self.current_run_id}, Batch size: {len(contexts)})")
        
        # Check cost budget
        if not self.cost_controller.can_make_api_call(check_cost=True):
            self.logger.error("Batch rule generation canceled due to cost limits")
            self.generation_status = RuleGenerationStatus.CANCELED
            return [{"error": "Rule generation canceled due to cost limits"}] * len(contexts)
        ...
        estimated_sequential_time = 0.5 * len(rules) * self.metrics["average_iterations"]  # Rough estimate
        ...
        self.logger.info(f"Batch rule generation completed (ID: {self.current_run_id}, Rules: {len(rules)}, Cost: ${total_cost:.2f})")
        return rules
    
    def _generate_rule_task(self,
                           context: Dict[str, Any],
                           rule_type: str,
        ...
        try:
            ...
            feedback = None
            iteration = 0  # Initialize iteration counter to avoid "possibly unbound" error
            for iteration in range(max_iterations):
                # Check cost limit
                ...
                if current_rule is not None:
                    ...
                    if iteration > 0 and quality - best_quality < self.improvement_threshold:
                        break
