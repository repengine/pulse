...
class AsyncMetricsCollector:
    ...
    @classmethod
    def get_instance(cls, config: Optional[Dict[str, Any]] = None) -> 'AsyncMetricsCollector':
            ...
            if wait_for_completion and self.worker_thread is not None:
                ...
                while not self.metrics_queue.empty():
                    if time.time() - wait_start > timeout:
                        self.logger.warning(f"Timeout waiting for metrics queue to empty. {self.metrics_queue.qsize()} items remaining.")
                        break
                    time.sleep(0.1)
                
                self.worker_thread.join(timeout=timeout)
                if self.worker_thread.is_alive():
                    self.logger.warning("AsyncMetricsCollector worker thread did not terminate within timeout")
                else:
                    self.logger.info("AsyncMetricsCollector worker thread stopped successfully")
            ...
            while not success and retries <= self.max_retries:
                try:
                    ...
                    if retries <= self.max_retries:
                        self.logger.warning(f"Failed to process metric {metric.get('id')}, retrying ({retries}/{self.max_retries}): {e}")
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        if stats["metrics_submitted"] > 0:
            stats["success_rate"] = (stats["metrics_processed"] / stats["metrics_submitted"]) * 100.0
        else:
            stats["success_rate"] = 100.0
            
        if stats["batches_processed"] > 0:
            stats["avg_batch_time"] = stats["processing_time"] / stats["batches_processed"]
        else:
...
def get_async_metrics_collector(config: Optional[Dict[str, Any]] = None) -> AsyncMetricsCollector:
    """
