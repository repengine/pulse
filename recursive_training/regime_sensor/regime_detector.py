...
class RegimeChangeEvent:
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "supporting_evidence": [event.event_id for event in self.supporting_evidence],
...
class RegimeDetector:
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        ...
        logger.info(f"RegimeDetector initialized with regime: {self.current_regime.value}")
    
    def _initialize_detection_methods(self):
        """Register default detection methods."""
        # Register basic detection methods
        self.register_detection_method("volatility_spike", self._detect_volatility_spike)
        self.register_detection_method("trend_reversal", self._detect_trend_reversal)
        self.register_detection_method("news_sentiment", self._detect_news_sentiment_shift)
        self.register_detection_method("economic_indicators", self._detect_economic_indicator_shift)
    ...
    def _check_for_regime_change(self):
        ...
        for method_name, detection_func in self.detection_methods.items():
            try:
                ...
                if result:
                    ...
                    logger.debug(f"Method {method_name} detected {regime.value} with confidence {confidence:.2f}")
        ...
        if highest_confidence >= self.min_confidence and new_regime != self.current_regime:
    ...
    def _handle_regime_change(
        ...
        logger.info(f"Regime change detected: {self.current_regime.value} -> {new_regime.value} with confidence {confidence:.2f}")
    ...
    def _detect_volatility_spike(self) -> Optional[Tuple[RegimeType, float, List[Event], Dict[str, Any]]]:
        """
        ...
        if volatility > 30:  # Simple threshold
            confidence = min(1.0, volatility / 50.0)  # Higher volatility = higher confidence
            
            # Collect supporting events
            supporting_events = [
                event for event in self.event_buffer[-20:]  # Look at recent events
                if event.event_type in [EventType.MARKET_MOVEMENT, EventType.NEWS, EventType.ECONOMIC_INDICATOR]
            ]
            ...
            return RegimeType.VOLATILITY_SHOCK, confidence, supporting_events, indicators
        
        return None
    
    def _detect_trend_reversal(self) -> Optional[Tuple[RegimeType, float, List[Event], Dict[str, Any]]]:
        """
        ...
        if sma_50 > sma_200 and self.current_regime != RegimeType.BULL_MARKET:
            ...
            supporting_events = [
                event for event in self.event_buffer[-30:]
                if event.event_type in [EventType.MARKET_MOVEMENT, EventType.CORPORATE_ANNOUNCEMENT]
            ]
    ...
    def _detect_news_sentiment_shift(self) -> Optional[Tuple[RegimeType, float, List[Event], Dict[str, Any]]]:
        """
        ...
        news_events = [
            event for event in self.event_buffer[-50:]  # Look at recent events
        ...
        # Crisis keywords
        crisis_keywords = ["crisis", "crash", "collapse", "plummet", "disaster", "fear", "panic"]
        recession_keywords = ["recession", "contraction", "downturn", "slowdown", "layoffs", "unemployment"]
        expansion_keywords = ["growth", "expansion", "bull", "recovery", "positive", "upswing", "boom"]
        inflation_keywords = ["inflation", "price increases", "rising prices", "cpi", "cost of living"]
        monetary_keywords = ["fed", "central bank", "interest rate", "rate hike", "rate cut", "monetary policy"]
        
        # Count keyword occurrences
        crisis_count = sum(1 for event in news_events if any(keyword in event.content.lower() for keyword in crisis_keywords))
        recession_count = sum(1 for event in news_events if any(keyword in event.content.lower() for keyword in recession_keywords))
        expansion_count = sum(1 for event in news_events if any(keyword in event.content.lower() for keyword in expansion_keywords))
        inflation_count = sum(1 for event in news_events if any(keyword in event.content.lower() for keyword in inflation_keywords))
        monetary_count = sum(1 for event in news_events if any(keyword in event.content.lower() for keyword in monetary_keywords))
        ...
        if monetary_count > 5:
            tightening_count = sum(1 for event in news_events if "hike" in event.content.lower() or "raise" in event.content.lower())
            easing_count = sum(1 for event in news_events if "cut" in event.content.lower() or "lower" in event.content.lower())
        ...
        if counts:
            ...
            confidence = min(1.0, count / len(news_events) * 2)  # Scale up to make it more sensitive
            ...
            if count >= 5 and confidence >= 0.3 and regime_type != self.current_regime:
                ...
                if regime_type == RegimeType.GEOPOLITICAL_CRISIS:
                    supporting_events = [event for event in news_events if any(keyword in event.content.lower() for keyword in crisis_keywords)]
                elif regime_type == RegimeType.RECESSION:
                    supporting_events = [event for event in news_events if any(keyword in event.content.lower() for keyword in recession_keywords)]
                elif regime_type == RegimeType.EXPANSION:
                    supporting_events = [event for event in news_events if any(keyword in event.content.lower() for keyword in expansion_keywords)]
                elif regime_type == RegimeType.INFLATION:
                    supporting_events = [event for event in news_events if any(keyword in event.content.lower() for keyword in inflation_keywords)]
                elif regime_type in [RegimeType.MONETARY_TIGHTENING, RegimeType.MONETARY_EASING]:
                    supporting_events = [event for event in news_events if any(keyword in event.content.lower() for keyword in monetary_keywords)]
    ...
    def _detect_economic_indicator_shift(self) -> Optional[Tuple[RegimeType, float, List[Event], Dict[str, Any]]]:
        """
        ...
        required_indicators = ['gdp_growth', 'unemployment', 'inflation', 'interest_rates']
        ...
        if gdp_growth < 0 and unemployment > 5.5:
            ...
            supporting_events = [
                event for event in self.event_buffer[-40:]
