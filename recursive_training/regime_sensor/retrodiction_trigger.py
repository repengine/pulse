...
from recursive_training.regime_sensor.regime_detector import RegimeType, RegimeChangeEvent
...
class RetrodictionSnapshot:
    ...
    def from_dict(cls, data: Dict[str, Any]) -> 'RetrodictionSnapshot':
        ...
        if "time_range_start" in data and "time_range_end" in data:
            try:
                start = datetime.fromisoformat(data["time_range_start"].replace('Z', '+00:00'))
                end = datetime.fromisoformat(data["time_range_end"].replace('Z', '+00:00'))
                time_range = (start, end)
            except ValueError:
                logger.warning(f"Invalid time range format, ignoring")
        ...
        if "processing_start" in data and data["processing_start"]:
            try:
                snapshot.processing_start = datetime.fromisoformat(data["processing_start"].replace('Z', '+00:00'))
            except ValueError:
                logger.warning(f"Invalid processing_start format, ignoring")
        
        if "processing_end" in data and data["processing_end"]:
            try:
                snapshot.processing_end = datetime.fromisoformat(data["processing_end"].replace('Z', '+00:00'))
            except ValueError:
                logger.warning(f"Invalid processing_end format, ignoring")
...
class RetrodictionTrigger:
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        ...
        self.storage_path = self.config.get('storage_path', 'data/retrodiction_snapshots')
    ...
    def handle_regime_change(self, regime_change: RegimeChangeEvent):
        ...
        if not self.auto_trigger_enabled:
            logger.info(f"Auto-triggering disabled, ignoring regime change: {regime_change}")
            return
        ...
        if regime_change.new_regime in [RegimeType.VOLATILITY_SHOCK, RegimeType.GEOPOLITICAL_CRISIS]:
            priority = TriggerPriority.CRITICAL
        
        # Create time range based on default window
        end_time = datetime.now()
        start_time = end_time.replace(
            year=end_time.year - 1 if self.default_time_window >= 365 else end_time.year,
    ...
    def _check_scheduled_snapshot(self):
        ...
        if self.last_scheduled is None or (now - self.last_scheduled).total_seconds() >= self.schedule_interval * 3600:
    ...
    def start(self):
        ...
        with self.lock:
            if not self.running:
                ...
                self.processing_thread = threading.Thread(target=self._snapshot_processor_loop)
