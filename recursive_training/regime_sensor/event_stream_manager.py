...
class EventStreamManager:
    ...
    def ingest_event(self, event: Event):
        ...
        should_process = all(filter_func(event) for filter_func in self.filters.values())
    ...
    def _store_event(self, event: Event):
        ...
        try:
            ...
            directory = os.path.join(self.storage_path, date_str, event_type_str, source_str)
    ...
    def start(self):
        ...
        with self.lock:
            if not self.running:
                ...
                self.processing_thread = threading.Thread(target=self._event_processor_loop)
    ...
    def create_mock_news_event(self, headline: str, source: str = "mock_source") -> Event:
        """
    ...
    def _extract_entities_from_headline(self, headline: str) -> List[str]:
        ...
        potential_entities = re.findall(r'\b[A-Z][A-Za-z]*(?:\s+[A-Z][A-Za-z]*)*\b', headline)
        
        # Filter out common words that might be capitalized
        common_words = {"THE", "A", "AN", "AND", "OR", "BUT", "FOR", "WITH", "IN", "ON", "AT"}
        filtered_entities = [entity for entity in potential_entities if entity.upper() not in common_words]
...
if __name__ == "__main__":
    ...
    events = [
        ...
        manager.create_mock_news_event("Federal Reserve raises interest rates by 0.25%")
    ]
