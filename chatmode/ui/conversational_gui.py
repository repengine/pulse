...
class ConversationalGUI(tk.Tk):
    def __init__(self):
        ...
        self.display_system_message("Type a query, choose a command from the dropdown, or click the buttons below for common actions.")
    ...
    def _setup_title_area(self):
        ...
        title_label = tk.Label(title_frame, text="Pulse AI Conversational Interface", 
                              font=title_font, bg=self.bg_color, fg=self.accent_color)
        title_label.pack(side=tk.LEFT, pady=5)
        
        # Add mode indicator
        self.mode_var = tk.StringVar(value="Conversation Mode")
        mode_label = tk.Label(title_frame, textvariable=self.mode_var, 
                             bg=self.accent_color, fg="white", 
                             padx=10, pady=2, relief=tk.RAISED)
        mode_label.pack(side=tk.RIGHT, pady=5)

    def _setup_conversation_area(self):
        """Sets up the main conversation history area"""
        conversation_frame = ttk.LabelFrame(self.main_frame, text="Conversation History")
    ...
    def _setup_structured_output_area(self):
        ...
        # Status display area
        status_frame = ttk.LabelFrame(self.recursive_learning_frame, text="Learning Cycles Status")
        ...
        self.start_learning_btn = ttk.Button(
            ...
            command=self._start_learning_cycle
        )
        ...
        self.stop_learning_btn = ttk.Button(
            control_frame,
            text="Stop Cycle",
            command=self._stop_learning_cycle
        )
        ...
        self.refresh_status_btn = ttk.Button(
            control_frame,
            text="Refresh Status",
            command=self._refresh_learning_status
        )
    ...
    def _update_parameter_frame(self, event=None):
        ...
        if command == "Run Simulation":
            tk.Label(self.param_frame, text="Simulation Parameters:", bg=self.bg_color).pack(side=tk.LEFT)
            ...
            tk.Label(self.param_frame, text="Symbol:", bg=self.bg_color).pack(side=tk.LEFT)
            self.data_symbol = tk.Entry(self.param_frame, width=10)
            self.data_symbol.pack(side=tk.LEFT, padx=5)
            
            tk.Label(self.param_frame, text="Data Type:", bg=self.bg_color).pack(side=tk.LEFT)
            self.data_type = ttk.Combobox(self.param_frame, values=["historical_prices", "news", "fundamentals"], width=15)
            self.data_type.pack(side=tk.LEFT, padx=5)
            
            tk.Label(self.param_frame, text="Date Range:", bg=self.bg_color).pack(side=tk.LEFT)
            ...
            tk.Label(self.param_frame, text="Item:", bg=self.bg_color).pack(side=tk.LEFT)
            ...
            tk.Label(self.param_frame, text="Query:", bg=self.bg_color).pack(side=tk.LEFT)
    ...
    def _setup_command_buttons(self):
        ...
        tk.Button(button_frame, text="Show Latest Forecast", command=lambda: self._quick_command("get_forecast"), **button_style).pack(side=tk.LEFT, padx=2)
        tk.Button(button_frame, text="Run Simulation", command=lambda: self._quick_command("run_simulation"), **button_style).pack(side=tk.LEFT, padx=2)
        tk.Button(button_frame, text="Get Trust Scores", command=lambda: self._quick_command("get_trust_score"), **button_style).pack(side=tk.LEFT, padx=2)
        tk.Button(button_frame, text="Clear History", command=self._clear_history, **button_style).pack(side=tk.RIGHT, padx=2)
    ...
    def _show_settings_dialog(self):
        ...
        model_type_frame = tk.LabelFrame(settings_dialog, text="Model Type", padx=10, pady=10, bg=self.bg_color)
        model_type_frame.pack(fill=tk.X, padx=10, pady=10)
        
        # Radio buttons for model type
        openai_radio = tk.Radiobutton(
            model_type_frame, text="OpenAI API", variable=self.llm_model_type,
            value="openai", bg=self.bg_color
        )
        openai_radio.pack(anchor=tk.W)
        
        mock_radio = tk.Radiobutton(
            model_type_frame, text="Mock (No API required)", variable=self.llm_model_type,
            value="mock", bg=self.bg_color
        )
        mock_radio.pack(anchor=tk.W)
        
        # OpenAI Settings Frame (only visible when OpenAI is selected)
        openai_frame = tk.LabelFrame(settings_dialog, text="OpenAI Settings", padx=10, pady=10, bg=self.bg_color)
        ...
        tk.Label(api_key_frame, text="API Key:", bg=self.bg_color).pack(side=tk.LEFT, padx=(0,5))
        
        # Create variable for API key
        self.api_key_var = tk.StringVar(value=os.environ.get("OPENAI_API_KEY", ""))
        api_key_entry = tk.Entry(api_key_frame, textvariable=self.api_key_var, width=40, show="*")
        ...
        show_key_check = tk.Checkbutton(
            api_key_frame, text="Show", variable=self.show_key,
            command=toggle_api_key_visibility, bg=self.bg_color
        )
        ...
        tk.Label(model_frame, text="Model:", bg=self.bg_color).pack(side=tk.LEFT, padx=(0,5))
        
        model_choices = ["gpt-4-turbo", "gpt-4o", "gpt-3.5-turbo"]
        model_dropdown = ttk.Combobox(
            model_frame, textvariable=self.llm_model_name, values=model_choices, width=20
        )
        model_dropdown.pack(side=tk.LEFT, padx=5)
        
        # Cost Estimation Frame
        cost_frame = tk.LabelFrame(openai_frame, text="Estimated Costs", padx=10, pady=10, bg=self.bg_color)
        cost_frame.pack(fill=tk.X, pady=10)
        
        tk.Label(cost_frame, text="GPT-4 Turbo: $0.01/1K input, $0.03/1K output", bg=self.bg_color).pack(anchor=tk.W)
        tk.Label(cost_frame, text="GPT-4o: $0.01/1K input, $0.03/1K output", bg=self.bg_color).pack(anchor=tk.W)
        tk.Label(cost_frame, text="GPT-3.5 Turbo: $0.0005/1K input, $0.0015/1K output", bg=self.bg_color).pack(anchor=tk.W)
        ...
        save_button = tk.Button(
            button_frame, text="Save Settings", command=lambda: self._save_settings(settings_dialog),
            bg=self.accent_color, fg="white", padx=10
        )
        ...
        x = self.winfo_rootx() + (self.winfo_width() - settings_dialog.winfo_width()) // 2
        y = self.winfo_rooty() + (self.winfo_height() - settings_dialog.winfo_height()) // 2
    ...
    def _save_settings(self, dialog):
        ...
        try:
            ...
            new_model = LLMModel(model_name=model_name, model_type=model_type, api_key=api_key)
            
            # Replace the model in the conversational core
            self.conversational_core.llm_model = new_model
            
            # Show success message
            self.display_system_message(f"LLM settings updated: {model_type} model {model_name}")
    ...
    def _clear_history(self):
        ...
        self.history_display.config(state='disabled')
        self.display_message("Pulse AI: Conversation history cleared. How can I help you?\n")
    ...
    def _get_parameters_from_ui(self):
        ...
        if command == "Run Simulation" and hasattr(self, 'simulation_params'):
            ...
            parameters = {
                ...
                "date_range": self.date_range.get().split(',') if self.date_range.get() else None
            }
    ...
    def send_message(self):
        ...
        threading.Thread(
            target=self._process_query_in_thread, 
            args=(user_text, intent, parameters)
        ).start()

    def _process_query_in_thread(self, user_text, intent="general_query", parameters=None):
        """
        ...
        try:
            ...
            self.after(0, lambda: self.display_system_message(f"Processing {intent} request..."))
            ...
            if intent == "run_simulation":
                ...
                self.after(0, lambda: self.display_structured_output('simulation',
                          {"response": response, "parameters": parameters}))
                          
            elif intent == "get_forecast":
                self.after(0, lambda: self.display_structured_output('forecast',
                          {"response": response, "parameters": parameters}))
                          
            elif intent == "query_memory":
                self.after(0, lambda: self.display_structured_output('memory',
                          {"response": response, "parameters": parameters}))
                          
            elif intent == "query_symbolic_system":
                self.after(0, lambda: self.display_structured_output('symbolic',
                          {"response": response, "parameters": parameters}))
            ...
            if retrieved_snippets:
                ...
                for s in retrieved_snippets:
                    if isinstance(s, dict):
                        ...
                        snippet_lines.append(f"--- Snippet from {file_path} ---\n{text}")
                    else:
                        # Old format: simple string
                        snippet_lines.append(f"--- Snippet ---\n{str(s)[:300]}...")
                
                snippet_text = "\n\n".join(snippet_lines)
                self.after(0, lambda: self.display_message(f"Retrieved Snippets:\n{snippet_text}\n"))

            # Update status to show success
            self.after(0, lambda: self.status_var.set("Ready"))
            self.after(0, lambda: self.status_label.config(fg="green"))
            self.after(0, lambda: self.display_system_message("Query processed successfully."))
            
        except Exception as e:
            # Display error in conversation and status area
            self.after(0, lambda: self.display_message(f"Error processing query: {e}\n"))
            self.after(0, lambda: self.status_var.set("Error"))
            self.after(0, lambda: self.status_label.config(fg="red"))
            self.after(0, lambda: self.display_system_message(f"Error: {str(e)}"))
    ...
    def _start_learning_cycle(self):
        ...
        # Process the command
        self._process_recursive_learning_command("start_recursive_learning", {
            "variables": variables,
    ...
    def _stop_learning_cycle(self):
        ...
        # Construct the command
        command_text = f"stop recursive learning cycle {cycle_id if cycle_id else 'all'}"
        
        # Display as user message and process it
        self.display_message(f"User: {command_text}\n")
        
        # Process the command
        self._process_recursive_learning_command("stop_recursive_learning", {
    ...
    def _process_recursive_learning_command(self, intent, parameters):
        ...
        threading.Thread(
            target=self._execute_recursive_learning_command,
        ...
        try:
            ...
            if tool:
                ...
                if result.get("status") == "success":
                    ...
                    self.after(0, lambda: self.display_message(f"Pulse AI: Successfully executed {intent}.\n"))
                    
                    # Update status display
                    self.after(0, lambda: self._update_learning_status_display(data))
                    
                    # Update the output area
                    self.after(0, lambda: self.display_structured_output('recursive_learning', data))
                    
                    # Select the recursive learning tab
                    self.after(0, lambda: self.output_notebook.select(4))  # Index 4 is the recursive learning tab
                else:
                    # Display error
                    error = result.get("error", "Unknown error")
                    self.after(0, lambda: self.display_message(f"Error executing {intent}: {error}\n"))
            else:
                self.after(0, lambda: self.display_message(f"Error: No tool found for intent '{intent}'\n"))
        
        except Exception as e:
            self.after(0, lambda: self.display_message(f"Error processing recursive learning command: {str(e)}\n"))
    ...
    def _update_learning_status_display(self, data):
        ...
        if data.get("status") == "started":
            # New cycle started
            self.learning_status.insert(tk.END, f"Started new learning cycle:\n")
            self.learning_status.insert(tk.END, f"  ID: {data.get('cycle_id')}\n")
            self.learning_status.insert(tk.END, f"  Variables: {', '.join(data.get('variables', []))}\n")
            self.learning_status.insert(tk.END, f"  Period: {data.get('time_period', {}).get('start')} to {data.get('time_period', {}).get('end')}\n")
            self.learning_status.insert(tk.END, f"  Batches: {data.get('batches', {}).get('total', 0)}\n")
            ...
            if not cycles:
                self.learning_status.insert(tk.END, "No active learning cycles found.\n")
            else:
                self.learning_status.insert(tk.END, f"Active learning cycles: {len(cycles)}\n\n")
                for cycle in cycles:
                    self.learning_status.insert(tk.END, f"Cycle ID: {cycle.get('cycle_id')}\n")
                    self.learning_status.insert(tk.END, f"  Status: {cycle.get('status', 'unknown')}\n")
                    self.learning_status.insert(tk.END, f"  Progress: {cycle.get('progress', {}).get('percentage', '0%')}\n")
                    self.learning_status.insert(tk.END, f"  Start time: {cycle.get('start_time')}\n")
                    self.learning_status.insert(tk.END, "\n")
        
        elif "cycle_id" in data:
            # Single cycle status
            self.learning_status.insert(tk.END, f"Cycle ID: {data.get('cycle_id')}\n")
            self.learning_status.insert(tk.END, f"Status: {data.get('status', 'unknown')}\n")
            
            progress = data.get('progress', {})
            if progress:
                self.learning_status.insert(tk.END, f"Progress: {progress.get('percentage', '0%')} ")
                self.learning_status.insert(tk.END, f"({progress.get('completed_batches', 0)}/{progress.get('total_batches', 0)} batches)\n")
            
            performance = data.get('performance', {})
            if performance:
                self.learning_status.insert(tk.END, f"Performance metrics:\n")
