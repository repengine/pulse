...
from iris.iris_plugins_finance import finance_plugins
...
def run_simulation(parameters=None, **kwargs):
    ...
    try:
        ...
        ws = WorldState()
        ws.sim_id = all_params.get("sim_id", f"chat_sim_{datetime.now().strftime('%Y%m%d_%H%M%S')}")
        ...
        return {
            "status": "failed",
            "error": str(e),
            "simulation_id": all_params.get("sim_id", f"chat_sim_{datetime.now().strftime('%Y%m%d_%H%M%S')}"),
            "parameters": all_params
        }
...
def get_data(symbol=None, data_type=None, date_range=None, **kwargs):
    ...
    logger.info(f"Retrieving data for symbol: {symbol}, type: {data_type}, range: {date_range}")
    ...
    try:
        ...
        if data_type in ['historical_prices', 'stock_price', 'market_data']:
            ...
            try:
                ...
                variable_info = next(
                    (var for var in catalog["variables"] if var["variable_name"] == symbol),
                    None
                )
                ...
                if variable_info:
                    ...
                    if result and "data" in result:
                        ...
                        values = [
                            item for item in processed_data.get("values", [])
                            if start_date_str <= item.get("date", "").split("T")[0] <= end_date_str
                        ]
                        ...
                        return {
                            ...
                            "data": values,
                            "source": processed_data.get("source", "historical_data_retriever"),
                            "retrieved_at": datetime.now().isoformat()
                        }
            except Exception as e:
                logger.warning(f"Historical data retrieval failed: {e}. Falling back to finance plugins.")
            ...
            try:
                ...
                if finance_data:
                    ...
                    symbol_data = [item for item in finance_data if item.get("name") == symbol]
                    if symbol_data:
                        return {
                            "symbol": symbol,
                            "data_type": data_type,
        ...
        for i in range(sample_days):
            ...
            if data_type in ['historical_prices', 'stock_price', 'market_data']:
                ...
                point = {
                    "date": date_str,
                    "value": f"Data for {symbol} ({data_type})"
                }
...
def get_forecast(symbol=None, horizon=None, **kwargs):
    ...
    try:
        ...
        if horizon == '1d':
            ...
            interval = "hourly"
            delta_func = lambda i: timedelta(hours=i)
        elif horizon == '1w':
            num_points = 7  # Daily for 1 week
            interval = "daily"
            delta_func = lambda i: timedelta(days=i)
        elif horizon == '1m':
...
def get_trust_score(item=None, context=None, **kwargs):
    ...
    try:
        ...
        if trust_score < 0.6:
            result["recommendations"].append("Consider additional data sources for higher confidence")
        elif trust_score < 0.8:
            result["recommendations"].append("Trust score is moderate - verify with alternative sources")
        else:
            result["recommendations"].append("Trust score is high enough for decision-making purposes")
...
def query_symbolic_system(query=None, **kwargs):
    ...
    try:
        ...
        # For rule queries
            ...
            condition = condition_match.group(1) if condition_match else "market_regime == 'high_volatility'"
            
            action_match = re.search(r"action[:\s]+([^;]+)", query_lower)
            action = action_match.group(1) if action_match else "adjust_overlay('hope', 0.3)"
            ...
            rule_result = {
                ...
                "affected_overlays": ["hope", "despair"] if "hope" in action else ["rage", "fatigue"]
            }
...
def explain_forecast(symbol=None, forecast_id=None, **kwargs):
    ...
    if not symbol and not forecast_id:
        return {"error": "Either symbol or forecast_id parameter is required", "status": "failed"}
    ...
    try:
        ...
        base_value = forecast_data.get("metadata", {}).get("base_value", 100.0) if forecast_data else 100.0
        forecast_points = forecast_data.get("forecast_points", []) if forecast_data else []
        confidence = forecast_data.get("metadata", {}).get("overall_confidence", 0.8) if forecast_data else 0.8
        ...
        trend_direction = "upward" if trend > 1 else "downward" if trend < -1 else "stable"
        trend_strength = "strong" if abs(trend) > 5 else "moderate" if abs(trend) > 2 else "weak"
        ...
        alternative_scenarios = [
            {
                ...
                "description": f"Downside scenario due to sudden increase in market volatility",
                "expected_impact": round(-abs(trend) * 2, 1)
            },
            {
                "probability": round(0.10 * confidence, 2),
                "description": f"Significant upside scenario if positive catalysts materialize",
                "expected_impact": round(abs(trend) * 3, 1)
            }
        ]
        
        # Generate the explanation
        explanation = f"This forecast for {target_symbol} projects a {trend_strength} {trend_direction} trend " \
...
def start_recursive_learning(variables=None, start_time=None, end_time=None, batch_size_days=30, **kwargs):
    """
    ...
    logger.info(f"Starting recursive learning: variables={variables}, period={start_time} to {end_time}")
    ...
    try:
        ...
        training_thread = threading.Thread(
            ...
            kwargs={"progress_callback": lambda p: logger.info(f"Training progress: {p['completed_percentage']}")}
        )
...
def get_recursive_learning_status(cycle_id=None, **kwargs):
    ...
    """
    logger.info(f"Getting recursive learning status: {cycle_id if cycle_id else 'all cycles'}")
        ...
        for cid in process_registry.list_processes():
            ...
            if coordinator:
                ...
                all_cycles.append({
                    ...
                    "start_time": coordinator.training_start_time.isoformat() if coordinator.training_start_time else None
                })
...
def configure_recursive_learning(variables=None, batch_size_days=None, max_workers=None, **kwargs):
    """
    ...
    """
    logger.info(f"Configuring recursive learning: variables={variables}, batch_size={batch_size_days}, workers={max_workers}")
...
def get_recursive_learning_metrics(cycle_id=None, metric_type=None, **kwargs):
    ...
    """
    logger.info(f"Getting recursive learning metrics: cycle={cycle_id}, type={metric_type}")
                ...
                if metric_type:
                    if metric_type == 'performance':
                        ...
                        return {
                            "cycle_id": cycle_id,
                            "metrics_type": "trust",
                            "data": summary.get("variables", {}).get("trust_scores", {}),
                            "timestamp": datetime.now().isoformat()
                        }
                    else:
                        return {
