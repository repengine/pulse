...
class Intent:
    ...
    def __init__(self, name: str, confidence: float = 0.0, entities: Optional[Dict[str, Any]] = None):
        self.name = name
...
class PatternMatcher:
    ...
    def __init__(self):
        ...
        self.patterns = {
            'run_simulation': [
                (r'(?i)run\s+(?:a\s+)?simulation(?:\s+with\s+)?(?P<parameters>.*)?', 0.9),
                (r'(?i)simulate(?:\s+with\s+)?(?P<parameters>.*)?', 0.8),
                (r'(?i)create\s+(?:a\s+)?simulation(?:\s+using\s+)?(?P<parameters>.*)?', 0.7)
            ],
            'get_data': [
                (r'(?i)(?:get|fetch|retrieve|show)\s+data(?:\s+for\s+)(?P<symbol>[A-Z]+)(?:.*?(?:type|kind)[\s:]+(?P<data_type>[a-zA-Z_]+))?(?:.*?(?:from|between|since|range)[\s:]+(?P<date_range>.+?))?', 0.9),
                (r'(?i)(?:data\s+for|get)\s+(?P<symbol>[A-Z]+)(?:.*?(?:type|kind)[\s:]+(?P<data_type>[a-zA-Z_]+))?(?:.*?(?:from|between|since|range)[\s:]+(?P<date_range>.+?))?', 0.8)
            ],
            'get_forecast': [
                (r'(?i)(?:get|show|display|create)\s+(?:a\s+)?forecast(?:\s+for\s+)?(?P<symbol>[A-Z]+)', 0.9),
                (r'(?i)forecast(?:\s+for\s+)(?P<symbol>[A-Z]+)', 0.8),
                (r'(?i)predict(?:\s+for\s+)(?P<symbol>[A-Z]+)', 0.7)
            ],
            'get_trust_score': [
                (r'(?i)(?:get|show|display|what\s+is\s+the)\s+trust\s+score(?:\s+for\s+)?(?P<item>.+)', 0.9),
                (r'(?i)how\s+trustworthy\s+is(?:\s+the\s+)?(?P<item>.+)', 0.8),
                (r'(?i)trust(?:\s+level)?(?:\s+for\s+)?(?P<item>.+)', 0.7)
            ],
            'query_memory': [
                (r'(?i)(?:query|search|look\s+up)\s+(?:the\s+)?memory(?:\s+for\s+)?(?P<query>.+)', 0.9),
                (r'(?i)(?:find|retrieve)\s+from\s+memory(?:\s+about\s+)?(?P<query>.+)', 0.8),
                (r'(?i)(?:what\s+do\s+you\s+remember\s+about|recall)\s+(?P<query>.+)', 0.7)
            ],
            'query_symbolic_system': [
                (r'(?i)(?:query|ask)\s+(?:the\s+)?symbolic\s+system(?:\s+about\s+)?(?P<query>.+)', 0.9),
                (r'(?i)symbolic\s+(?:query|question)(?:\s+about\s+)?(?P<query>.+)', 0.8),
                (r'(?i)(?:what\s+does\s+the\s+)?symbolic\s+system\s+(?:say|think)\s+about\s+(?P<query>.+)', 0.7)
            ],
            'explain_forecast': [
                (r'(?i)explain(?:\s+this)?\s+forecast(?:\s+for\s+)?(?P<symbol>[A-Z]+)?', 0.9),
                (r'(?i)(?:why|how)\s+(?:did\s+you|is\s+this)\s+forecast(?:\s+for\s+)?(?P<symbol>[A-Z]+)?', 0.8),
                (r'(?i)(?:details|more\s+information)\s+(?:about|on)\s+(?:this\s+)?forecast(?:\s+for\s+)?(?P<symbol>[A-Z]+)?', 0.7)
            ],
            'start_recursive_learning': [
                (r'(?i)(?:start|begin|launch|initiate)\s+(?:a\s+)?recursive\s+learning(?:\s+cycle)?(?:\s+with\s+)?(?P<parameters>.*)?', 0.9),
                (r'(?i)(?:run|execute)\s+(?:a\s+)?recursive\s+(?:learning|training)(?:\s+cycle)?(?:\s+with\s+)?(?P<parameters>.*)?', 0.8),
                (r'(?i)(?:start|begin|launch)\s+(?:a\s+)?(?:new\s+)?training\s+cycle(?:\s+with\s+)?(?P<parameters>.*)?', 0.7)
            ],
            'stop_recursive_learning': [
                (r'(?i)(?:stop|halt|end|terminate)\s+(?:the\s+)?recursive\s+learning(?:\s+cycle)?(?:\s+with\s+id\s+)?(?P<cycle_id>[a-zA-Z0-9_-]+)?', 0.9),
                (r'(?i)(?:cancel|abort)\s+(?:the\s+)?recursive\s+(?:learning|training)(?:\s+cycle)?(?:\s+with\s+id\s+)?(?P<cycle_id>[a-zA-Z0-9_-]+)?', 0.8),
                (r'(?i)(?:stop|halt|end|terminate)\s+(?:the\s+)?training\s+cycle(?:\s+with\s+id\s+)?(?P<cycle_id>[a-zA-Z0-9_-]+)?', 0.7)
            ],
            'get_recursive_learning_status': [
                (r'(?i)(?:what\s+is\s+the\s+)?status\s+of\s+(?:the\s+)?recursive\s+learning(?:\s+cycle)?(?:\s+with\s+id\s+)?(?P<cycle_id>[a-zA-Z0-9_-]+)?', 0.9),
                (r'(?i)(?:check|get|show)\s+(?:the\s+)?recursive\s+(?:learning|training)\s+status(?:\s+for\s+id\s+)?(?P<cycle_id>[a-zA-Z0-9_-]+)?', 0.8),
                (r'(?i)(?:how\s+is|progress\s+of)\s+(?:the\s+)?recursive\s+learning(?:\s+cycle)?(?:\s+with\s+id\s+)?(?P<cycle_id>[a-zA-Z0-9_-]+)?', 0.7)
            ],
            'configure_recursive_learning': [
                (r'(?i)(?:configure|setup|set)\s+(?:the\s+)?recursive\s+learning(?:\s+with\s+)?(?P<parameters>.*)?', 0.9),
                (r'(?i)(?:update|change|modify)\s+(?:the\s+)?recursive\s+learning\s+(?:configuration|settings)(?:\s+with\s+)?(?P<parameters>.*)?', 0.8),
                (r'(?i)(?:adjust|tune)\s+(?:the\s+)?recursive\s+learning\s+parameters(?:\s+with\s+)?(?P<parameters>.*)?', 0.7)
            ],
            'get_recursive_learning_metrics': [
                (r'(?i)(?:get|show|display)\s+(?:the\s+)?recursive\s+learning\s+(?:metrics|results)(?:\s+for\s+cycle\s+)?(?P<cycle_id>[a-zA-Z0-9_-]+)?(?:\s+of\s+type\s+)?(?P<metric_type>[a-zA-Z_]+)?', 0.9),
                (r'(?i)(?:metrics|results)\s+(?:of|for)\s+(?:the\s+)?recursive\s+learning(?:\s+cycle\s+)?(?P<cycle_id>[a-zA-Z0-9_-]+)?(?:\s+of\s+type\s+)?(?P<metric_type>[a-zA-Z_]+)?', 0.8),
                (r'(?i)(?:how\s+is\s+the\s+)?performance\s+of\s+(?:the\s+)?recursive\s+learning(?:\s+cycle\s+)?(?P<cycle_id>[a-zA-Z0-9_-]+)?', 0.7)
            ]
        }
    ...
    def extract_entities(self, text: str, entity_matches: Dict[str, Any]) -> Dict[str, Any]:
        """
    ...
    def match(self, text: str, conversation_context: Optional[Dict[str, Any]] = None) -> List[Intent]:
        """
            ...
            for pattern, base_confidence in patterns:
                ...
                if match:
                    ...
                    if conversation_context:
                        ...
                        if any(entity in conversation_context.get('mentioned_entities', {}) 
        ...
        if detected_intent:
            ...
            for entity_name, entity_value in detected_intent.entities.items():
                ...
                if entity_value not in self.metadata["mentioned_entities"][entity_name]:
                    self.metadata["mentioned_entities"][entity_name].append(entity_value)
...
class ConversationalCore:
    def __init__(self, model_name=None, model_type=None, api_key=None):
        ...
        if self.context_provider.vector_store is None:
            logger.warning("Vector store could not be loaded. RAG will not be available.")

        # 2. Initialize LLM and Domain Adapter
        try:
            self.llm_model = LLMModel(model_name=model_name, model_type=model_type, api_key=api_key)
...
"""
    ...
    def recognize_intent(self, query, conversation_context=None):
        ...
        print(f"Recognized intents: {[i.name for i in intents]} with confidences: {[i.confidence for i in intents]}")
    ...
    def execute_tool(self, intent_name, parameters):
        ...
        if not tool:
            return {"error": f"No tool found for intent: {intent_name}", "status": "failed"}
    ...
    def process_query(self, user_query):
        ...
        try:
            if self.context_provider:
                search_results = self.context_provider.get_relevant_context(user_query, k=5)
                # Format snippets for the prompt using the context provider's formatting
                codebase_context = self.context_provider.format_snippets_for_prompt(search_results)
                retrieved_snippets = search_results
                logger.info(f"Retrieved {len(retrieved_snippets)} relevant snippets for query.")
            else:
                codebase_context = "No relevant snippets found: context provider unavailable."
                logger.warning("Context provider unavailable. Unable to retrieve snippets.")
        except Exception as e:
    ...
    def _save_interaction_log(self, user_query: str, prompt: str, response: str) -> None:
        """
