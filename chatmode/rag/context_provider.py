...
class ContextProvider:
    ...
    """
    def __init__(self, vector_store: Optional[CodebaseVectorStore] = None,
                 max_snippets: int = 5, min_similarity: float = 0.9):
        """
        ...
        if not self.vector_store:
            try:
                ...
                if not self.vector_store:
                    logger.warning("Failed to load vector store automatically. Some features will be limited.")
    ...
    def get_relevant_context(self, query: str, k: Optional[int] = None) -> List[Dict[str, Any]]:
        """
        ...
        try:
            ...
            filtered_results = [r for r in results if r.get('score', float('inf')) <= self.min_similarity]
            
            # Log results
            logger.info(f"Retrieved {len(filtered_results)} relevant context snippets for query: '{query[:30]}...'")
    ...
    def format_snippets_for_prompt(self, snippets: List[Dict[str, Any]], 
        ...
        for i, snippet in enumerate(snippets):
            ...
            # Format the snippet
            formatted_snippet = f"SNIPPET {i+1}: {identifier} in {file_path}\n{text}\n"
