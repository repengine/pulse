...
def build_and_save_vector_store(directories=None, model_name="all-MiniLM-L6-v2", quantize=False):
    """
    ...
    try:
        ...
        logger.info(f"Loaded a total of {len(documents)} documents from specified directories.")
    ...
    if quantize and hasattr(vector_store, 'index'):
        logger.warning("Quantization is disabled in this version - using standard index")
    ...
    try:
        ...
        logger.info(f"Vector store build complete in {build_stats['elapsed_seconds']} seconds.")
...
def load_vector_store(model_name="all-MiniLM-L6-v2"):
    ...
    if not os.path.exists(VECTOR_STORE_PATH) or not os.path.exists(METADATA_PATH):
        logger.warning("Vector store files not found. Please build the vector store first.")
        return None
    ...
    try:
        ...
        if index.d != embedding_dimension:
            logger.error(f"Error loading index: Dimension mismatch. Expected {embedding_dimension}, got {index.d}")
            return None
        ...
        logger.info(f"Vector store loaded successfully. Total vectors: {loaded_vector_store.index.ntotal}")
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Build and test the Pulse codebase vector store.")
    parser.add_argument('--action', choices=['build', 'test', 'info'], default='build',
    ...
    parser.add_argument('--quantize', action='store_true',
                       help='Quantize vectors to reduce size')
    parser.add_argument('--query', default="How does the conversational interface work?",
    ...
    if args.action == 'build':
        build_stats = build_and_save_vector_store(model_name=args.model, quantize=args.quantize)
        if build_stats.get('status') == 'success':
            logger.info("Vector store build completed successfully!")
        else:
            logger.error(f"Vector store build failed: {build_stats.get('reason', 'Unknown error')}")
