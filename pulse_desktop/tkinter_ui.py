...
class PulseApp:
    def __init__(self, root):
        ...
        self.status_bar = ttk.Label(
            root, text="Ready", relief="sunken", anchor="w", padding=(5, 2)
        )
        ...
        self.learning_frame = ttk.Frame(
            self.notebook, padding="10"
        )  # Renamed from ai_training_review_frame
    ...
    def _init_dashboard_tab(self):
        ...
        # Header
        ttk.Label(
            self.dashboard_frame, text="Pulse System Dashboard", style="Header.TLabel"
        ).pack(pady=(10, 20))

        # System status display
        status_frame = ttk.LabelFrame(
            self.dashboard_frame, text="System Status", padding=10
        )
        ...
        check_button = ttk.Button(
            status_frame, text="Check Connections", command=self.check_connections
        )
        check_button.pack(pady=(10, 5))

        # Quick Actions section
        actions_frame = ttk.LabelFrame(
            self.dashboard_frame, text="Quick Actions", padding=10
        )
        ...
        # Row 1 buttons
        run_forecast_btn = ttk.Button(
            btn_frame, text="Run Forecast", command=lambda: self.notebook.select(1)
        )  # Switch to Forecasts tab
        run_forecast_btn.grid(row=0, column=0, padx=10, pady=5, sticky="ew")

        run_retrodiction_btn = ttk.Button(
            btn_frame, text="Run Retrodiction", command=lambda: self.notebook.select(2)
        )  # Switch to Retrodiction tab
        run_retrodiction_btn.grid(row=0, column=1, padx=10, pady=5, sticky="ew")

        # Row 2 buttons
        start_autopilot_btn = ttk.Button(
            btn_frame, text="Start Autopilot", command=self.start_autopilot_from_dash
        )
        start_autopilot_btn.grid(row=1, column=0, padx=10, pady=5, sticky="ew")

        view_analysis_btn = ttk.Button(
            btn_frame, text="Analysis Tools", command=lambda: self.notebook.select(5)
        ...
        activity_frame = ttk.LabelFrame(
            self.dashboard_frame, text="Recent Activity", padding=10
        )
    ...
    def check_connections(self):
        ...
        try:
            ...
            if forecast_status and forecast_status.get("status") == "ready":
                self.forecast_status.config(
                    text="Forecast Engine: Ready", foreground="green"
                )
                self.log_activity("Forecast engine is ready")
            else:
                self.forecast_status.config(
                    text="Forecast Engine: Not Ready", foreground="orange"
                )
                self.log_activity("Forecast engine is not ready")
        except Exception:
            self.forecast_status.config(
                text="Forecast Engine: Unknown", foreground="gray"
            )
        ...
        try:
            ...
            if autopilot_status and autopilot_status.get("status"):
                ...
                self.dashboard_autopilot_status.config(
                    text=f"Autopilot: {status_text}", foreground=color
                )
                self.log_activity(f"Autopilot status: {status_text}")
            else:
                self.dashboard_autopilot_status.config(
                    text="Autopilot: Unknown", foreground="gray"
                )
        except Exception:
            self.dashboard_autopilot_status.config(
                text="Autopilot: Error", foreground="red"
            )
    ...
    def start_autopilot_from_dash(self):
        ...
        self.dashboard_autopilot_status.config(
            text="Autopilot: Running", foreground="green"
        )
    ...
    def _init_forecasts_tab(self):
        ...
        self.submit_forecast_button = ttk.Button(
            forecast_controls_frame,
            text="Submit for Training Review",
            command=self.submit_forecasts_for_review,
        )
        self.submit_forecast_button.pack(side="left", padx=10)
        self.submit_forecast_button.config(
            state="disabled"
        )  # Initially disabled until forecasts are loaded
        ...
        self.forecasts_tree = ttk.Treeview(
            table_frame, columns=("Variable", "Value"), show="headings"
        )
        ...
        tree_scrollbar = ttk.Scrollbar(
            self.forecasts_tree, orient="vertical", command=self.forecasts_tree.yview
        )
    ...
    def fetch_forecasts(self):
        ...
        self.forecasts_tree.delete(
            *self.forecasts_tree.get_children()
        )  # Clear existing data
    ...
    def _visualize_forecast(self, forecast_data):
            ...
            if "historical" in var_data and "forecasted" in var_data:
                variables[var_name] = {
                    ...
                    "dates": var_data.get(
                        ...
                        list(
                            range(
                                len(var_data.get("historical", []))
                                + len(var_data.get("forecasted", []))
                            )
                        ),
                    ),
                }
        ...
        for var_name, var_data in variables.items():
            ...
            x_forecast = (
                ...
                if dates
                else list(range(len(historical), len(historical) + len(forecasted)))
            )

            # Plot historical data
            ax.plot(
                x_hist, historical, "b-", label="Historical", marker="o", markersize=4
            )
            ...
            if "confidence_lower" in var_data and "confidence_upper" in var_data:
                ...
                if len(forecasted) > 1:
                    volatility = sum(
                        [
                            abs(forecasted[i] - forecasted[i - 1])
            ...
            if x_hist and x_forecast:
                split_point = (
                    x_hist[-1] + (x_forecast[0] - x_hist[-1]) / 2
        ...
        if "scenarios" in forecast_data and var_index <= rows * cols:
            ...
            if variables and forecast_data.get("scenarios"):
                ...
                x_forecast = (
                    dates[len(historical) :]
                    if dates and "historical" in var_data
                    else list(range(len(forecasted)))
                )

                # Plot each scenario
                for scenario_name, scenario_data in scenarios.items():
                    if (
                        first_var in scenario_data
                        and "values" in scenario_data[first_var]
                    ):
    ...
    def submit_forecasts_for_review(self):
        ...
        if not hasattr(self, "current_forecast_data") or not self.current_forecast_data:
            messagebox.showerror(
                "Submission Error", "No forecast data available to submit."
            )
            return

        self.forecasts_status_label.config(
            text="Submitting forecasts for training review...", foreground="blue"
        )
        ...
        response = self.make_request(
            "POST", "/forecasts/submit_review", data=submission_data
        )
        ...
        if response and response.get("status") == "success":
            ...
            self.forecasts_status_label.config(
                text=f"Failed to submit forecasts: {error_msg}", foreground="red"
            )
            messagebox.showerror(
                "Submission Error", f"Failed to submit forecasts: {error_msg}"
            )
    ...
    def _init_retrodiction_tab(self):
        ...
        ttk.Label(
            self.retrodiction_frame,
            text="Retrodiction Simulation",
            style="Header.TLabel",
        ).pack(pady=10)
        ...
        ttk.Label(input_frame, text="Variables of Interest (comma-separated):").grid(
            row=2, column=0, padx=5, pady=2, sticky="w"
        )
        self.variables_of_interest_entry = ttk.Entry(input_frame, width=40)
        self.variables_of_interest_entry.grid(
        ...
        # Run Button
        self.run_retrodiction_button = ttk.Button(
            control_frame,
            text="Run Retrodiction Simulation",
        ...
        self.submit_retrodiction_button = ttk.Button(
            control_frame,
            text="Submit for Training Review",
        ...
        text_scrollbar = ttk.Scrollbar(
            self.retrodiction_results_text,
            orient="vertical",
            command=self.retrodiction_results_text.yview,
        )
    ...
    def _run_retrodiction_simulation(self):
        ...
        self.retrodiction_status_label.config(
            text="Running retrodiction simulation...", foreground="blue"
        )
        self.run_retrodiction_button.config(
            state="disabled"
        ...
        threading.Thread(
            target=self._run_retrodiction_background,
        ...
        try:
            ...
            if response and response.get("status") == "success":
                ...
                error_msg = (
                    response.get("error", "Unknown error")
                    if response
                    else "Failed to run retrodiction"
                )
    ...
    def _handle_retrodiction_success(self, response):
        ...
        # Update status
        self.retrodiction_status_label.config(
            text="Retrodiction completed successfully.", foreground="green"
        )
    ...
    def _handle_retrodiction_error(self, error_message):
        ...
        self.run_retrodiction_button.config(state="normal")  # Re-enable button
        self.submit_retrodiction_button.config(
            state="disabled"
    ...
    def _visualize_retrodiction(self, retrodiction_data):
        ...
        for var_name, var_data in variables.items():
            ...
            dates = (
                var_data["dates"]
                if var_data["dates"]
                else list(range(len(historical) + len(retrodicted)))
            )
            ...
            x_retro = (
                dates[len(historical) :]
                if dates
                else list(range(len(historical), len(historical) + len(retrodicted)))
            )
            ax.plot(
                x_retro,
                retrodicted,
    ...
    def submit_retrodiction_for_review(self):
        ...
        if (
            not hasattr(self, "current_retrodiction_results")
            or not self.current_retrodiction_results
        ):
            messagebox.showerror(
                "Submission Error", "No retrodiction results available to submit."
            )
            return

        self.retrodiction_status_label.config(
            text="Submitting retrodiction for training review...", foreground="blue"
        )
        ...
        # Submit the request
        response = self.make_request(
            "POST", "/retrodiction/submit_review", data=submission_data
        )
        ...
        if response and response.get("status") == "success":
            ...
            self.retrodiction_status_label.config(
                text=f"Failed to submit retrodiction: {error_msg}", foreground="red"
            )
            messagebox.showerror(
                "Submission Error", f"Failed to submit retrodiction: {error_msg}"
            )
    ...
    def _init_autopilot_tab(self):
        ...
        # Header
        ttk.Label(
            self.autopilot_frame, text="Pulse Autopilot Control", style="Header.TLabel"
        ).pack(pady=(10, 20))

        # Control panel
        control_panel = ttk.LabelFrame(
            self.autopilot_frame, text="Autopilot Control Panel", padding=10
        )
        ...
        self.autopilot_status_label = ttk.Label(
        ...
        self.start_button = ttk.Button(
            button_frame, text="Start Autopilot", command=self.start_autopilot
        )
        self.start_button.grid(row=0, column=0, padx=5, pady=5)

        self.stop_button = ttk.Button(
            button_frame, text="Stop Autopilot", command=self.stop_autopilot
        )
        self.stop_button.grid(row=0, column=1, padx=5, pady=5)
        self.stop_button.config(state="disabled")  # Initially disabled

        self.check_status_button = ttk.Button(
            button_frame, text="Check Status", command=self.check_autopilot_status
        )
        self.check_status_button.grid(row=0, column=2, padx=5, pady=5)

        # Configuration section
        config_frame = ttk.LabelFrame(
            self.autopilot_frame, text="Autopilot Configuration", padding=10
        )
        ...
        ttk.Label(mode_frame, text="Run Mode:").grid(
            row=0, column=0, padx=5, pady=5, sticky="w"
        )
        ...
        ttk.Label(interval_frame, text="Check Interval (seconds):").grid(
            row=0, column=0, padx=5, pady=5, sticky="w"
        )
        self.check_interval = tk.StringVar(value="300")
        interval_entry = ttk.Entry(
            interval_frame, textvariable=self.check_interval, width=10
        )
        interval_entry.grid(row=0, column=1, padx=5, pady=5, sticky="w")

        # Advanced options
        advanced_frame = ttk.LabelFrame(
            config_frame, text="Advanced Options", padding=10
        )
        ...
        ttk.Checkbutton(
            advanced_frame, text="Enable Learning", variable=self.enable_learning
        ).pack(anchor="w", pady=2)

        self.enable_alerts = tk.BooleanVar(value=True)
        ttk.Checkbutton(
            advanced_frame, text="Enable Alerts", variable=self.enable_alerts
        ).pack(anchor="w", pady=2)

        self.aggressive_mode = tk.BooleanVar(value=False)
        ttk.Checkbutton(
            advanced_frame,
            text="Aggressive Intervention",
            variable=self.aggressive_mode,
        ).pack(anchor="w", pady=2)

        # Save config button
        save_button = ttk.Button(
            config_frame, text="Save Configuration", command=self.save_autopilot_config
        )
        save_button.pack(pady=10)

        # Log section
        log_frame = ttk.LabelFrame(
            self.autopilot_frame, text="Autopilot Activity Log", padding=10
        )
    ...
    def start_autopilot(self):
        ...
        try:
            ...
            if interval < 10:
                messagebox.showwarning(
                    "Invalid Interval", "Interval must be at least 10 seconds."
                )
                return
        except ValueError:
            messagebox.showwarning(
                "Invalid Interval", "Interval must be a valid number."
            )
            return
        ...
        if response and response.get("status") == "started":
            ...
            messagebox.showinfo(
                "Autopilot Started", "Autopilot has been started successfully."
            )
        else:
            error_msg = (
                response.get("error", "Unknown error")
            ...
            messagebox.showerror(
                "Start Error", f"Failed to start autopilot: {error_msg}"
            )
    ...
    def stop_autopilot(self):
        ...
        if response and response.get("status") == "stopped":
            ...
            messagebox.showinfo(
                "Autopilot Stopped", "Autopilot has been stopped successfully."
            )
        else:
            error_msg = (
                response.get("error", "Unknown error") if response else "Failed to stop"
            )
        ...
        if response and "status" in response:
            ...
            if status == "running":
                ...
                if "last_action" in response:
                    self._update_autopilot_log(
                        "Last action: " + response["last_action"]
                    )
            else:
                self.autopilot_status_label.config(text="Stopped", foreground="gray")
                self.start_button.config(state="normal")
    ...
    def save_autopilot_config(self):
        ...
        if response and response.get("status") == "configured":
            ...
            messagebox.showinfo(
                "Configuration Saved", "Autopilot configuration has been saved."
            )
        else:
            error_msg = (
                response.get("error", "Unknown error")
            ...
            messagebox.showerror(
                "Configuration Error", f"Failed to save configuration: {error_msg}"
            )
    ...
    def trigger_ai_training_audit(self):
        ...
        if not previous_batch_id or not current_batch_id:
            self._update_ai_review_status(
                "Please enter both Previous Batch ID and Current Batch ID.", "red"
            )
            return
        ...
        if response_data:
            ...
            self.audit_report_text.insert(
                tk.END, "Error: Failed to trigger audit request."
            )
    ...
    def _display_audit_report(self, report):
        ...
        try:
            ...
            self.audit_report_text.insert(
                tk.END, f"Error displaying report: {e}\n{report}"
            )
    ...
    def _update_ai_review_status(self, message, color="black"):
        ...
        if data and data.get("status") == "success":
            ...
            error_message = (
                data.get("error", "Failed to start autopilot.")
                if data
                else "Failed to start autopilot."
            )
            self.autopilot_status_label.config(
                text=f"Autopilot Status: Error - {error_message}"
            )
        ...
        if data and data.get("status") == "success":
            ...
            error_message = (
                data.get("error", "Failed to stop autopilot.")
                if data
                else "Failed to stop autopilot."
            )
            self.autopilot_status_label.config(
                text=f"Autopilot Status: Error - {error_message}"
            )
    ...
    def fetch_autopilot_status(self):
        ...
        if data and "status" in data:
            self.autopilot_status_label.config(
                text=f"Autopilot Status: {data['status']}"
            )
        else:
            error_message = (
                data.get("error", "Failed to fetch status.")
                if data
                else "Failed to fetch status."
            )
            self.autopilot_status_label.config(
                text=f"Autopilot Status: Error - {error_message}"
            )
    ...
    def fetch_autopilot_history(self):
        ...
        self.autopilot_history_tree.delete(
            *self.autopilot_history_tree.get_children()
        )  # Clear existing data
        ...
        if data and isinstance(data, list):
            for run in data:
                ...
                self.autopilot_history_tree.insert(
                    "", "end", values=(run_id, start_time, end_time, status)
                )
        else:
            error_message = (
                data.get("error", "Unknown error")
                if data and isinstance(data, dict)
                else "Failed to fetch history."
            )
    ...
    def _start_autopilot_status_polling(self):
        ...
        self.autopilot_status_polling_job = self.root.after(
            5000, self._start_autopilot_status_polling
        )
    ...
    def _init_learning_tab(self):
        ...
        ttk.Label(
            self.learning_frame,
            text="Learning & Training Review",
            style="Header.TLabel",
        ).pack(pady=(10, 20))

        # Training Audit Controls Panel
        audit_frame = ttk.LabelFrame(
            self.learning_frame, text="Training Audit Controls", padding=10
        )
        ...
        ttk.Label(batch_frame, text="Previous Batch ID:").grid(
            row=0, column=0, padx=5, pady=5, sticky="w"
        )
        ...
        ttk.Label(batch_frame, text="Current Batch ID:").grid(
        ...
        audit_button = ttk.Button(
            audit_frame,
            text="Trigger AI Training Audit",
        ...
        report_frame = ttk.LabelFrame(
            self.learning_frame, text="Audit Report", padding=10
        )
        ...
        log_frame = ttk.LabelFrame(
            self.learning_frame, text="Learning Log Summary", padding=10
        )
        log_frame.pack(padx=10, pady=10, fill="x")

        # Add a button to load learning events
        load_log_button = ttk.Button(
            log_frame, text="Load Learning Events", command=self.load_learning_events
        )
            ...
            if self.has_pulse_modules and "learning.recursion_audit" in sys.modules:
                ...
                if events:
                    ...
                    for event in events[:5]:  # Show the 5 most recent events
                        ...
                        if "data" in event:
                            if (
                                event_type == "variable_weight_change"
                                and "variable" in event["data"]
                            ):
                                var = event["data"].get("variable", "")
                                old = event["data"].get("old_weight", "")
                                new = event["data"].get("new_weight", "")
                                summary_text += (
                                    f"  Variable: {var}, Weight: {old} → {new}\n"
                                )
                            elif (
                                event_type == "symbolic_upgrade"
                                and "plan" in event["data"]
                            ):
                    ...
                    self.learning_log_summary.delete("1.0", tk.END)
                    self.learning_log_summary.insert(
                        tk.END, "No learning events found in the log."
                    )
            else:
                self.learning_log_summary.delete("1.0", tk.END)
                self.learning_log_summary.insert(
                    tk.END, "Learning log viewer module is not available."
                )
        except Exception as e:
            self.learning_log_summary.delete("1.0", tk.END)
            self.learning_log_summary.insert(
                tk.END, f"Error loading learning events: {str(e)}"
            )
    ...
    def _init_analysis_tab(self):
        ...
        # Header
        ttk.Label(
            self.analysis_frame, text="Data Analysis Tools", style="Header.TLabel"
        ).pack(pady=(10, 20))
        ...
        ttk.Label(var_select_frame, text="Variable:").grid(
            row=0, column=0, padx=5, pady=5, sticky="w"
        )
        ...
        ttk.Label(var_select_frame, text="Time Range:").grid(
            row=1, column=0, padx=5, pady=5, sticky="w"
        )
        ...
        ttk.Label(prev_batch_frame, text="Previous Batch ID:").grid(
            row=0, column=0, padx=5, pady=5, sticky="w"
        )
        ...
        ttk.Label(curr_batch_frame, text="Current Batch ID:").grid(
            row=0, column=0, padx=5, pady=5, sticky="w"
        )
        ...
        ttk.Label(cluster_type_frame, text="Cluster Type:").grid(
            row=0, column=0, padx=5, pady=5, sticky="w"
        )
        self.cluster_type_combo = ttk.Combobox(
            cluster_type_frame,
            width=30,
        ...
        ttk.Label(viz_options_frame, text="Chart Type:").grid(
            row=0, column=0, padx=5, pady=5, sticky="w"
        )
        ...
        ttk.Label(time_period_frame, text="Time Period:").grid(
            row=0, column=0, padx=5, pady=5, sticky="w"
        )
        ...
        ttk.Label(metrics_frame, text="Metrics:").grid(
            row=0, column=0, padx=5, pady=5, sticky="w"
        )
        ...
        report_display_frame = ttk.LabelFrame(
            learning_metrics_frame, text="Learning Report"
        )
        report_display_frame.pack(fill="both", expand=True, padx=5, pady=5)

        self.learning_report_text = scrolledtext.ScrolledText(
            report_display_frame, wrap=tk.WORD
        )
        ...
        ttk.Button(
            export_frame,
            text="Export Current Analysis",
            command=self.export_current_analysis,
        ).pack(side="left", padx=5)

        ttk.Button(
            export_frame, text="Export All Analyses", command=self.export_all_analyses
        ).pack(side="left", padx=5)
    ...
    def generate_cluster_visualization(self):
        ...
        self.update_status(
            f"Generating {chart_type} visualization for {cluster_type}..."
        )
        ...
        try:
            ...
            if hasattr(self, "has_pulse_modules") and self.has_pulse_modules:
                ...
                self.generate_dummy_cluster_visualization(chart_type)
                self.update_status(
                    "Generated sample visualization (modules not available)"
                )
        except Exception as e:
    ...
    def _visualize_clusters(self, clusters, chart_type):
        ...
        ax.text(
            0.5,
            0.5,
            "Real cluster data visualization would go here",
            ha="center",
            va="center",
            fontsize=12,
        )
        ...
        if chart_type == "Network Graph":
            ...
            try:
                ...
                ax.text(
                    0.5,
                    0.5,
                    "scipy not available to create dendrogram",
                    ha="center",
                    va="center",
                )
    ...
    def _init_diagnostics_tab(self):
        ...
        # Header
        ttk.Label(
            self.diagnostics_frame, text="System Diagnostics", style="Header.TLabel"
        ).pack(pady=(10, 20))

        # System Health Monitoring
        health_frame = ttk.LabelFrame(
            self.diagnostics_frame, text="System Health", padding=10
        )
        ...
        ttk.Label(mem_frame, text="Memory Usage:").grid(
            row=0, column=0, padx=5, pady=5, sticky="w"
        )
        ...
        ttk.Label(cpu_frame, text="CPU Usage:").grid(
            row=0, column=0, padx=5, pady=5, sticky="w"
        )
        ...
        # Refresh button
        ttk.Button(
            health_frame, text="Refresh Metrics", command=self.refresh_system_metrics
        ).pack(pady=10)

        # Logs & Debug
        logs_frame = ttk.LabelFrame(
            self.diagnostics_frame, text="Logs & Debug Info", padding=10
        )
        ...
        ttk.Label(log_select_frame, text="Select Log:").grid(
            row=0, column=0, padx=5, pady=5, sticky="w"
        )
        ...
        # View log button
        ttk.Button(
            log_select_frame, text="View Log", command=self.view_selected_log
        ).grid(row=0, column=2, padx=5, pady=5)

        # Clear log button
        ttk.Button(
            log_select_frame, text="Clear Log", command=self.clear_selected_log
        ...
        tools_frame = ttk.LabelFrame(
            self.diagnostics_frame, text="Diagnostic Tools", padding=10
        )
        ...
        ttk.Button(
            tools_btn_frame, text="Test Connections", command=self.test_connections
        ).grid(row=0, column=0, padx=5, pady=5)

        ttk.Button(
            tools_btn_frame,
            text="Verify Data Integrity",
            command=self.verify_data_integrity,
        ).grid(row=0, column=1, padx=5, pady=5)

        ttk.Button(
            tools_btn_frame,
            text="Check Module Status",
            command=self.check_module_status,
        ).grid(row=0, column=2, padx=5, pady=5)
    ...
    def refresh_system_metrics(self):
        ...
        try:
            ...
            self.update_status(
                "Could not refresh system metrics - psutil module not available"
            )
        except Exception as e:
    ...
    def clear_selected_log(self):
        ...
        confirm = messagebox.askyesno(
            "Confirm", f"Are you sure you want to clear the {log_type}?"
        )
        if not confirm:
            return
    ...
    def test_connections(self):
        ...
        try:
            ...
            self.diagnostics_result.insert(
                tk.END, "✗ Database: Error checking status\n"
            )
        ...
        try:
            ...
            if os.path.exists(logs_dir) and os.access(logs_dir, os.R_OK | os.W_OK):
                self.diagnostics_result.insert(
                    tk.END, "✓ File System: Read/Write access OK\n"
                )
            else:
                self.diagnostics_result.insert(tk.END, "✗ File System: Access issues\n")
        except Exception:
            self.diagnostics_result.insert(
                tk.END, "✗ File System: Error checking access\n"
            )
                ...
                try:
                    with open(full_path, "r", encoding="utf-8") as f:
                        ...
                        if file_path.endswith(".yaml") or file_path.endswith(".yml"):
                            ...
                            json.load(f)
                            self.diagnostics_result.insert(
                                tk.END, f"✓ {file_path}: Valid JSON\n"
                            )
                        else:
                            # For other files, just check they're readable
                            f.read()
                            self.diagnostics_result.insert(
                                tk.END, f"✓ {file_path}: Readable\n"
                            )
                except Exception as e:
                    self.diagnostics_result.insert(
                        tk.END, f"✗ {file_path}: Error - {str(e)}\n"
                    )
            else:
    ...
    def check_module_status(self):
        ...
        for module_name in modules_to_check:
            if module_name in sys.modules:
                ...
                try:
                    ...
                    self.diagnostics_result.insert(
                        tk.END, f"✓ {module_name}: Available\n"
                    )
                except ImportError:
                    self.diagnostics_result.insert(
                        tk.END, f"✗ {module_name}: Not available\n"
                    )
        ...
        for module_name in optional_modules:
            try:
                ...
                self.diagnostics_result.insert(
                    tk.END, f"✗ {module_name}: Not installed\n"
                )
    ...
    def load_variable_trace(self):
        ...
        if not selected_var:
            messagebox.showwarning(
                "Selection Required", "Please select a variable to analyze."
            )
            return
        ...
        try:
            ...
            if hasattr(self, "has_pulse_modules") and self.has_pulse_modules:
                ...
                try:
                    from dev_tools.pulse_ui_plot import (
                        load_variable_trace,
                        plot_variables,
                    )
                    ...
                    self.update_status(
                        f"Variable trace for {selected_var} loaded successfully"
                    )
                except Exception as e:
                ...
                self.generate_dummy_variable_plot(selected_var)
                self.update_status(
                    f"Generated sample data for {selected_var} (modules not available)"
                )
        except Exception as e:
    ...
    def generate_dummy_variable_plot(self, variable_name):
        ...
        _toolbar = NavigationToolbar2Tk(canvas, self.var_trace_plot_frame)
        ...
        try:
            if hasattr(self, "has_pulse_modules") and self.has_pulse_modules:
                try:
                    ...
                    # Display header
                    self.learning_report_text.insert(
                        tk.END, f"Learning Report for {time_period}\n"
                    )
                    ...
                    for event_type, count in summary.items():
                        self.learning_report_text.insert(
                            tk.END, f"{event_type}: {count} events\n"
                        )
                    ...
                    self.learning_report_text.insert(
                        tk.END,
                        f"Error generating report with actual module: {str(e)}\n",
                    )
                    # Fall back to dummy report
                    self.generate_dummy_learning_report(
                        time_period, include_accuracy, include_trust, include_events
                    )
            else:
                ...
                self.update_status(
                    "Generated sample learning report (modules not available)"
                )
        except Exception as e:
            self.learning_report_text.insert(
                tk.END, f"Error generating report: {str(e)}"
            )
            self.update_status(f"Error generating learning report: {str(e)}")

    def generate_dummy_learning_report(
        self, time_period, include_accuracy, include_trust, include_events
    ):
        ...
        if include_accuracy:
            ...
            self.learning_report_text.insert(tk.END, "-" * 30 + "\n")
            self.learning_report_text.insert(
                tk.END, "Overall Prediction Accuracy: 76.3%\n"
            )
            self.learning_report_text.insert(tk.END, "Symbolic Arc Prediction: 82.1%\n")
            self.learning_report_text.insert(
                tk.END, "Capital Outcome Prediction: 71.8%\n\n"
            )
        ...
        if include_trust:
            ...
            self.learning_report_text.insert(
                tk.END, "Variable Volatility Index: 0.342\n\n"
            )
        ...
        if include_events:
            ...
            for i in range(10):
                event_time = (now - timedelta(hours=i * 3)).strftime(
                    "%Y-%m-%d %H:%M:%S"
                )
                event_type = event_types[i % len(event_types)]

                self.learning_report_text.insert(
                    tk.END, f"{event_time} - {event_type}\n"
                )

                if event_type == "variable_weight_change":
                    var_name = f"var_{chr(65 + i % 8)}"  # var_A to var_H
                    self.learning_report_text.insert(
                        tk.END, f"  variable: {var_name}\n"
                    )
                    self.learning_report_text.insert(
                        tk.END, f"  old_weight: {0.8 + (i % 5) * 0.05:.2f}\n"
                    )
                    self.learning_report_text.insert(
                        tk.END, f"  new_weight: {0.75 + (i % 7) * 0.05:.2f}\n"
                    )
                elif event_type == "symbolic_upgrade":
                    self.learning_report_text.insert(
                        tk.END, f"  hope: +{(i % 10) * 0.03:.2f}\n"
                    )
                    self.learning_report_text.insert(
                        tk.END, f"  despair: -{(i % 8) * 0.02:.2f}\n"
                    )
                elif event_type == "revision_trigger":
                    self.learning_report_text.insert(
                        tk.END,
                        f"  reason: {'fragmentation > threshold' if i % 2 else 'confidence < minimum'}\n",
                    )
                elif event_type == "arc_regret":
                    self.learning_report_text.insert(
                        tk.END, f"  arc_hope: {(i % 10) * 0.1:.2f}\n"
                    )
                    self.learning_report_text.insert(
                        tk.END, f"  arc_despair: {(10 - i % 10) * 0.1:.2f}\n"
                    )
    ...
    def export_current_analysis(self):
        ...
        if tab_text != "Analysis":
            messagebox.showinfo(
                "Export", "Please navigate to the Analysis tab to export analysis."
            )
            return
        ...
        if notebook_tab_idx is None:
            messagebox.showwarning(
                "Export Error", "Could not determine the current analysis tab."
            )
            return
        ...
        try:
            ...
            with open(file_path, "w") as f:
                ...
                f.write(
                    "This is a placeholder for the actual analysis export functionality.\n"
                )
    ...
    def export_all_analyses(self):
        ...
        try:
            ...
            with open(report_path, "w") as f:
                f.write("Pulse Comprehensive Analysis Report\n")
                f.write(
                    f"Generated: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n\n"
                )
                f.write(
                    "This is a placeholder for the actual comprehensive export functionality.\n"
                )
            ...
            messagebox.showinfo(
                "Export Complete", f"All analyses exported to {report_path}"
            )
...
if __name__ == "__main__":
    ...
    app = PulseApp(root)
    root.mainloop()
    # Removed erroneous: _frame.pack(fill="both", expand=True, pady=5)
    # This appeared to be a malformed line, possibly a copy-paste or merge error.
    # If this frame packing was intended for a specific widget, it needs to be correctly placed.
