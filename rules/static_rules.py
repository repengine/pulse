    ...
    rules = [
        {
            ...
            "threshold": param_overrides.get("R001_EnergySpike", {}).get(
                "threshold", CONFIDENCE_THRESHOLD
            ),
            "effect_size": param_overrides.get("R001_EnergySpike", {}).get(
                "effect_size", 0.01
            ),
            "condition": lambda s, th=None: get_variable(s, "energy_price_index")
            > (th if th is not None else CONFIDENCE_THRESHOLD),
            "effects": lambda s, eff=None: set_variable(
                s,
                "inflation_index",
                get_variable(s, "inflation_index") + (eff if eff is not None else 0.01),
            ),
        ...
        },
        {
            ...
            "threshold": param_overrides.get("R002_TrustRebound", {}).get(
                "threshold", 0.65
            ),
            "effect_size": param_overrides.get("R002_TrustRebound", {}).get(
                "effect_size", 0.02
            ),
            "condition": lambda s, th=None: get_variable(s, "public_trust_level")
            > (th if th is not None else 0.65),
            "effects": lambda s, eff=None: set_variable(
                s,
                "ai_policy_risk",
                max(
                    0,
                    get_variable(s, "ai_policy_risk")
                    - (eff if eff is not None else 0.02),
                ),
            ),
