...
def is_certified_forecast(forecast: Dict[str, Any]) -> bool:
    ...
    if not isinstance(forecast, dict):
        logger.warning(f"Non-dictionary passed to is_certified_forecast: {type(forecast)}")
        return False
...
def explain_certification(forecast: Dict[str, Any]) -> str:
    ...
    if forecast.get("alignment_score", 0) < 75:
        reasons.append(f"Alignment score is {forecast.get('alignment_score', 0)}, needs 75+")
...
def tag_certified_forecasts(forecasts: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    if not isinstance(forecasts, list):
        logger.error(f"Expected list for tag_certified_forecasts, got {type(forecasts)}")
        return []
        
    result = []
    for forecast in forecasts:
        if not isinstance(forecast, dict):
            logger.warning(f"Skipping non-dictionary item in forecasts: {type(forecast)}")
            continue
...
def generate_certified_digest(forecasts: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    """
    if not isinstance(forecasts, list):
        logger.error(f"Expected list for generate_certified_digest, got {type(forecasts)}")
    ...
    return {
        ...
        "certified_trace_ids": [f.get("trace_id") for f in tagged if f.get("certified", False)]
    }
...
def _test_forecast_fidelity_certifier():
    ...
    dummy = [
        ...
        {"trust_label": "🟢 Trusted", "license_status": "✅ Approved", "alignment_score": 70, "confidence": 0.7, "drift_flag": False, "symbolic_fragmented": False, "symbolic_revision_needed": False},
        {"trust_label": "🟢 Trusted", "license_status": "✅ Approved", "alignment_score": 80, "confidence": 0.5, "drift_flag": False, "symbolic_fragmented": False, "symbolic_revision_needed": False},
    ]
