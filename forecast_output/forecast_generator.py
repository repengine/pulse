...
def generate_forecast(input_features: Dict, causal_model: Optional[StructuralCausalModel] = None) -> Dict:
    ...
    if not isinstance(input_features, dict):
        logger.error(f"Invalid input_features type: {type(input_features)}. Expected dict. Using empty dict.")
        input_features = {}
    ...
    logger.info(
        ...
        {k: input_features[k] for k in list(input_features.keys())[:3]} if input_features else {}
    )
    ...
    try:
        ...
        if not isinstance(simulation_forecast, dict):
            logger.error(f"Invalid simulation_forecast type: {type(simulation_forecast)}. Expected dict. Using default.")
            simulation_forecast = {"value": 0.0}
        if "value" not in simulation_forecast:
            logger.warning("simulation_forecast missing 'value' key. Adding default value: 0.0")
            simulation_forecast["value"] = 0.0
        # Ensure value is numeric
        try:
            simulation_forecast["value"] = float(simulation_forecast["value"])
        except (ValueError, TypeError):
            logger.warning(f"Invalid simulation_forecast value: {simulation_forecast.get('value')}. Using default: 0.0")
            simulation_forecast["value"] = 0.0
    ...
    if getattr(pulse_config, "AI_FORECAST_ENABLED", True):
        try:
                    ...
                    if isinstance(value, (int, float)):
                        ...
                        for nested_key, nested_value in value.items():
                            if isinstance(nested_value, (int, float)):
                                numeric_features[f"{key}_{nested_key}"] = float(nested_value)
                            elif nested_value is not None:
                                try:
                                    numeric_value = float(nested_value)
                                    numeric_features[f"{key}_{nested_key}"] = numeric_value
                                    logger.info(f"Converted non-numeric value for {key}_{nested_key}: {nested_value} to {numeric_value}")
                                except (ValueError, TypeError):
                                    logger.warning(f"Skipping non-numeric value for {key}_{nested_key}: {nested_value}")
                    elif value is not None:
                        # Attempt to convert string or other values to float if possible
                        try:
                            numeric_value = float(value)
                            numeric_features[key] = numeric_value
                            logger.info(f"Converted non-numeric value for {key}: {value} to {numeric_value}")
                        except (ValueError, TypeError):
                            logger.warning(f"Skipping non-numeric value for {key}: {value}")
                except Exception as e:
                    logger.warning(f"Error processing feature {key}: {e}")

            logger.info(f"Filtered numeric features: {len(numeric_features)} of {len(input_features)} total features")
            ...
            if not isinstance(ai_forecast, dict):
                logger.error(f"Invalid ai_forecast type: {type(ai_forecast)}. Expected dict. Using default.")
                ai_forecast = {"adjustment": 0.0}
            if "adjustment" not in ai_forecast:
                logger.warning("ai_forecast missing 'adjustment' key. Adding default adjustment: 0.0")
                ai_forecast["adjustment"] = 0.0
            ...
            try:
                ...
                logger.warning(f"Invalid ai_forecast adjustment: {ai_forecast.get('adjustment')}. Using default: 0.0")
                ai_forecast["adjustment"] = 0.0
