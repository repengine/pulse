...
def trust_label_distribution(forecasts: List[Dict]) -> Dict[str, int]:
    labels = [f.get("trust_label", "None") for f in forecasts]
    return dict(Counter(labels))


def symbolic_arc_shift(previous: List[Dict], current: List[Dict]) -> Dict[str, int]:
    """
...
def generate_recursion_report(previous: List[Dict], current: List[Dict]) -> Dict:
    """
