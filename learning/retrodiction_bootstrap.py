...
for date in dates:
    WORLDSTATE_TIMELINE[date].update({"co2_ppm": None, "global_temp_anomaly": None})
...
for date in dates:
    WORLDSTATE_TIMELINE[date].update(
        {"global_population": None, "urbanization_rate": None}
    )
...
for date in dates:
    WORLDSTATE_TIMELINE[date].update(
        {"global_democracy_index": None, "political_risk_events": None}
    )
