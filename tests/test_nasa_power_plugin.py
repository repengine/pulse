...
class TestNasaPowerPlugin(unittest.TestCase):
    ...
    @patch('iris.iris_plugins_variable_ingestion.nasa_power_plugin.ensure_data_directory')
    ...
    @patch('iris.iris_plugins_variable_ingestion.nasa_power_plugin.save_request_metadata')
    ...
    def test_fetch_signals(self, mock_save_processed, mock_save_response, mock_save_metadata, mock_get):
        """Test fetching signals from NASA POWER API."""
        # Mock the API response
        mock_get.return_value = MockResponse(self.mock_nasa_data, 200, {"Content-Type": "application/json"})
    ...
    def test_get_most_recent_date_with_data(self):
        ...
        most_recent = self.plugin._get_most_recent_date_with_data(param_data)
        self.assertEqual(most_recent, "20250430")  # Falls back to earlier date with valid data
