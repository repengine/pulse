...
def metrics_store(mock_config):
    ...
    with patch('os.path.exists', return_value=False):
        with patch('recursive_training.metrics.metrics_store.Path.mkdir', return_value=None):
...
class TestMetricsStore:
    ...
    def test_store_metric(self, metrics_store, sample_metric):
        ...
        with patch('builtins.open', mock_open()):
            ...
            metrics_store._update_indices.assert_called_once_with("test_metric_id", sample_metric)
    ...
    def test_get_metric(self, metrics_store):
        ...
        with patch('os.path.exists', return_value=True):
            with patch('builtins.open', mock_open(read_data=json.dumps({"id": "disk_id", "value": 73}))):
    ...
    def test_update_indices(self, metrics_store, sample_metric):
        ...
        assert "test_metric_id" in metrics_store.metrics_indices["by_type"].get("training_iteration", [])
        assert "test_metric_id" in metrics_store.metrics_indices["by_model"].get("test_model", [])
        
        # Verify date indexing
        date_part = sample_metric["timestamp"].split("T")[0]
        assert "test_metric_id" in metrics_store.metrics_indices["by_date"].get(date_part, [])
        
        # Verify tag indexing
        for tag in sample_metric["tags"]:
            assert "test_metric_id" in metrics_store.metrics_indices["by_tag"].get(tag, [])
    ...
    def test_update_summary(self, metrics_store, sample_metric):
        ...
        assert metrics_store.metrics_summary["time_range"]["start"] == sample_metric["timestamp"]
        assert metrics_store.metrics_summary["time_range"]["end"] == sample_metric["timestamp"]
    ...
    def test_query_metrics(self, metrics_store):
        ...
        def mock_get_metric(metric_id):
            return {
                ...
                "metric_type": "training_iteration" if "metric1" in metric_id or "metric2" in metric_id or "metric3" in metric_id else "validation",
                "model": "model_a" if "metric1" in metric_id or "metric3" in metric_id or "metric4" in metric_id else "model_b"
            }
        ...
        results = metrics_store.query_metrics(start_date="2025-04-30", end_date="2025-04-30")
        assert len(results) == 3
