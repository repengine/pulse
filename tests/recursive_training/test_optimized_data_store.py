...
class TestOptimizedDataStore:
    ...
    def test_batch_retrieve(self, optimized_store):
        ...
        optimized_store.retrieve = MagicMock(side_effect=lambda id: {"id": id, "value": int(id.split("_")[1])})
            ...
            with patch('os.path.exists', return_value=False):
                with patch('recursive_training.data.data_store.Path.mkdir', return_value=None):
                    with patch('builtins.open', MagicMock()):
                        ...
                        expected_extension = ".parquet" if expected_format == "parquet" else \
                                           ".h5" if expected_format == "hdf5" else ".pkl"
