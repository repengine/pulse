...
class TestS3DataStore:
    ...
    def test_store_dataset_s3(self, s3_store, sample_data):
        ...
        fixed_timestamp_str = "123456" # This needs to be YYYYMMDDHHMMSS
        ...
        with patch('recursive_training.data.data_store.datetime') as mock_datetime_in_module:
            ...
            def mock_get_optimized_storage_path_side_effect(dataset_name_arg):
                print(f"mock_get_optimized_storage_path called with: {dataset_name_arg}")
            ...
            with patch('recursive_training.data.s3_data_store.S3DataStore._get_optimized_storage_path', return_value=Path(local_path)) as mock_get_path:
                ...
                with patch('os.path.exists', return_value=True) as mock_exists:
                    ...
                    with patch.object(s3_store, '_upload_to_s3', return_value=True) as mock_upload:
                        ...
                        result_id = s3_store.store_dataset_s3(dataset_name_param, sample_data)
                        ...
                        mock_returned_datetime_obj.strftime.assert_called_with('%Y%m%d%H%M%S')
                        ...
                        mock_upload.assert_called_once_with(str(Path(local_path)), s3_store.s3_data_bucket, expected_s3_key)
    ...
    def test_fallback_to_parent_methods(self, s3_store, sample_data):
        ...
        with patch.object(s3_store, '_read_parquet_from_s3', return_value=None):
            with patch.object(s3_store, '_ensure_dataset_in_cache', return_value=None):
                ...
                with patch('recursive_training.data.optimized_data_store.OptimizedDataStore.retrieve_dataset_optimized',
