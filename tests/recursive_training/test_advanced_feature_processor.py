...
class TestAdvancedFeatureProcessor(unittest.TestCase):
    ...
    def test_process(self, mock_ssl, mock_graph, mock_tf):
        ...
        mock_graph.assert_called_once_with(self.data_items, self.config["graph_features"])
        mock_ssl.assert_called_once_with(self.data_items, self.config["self_supervised"])
    ...
    def test_process_with_disabled_techniques(self):
        ...
        with patch('recursive_training.data.advanced_feature_processor.apply_time_frequency_decomposition') as mock_tf:
            ...
            self.assertFalse(result["metadata"]["techniques_applied"]["self_supervised"])
