...
from recursive_training.data.streaming_data_store import StreamingDataStore, PYARROW_AVAILABLE
...
class TestStreamingDataStore(unittest.TestCase):
    ...
    def test_access_pattern_tracking(self):
        ...
        store._track_access_pattern("test_dataset", "2023-01-01", "2023-01-31", ["value"])
        store._track_access_pattern("test_dataset", "2023-02-01", "2023-02-28", ["value", "category"])
    ...
    def test_preloading(self):
        ...
        store._track_access_pattern("dataset2", "2023-02-01", "2023-02-28", ["category"])
        
        # Mock the prefetch method to track which datasets would be preloaded
        with patch.object(store, '_prefetch_data', return_value=pd.DataFrame()) as mock_prefetch:
            # Test preloading
    ...
    def test_streaming_store(self):
        ...
        def data_generator():
            for i in range(300):
                yield {
                    "timestamp": (pd.Timestamp('2023-01-01') + pd.Timedelta(days=i)).isoformat(),
                    "value": float(np.random.randn()),
                    "category": np.random.choice(['A', 'B', 'C'])
                }
        
        # Store the streaming data with a small batch size to test batching
        dataset_id = store.store_dataset_streaming(
