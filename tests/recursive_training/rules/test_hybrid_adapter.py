...
def hybrid_adapter(mock_config, mock_cost_controller):
    ...
    with patch('recursive_training.rules.hybrid_adapter.get_cost_controller') as mock_get_cost_controller:
        with patch('recursive_training.rules.hybrid_adapter.get_config') as mock_get_config:
            # Setup mocks
...
class TestDictToObjectConversion:
    ...
    def test_basic_conversion(self, hybrid_adapter, sample_rule_dict):
        ...
        for i, condition in enumerate(rule_obj.conditions):
            ...
            assert condition.parameters == sample_rule_dict["conditions"][i]["parameters"]
            assert condition.description == sample_rule_dict["conditions"][i]["description"]
        ...
        assert rule_obj.metadata.created_at == sample_rule_dict["metadata"]["created_at"]
...
class TestObjectToDictConversion:
    ...
    def test_basic_conversion(self, hybrid_adapter, sample_rule_dict):
        ...
        # Verify metadata
        assert result_dict["metadata"]["created_at"] == sample_rule_dict["metadata"]["created_at"]
        assert result_dict["metadata"]["version"] == sample_rule_dict["metadata"]["version"]
        assert result_dict["metadata"]["status"] == sample_rule_dict["metadata"]["status"]
...
class TestCostTracking:
    ...
    def test_track_conversion_cost(self, hybrid_adapter, sample_rule_dict, mock_cost_controller):
