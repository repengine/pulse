...
def rule_repository(mock_config, tmp_path):
    ...
    with patch(
        "recursive_training.rules.rule_repository.get_config"
    ) as mock_get_config:
        # Setup config mock
...
class TestRuleStorage:
    ...
    def test_add_rule(self, rule_repository, sample_rule):
        ...
        assert (
            rule_repository.rule_index[sample_rule["id"]]["type"] == sample_rule["type"]
        )

        # Verify file was created
        rule_path = os.path.join(
            rule_repository.active_rules_path, f"{sample_rule['id']}_v1.json"
        )
    ...
    def test_update_rule(self, rule_repository, sample_rule):
        ...
        rule_path_v1 = os.path.join(
            rule_repository.active_rules_path, f"{sample_rule['id']}_v1.json"
        )
        rule_path_v2 = os.path.join(
            rule_repository.active_rules_path, f"{sample_rule['id']}_v2.json"
        )
    ...
    def test_delete_rule(self, rule_repository, sample_rule):
        ...
        assert sample_rule["id"] in rule_repository.rule_index
        assert (
            rule_repository.rule_index[sample_rule["id"]]["status"]
            == RuleStatus.ARCHIVED.value
        )
        ...
        archive_path = os.path.join(
            rule_repository.archive_path, f"{sample_rule['id']}_v1.json"
        )
...
class TestRuleStatusManagement:
    ...
    def test_change_rule_status(self, rule_repository, sample_rule):
        ...
        result = rule_repository.change_rule_status(
            sample_rule["id"], RuleStatus.DEPRECATED
        )
...
class TestBackupRestore:
    ...
    def test_create_backup(self, rule_repository, sample_rule):
        ...
        assert os.path.exists(
            os.path.join(active_backup, f"{sample_rule['id']}_v1.json")
        )
