...
def sample_invalid_rule():
    ...
    return {
        ...
        "actions": [{"variable": "discount", "value": 0.05}],
    }
    ...
    return [
        {
            "id": "rule_1",
            "type": "discount",
            "conditions": [{"variable": "price", "operator": ">", "value": 100}],
            "actions": [{"variable": "discount", "value": 0.1}],
            "priority": 1,
        ...
        },
        {
            ...
            "actions": [{"variable": "discount", "value": 0.15}],
            "priority": 2,
...
def rule_evaluator(mock_config, mock_cost_controller, mock_metrics_store):
    ...
    with patch(
        "recursive_training.rules.rule_evaluator.get_cost_controller"
    ) as mock_get_cost_controller:
        with patch(
            "recursive_training.rules.rule_evaluator.get_metrics_store"
        ) as mock_get_metrics_store:
            with patch(
                "recursive_training.rules.rule_evaluator.get_config"
            ) as mock_get_config:
                # Setup mocks
        ...
        with patch("recursive_training.rules.rule_evaluator.get_cost_controller"):
            with patch("recursive_training.rules.rule_evaluator.get_metrics_store"):
                with patch("recursive_training.rules.rule_evaluator.get_config"):
                    ...
                    assert (
                        evaluator.min_acceptable_score
                        == mock_config.min_acceptable_score
                    )
...
class TestRuleEvaluation:
    ...
    def test_evaluate_rule_syntax_only(
        self, rule_evaluator, sample_valid_rule, sample_context
    ):
    ...
    def test_evaluate_rule_comprehensive(
        self, rule_evaluator, sample_valid_rule, sample_context
    ):
        """Test evaluating rule with comprehensive scope."""
        # Patch internal evaluation methods for controlled testing
        with (
            patch.object(rule_evaluator, "_evaluate_syntax") as mock_syntax,
    ...
    def test_syntax_evaluation_failure(
        self, rule_evaluator, sample_invalid_rule, sample_context
    ):
    ...
    def test_error_handling(self, rule_evaluator, sample_valid_rule, sample_context):
        ...
        with patch.object(
            rule_evaluator, "_evaluate_syntax", side_effect=Exception("Simulated error")
        ):
            # Evaluate rule
            result = rule_evaluator.evaluate_rule(
                rule=sample_valid_rule, context=sample_context
            )
...
class TestRuleComparison:
    ...
    def test_compare_rules(
        self, rule_evaluator, sample_rules_for_comparison, sample_context
    ):
        ...
        with patch.object(rule_evaluator, "compare_rules") as mock_compare:
            ...
            )  # Improvements from rule_1 and rule_2 to rule_3
            assert any(
                imp["from_rule"] == "rule_1" and imp["improvement"] == 0.2
                for imp in result["improvements"]
            )
            assert any(
                imp["from_rule"] == "rule_2" and imp["improvement"] == 0.1
                for imp in result["improvements"]
            )

    def test_compare_empty_rules(self, rule_evaluator, sample_context):
        """Test comparing an empty rules list."""
        result = rule_evaluator.compare_rules(rules=[], context=sample_context)
