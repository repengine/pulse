...
def ingestion_manager(mock_config):
    ...
    with patch(
        "recursive_training.data.ingestion_manager.get_config"
    ) as mock_get_config:
        ...
        with patch(
            "recursive_training.data.ingestion_manager.RecursiveDataStore"
        ) as mock_data_store:
...
class TestRecursiveDataIngestionManager:
            ...
            with patch("recursive_training.data.ingestion_manager.RecursiveDataStore"):
                with patch(
                    ...
                    with patch("os.path.exists", return_value=True):
                        manager = RecursiveDataIngestionManager(
                            config_path="fake_path.json"
                        )
    ...
    def test_process_data(self, ingestion_manager, sample_data_items):
        ...
        with patch(
            "recursive_training.data.data_store.RecursiveDataStore.get_instance",
    ...
    def test_singleton_pattern(self):
        ...
        with patch(
            "recursive_training.data.ingestion_manager.RecursiveDataIngestionManager"
        ) as mock_class:
            mock_instance = MagicMock()
