...
def enhanced_curriculum(mock_config, mock_data_store):
    with patch(
        "recursive_training.advanced_metrics.retrodiction_curriculum.get_data_store",
...
def test_update_curriculum(enhanced_curriculum):
    ...
    enhanced_curriculum.update_curriculum(
        current_iteration=2, recent_metrics={"mse": 0.05}, model=None
    )
    new_state = enhanced_curriculum.get_curriculum_state()
    assert isinstance(new_state, dict)
    assert (
        new_state != prev_state or new_state == prev_state
    )  # State may or may not change, but must be dict
...
def test_hybrid_rule_integration(enhanced_curriculum):
    ...
    enhanced_curriculum.update_curriculum(
        current_iteration=4,
        recent_metrics={"mse": 0.04, "rule_type": "hybrid"},
        model=None,
    )
    state = enhanced_curriculum.get_curriculum_state()
    assert isinstance(state, dict)
