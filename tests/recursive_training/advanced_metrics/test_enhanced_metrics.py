...
def mock_metrics_store():
    ...
    store.track_cost.return_value = {"total_cost": 10.0, "status": "ok"} # Ensure status is present
...
def enhanced_metrics(mock_config, mock_metrics_store, mock_cost_controller):
    with patch("recursive_training.advanced_metrics.enhanced_metrics.get_metrics_store", return_value=mock_metrics_store), \
         patch("recursive_training.advanced_metrics.enhanced_metrics.get_cost_controller", return_value=mock_cost_controller):
...
def test_statistical_significance_test(enhanced_metrics):
    ...
    result = enhanced_metrics.statistical_significance_test(errors_a, errors_b, test_type="ttest")
    assert isinstance(result, dict)
...
def test_track_advanced_iteration(enhanced_metrics, monkeypatch):
    ...
    # Apply the mocks
    monkeypatch.setattr(enhanced_metrics.__class__.__bases__[0], 'track_iteration', mock_track_iteration)
    monkeypatch.setattr(enhanced_metrics.metrics_store, 'store_metric', mock_store_metric)
...
def test_compare_models_advanced(enhanced_metrics):
    ...
    result = enhanced_metrics.compare_models_advanced(model_names, metric_names=["accuracy"])
    assert isinstance(result, dict)
    assert "overall_best_model" in result or "best_models_by_metric" in result or "comparisons" in result
