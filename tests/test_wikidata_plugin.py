...
class TestWikidataPlugin(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.mock_company_data = {
            "head": {
                "vars": ["companyLabel", "foundingDate", "employeeCount", "revenueUSD", "netIncomeUSD", 
                         "country", "industryLabel", "ceoLabel", "websiteLabel"]
            },
            "results": {
                "bindings": [
                    {
        ...
        self.mock_country_data = {
            "head": {
                "vars": ["countryLabel", "population", "gdpUSD", "gdpPerCapitaUSD", 
                         "currencyLabel", "inflationRate", "unemploymentRate", "centralBankLabel"]
            },
            "results": {
                "bindings": [
                    {
        ...
        self.mock_central_bank_data = {
            "head": {
                "vars": ["centralBankLabel", "foundingDate", "countryLabel", 
                         "currencyLabel", "bankRatePercent", "reservesUSD", "governorLabel"]
            },
            "results": {
                "bindings": [
                    {
    ...
    def test_plugin_init(self, mock_ensure_dir):
        ...
        self.assertEqual(plugin.concurrency, 1)  # Lower concurrency due to API rate limits
    ...
    def test_execute_sparql_query_company(self, mock_save_response, mock_save_metadata, mock_get):
        """Test execution of SPARQL query for a company."""
        # Mock the API response
        mock_get.return_value = MockResponse(self.mock_company_data, 200, {"Content-Type": "application/json"})
    ...
    def test_process_company_results(self, mock_save_processed, mock_save_response, mock_save_metadata, mock_get):
        ...
        self.assertGreaterEqual(len(signals), 4)  # At least 4 signals (3 metrics + profile)
        ...
        self.assertEqual(employee_count_signal["metadata"]["data_type"], "employee_count")
    ...
    @patch('iris.iris_plugins_variable_ingestion.wikidata_plugin.save_api_response')
    @patch('iris.iris_plugins_variable_ingestion.wikidata_plugin.save_processed_data')
    def test_process_country_results(self, mock_save_processed, mock_save_response, mock_save_metadata, mock_get):
        """Test processing of country query results into signals."""
        # Mock the API response and execute the query
        mock_get.return_value = MockResponse(self.mock_country_data, 200, {"Content-Type": "application/json"})
    ...
    @patch('random.choice')
    @patch('iris.iris_plugins_variable_ingestion.wikidata_plugin.WikidataPlugin._execute_sparql_query')
    @patch('iris.iris_plugins_variable_ingestion.wikidata_plugin.WikidataPlugin._process_query_results')
    def test_fetch_signals(self, mock_process_results, mock_execute_query, mock_random_choice):
        ...
        mock_process_results.return_value = [
            {"name": "wikidata_countries_united_states_population", "value": 331900000.0},
