import pytest
from simulation_engine.simulator_core import simulate_turn, simulate_forward, simulate_backward, simulate_counterfactual, validate_variable_trace, _validate_overlay, _copy_overlay, reset_state, inverse_decay, get_overlays_dict, log_to_file, reverse_rule_engine
...
def test_validate_overlay_valid():
    ...
    valid_overlay = {"hope": 0.5, "despair": 0.2, "neutral": 0.0} # Ensure values are floats
...
def test_copy_overlay(basic_worldstate):
    ...
    assert copied_overlays is not original_overlays_dict # Ensure it's a copy, not the same object
...
def test_reset_state(basic_worldstate):
    ...
    assert basic_worldstate.overlays.as_dict() == {"hope": 0.0, "despair": 0.0, "rage": 0.0, "fatigue": 0.0, "trust": 0.0}
    assert basic_worldstate.variables.as_dict() == {"energy_cost": 0.0}
    assert basic_worldstate.turn == 0
    assert basic_worldstate.event_log == []
    assert basic_worldstate.capital.as_dict() == {"nvda": 0.0, "msft": 0.0, "ibit": 0.0, "spy": 0.0, "cash": 0.0} # Capital resets to 0.0 for existing keys
...
def test_inverse_decay():
    ...
    assert inverse_decay(0.5, rate=0.05, floor=0.2, ceil=0.8) == 0.55 # Test custom bounds
...
def test_get_overlays_dict(basic_worldstate):
    ...
    assert get_overlays_dict(basic_worldstate) == {"hope": 0.5, "despair": 0.2, "rage": 0.5, "fatigue": 0.5, "trust": 0.5}
...
@patch("simulation_engine.simulator_core._copy_overlay") # Patch _copy_overlay as it's used internally
def test_simulate_turn_summary(
    ...
    mock_copy_overlay.side_effect = lambda x: x.as_dict() if hasattr(x, 'as_dict') else dict(x)

    mock_tag_symbolic_state.return_value = {"symbolic_tag": "TestTag", "symbolic_score": 0.8}
    mock_enrich_trust_metadata.side_effect = lambda x: {**x, "trust_label": "Trusted", "confidence": 0.9}
...
def test_simulate_turn_invalid_state():
    ...
    with pytest.raises(ValueError, match="Expected WorldState"):
        simulate_turn({}, use_symbolism=True) # Pass an empty dict instead of WorldState
...
def test_simulate_forward_basic(
    ...
    mock_simulate_turn.side_effect = [{"turn": i, "overlays": {"hope": 0.5 + i*0.1}} for i in range(3)]
...
@patch("simulation_engine.simulator_core.tag_symbolic_state")
@patch("simulation_engine.simulator_core.match_rule_by_delta")
@patch("simulation_engine.simulator_core.get_all_rule_fingerprints")
...
def test_simulate_backward(
    ...
    mock_get_all_rule_fingerprints,
    mock_match_rule_by_delta,
    mock_tag_symbolic_state,
    mock_inverse_decay,
    basic_worldstate
):
    """Test simulate_backward function."""
    mock_inverse_decay.side_effect = lambda value, rate: value + rate # Simple inverse decay mock
    mock_tag_symbolic_state.return_value = {"symbolic_tag": "BackwardTag"}
    mock_match_rule_by_delta.return_value = [("Rule1", 0.9)]
    mock_score_symbolic_trace.return_value = {"arc_label": "BackwardArc", "volatility_score": 0.5, "arc_certainty": 0.7}
    ...
    mock_reverse_rule_engine.side_effect = lambda *args: {"rule_chains": [], "symbolic_tags": [], "trust_scores": [], "symbolic_confidence": [], "suggestions": []}
...
def test_simulate_counterfactual(
    ...
    mock_simulate_forward.side_effect = [
        [{"turn": 0, "overlays": {"hope": 0.5}}, {"turn": 1, "overlays": {"hope": 0.6}}], # Base trace
        [{"turn": 0, "overlays": {"hope": 0.7}}, {"turn": 1, "overlays": {"hope": 0.8}}]  # Fork trace
    ]
...
def test_reverse_rule_engine_feature(mock_trace_causal_paths, mock_get_fingerprints):
    ...
    for chain in result["rule_chains"]:
        assert isinstance(chain, list) or (isinstance(chain, list) and chain and chain[0] == "SUGGEST_NEW_RULE")
