...
class TestOpenFdaPlugin(unittest.TestCase):
    def setUp(self):
        ...
        self.mock_count_data = {
            "meta": {
                ...
                "results": {"skip": 0, "limit": 10, "total": 125},
            },
            "results": [
                {"term": "Term 1", "count": 50},
                {"term": "Term 2", "count": 40},
            ],
            "count": [
        ...
        self.mock_recalls_data = {
            "meta": {
                ...
                "license": "API license text",
                "last_updated": "2025-05-01",
                "results": {"skip": 0, "limit": 10, "total": 50},
            },
            "results": [
                {
    ...
    def test_process_count_data(
        ...
        mock_get.return_value = MockResponse(
            self.mock_count_data, 200, {"Content-Type": "application/json"}
        )

        # Process count data directly
        count_results = self.mock_count_data.get("count", [])
        signals = self.plugin._process_count_data(
            "drug_events",
    ...
    def test_process_drug_events_data(
        ...
        # Mock the API response
        mock_get.return_value = MockResponse(
            self.mock_drug_events_data, 200, {"Content-Type": "application/json"}
        )
        ...
        self.assertAlmostEqual(
            serious_signal["value"], 66.67, places=2
        )  # 2/3 events are serious
    ...
    def test_process_recalls_data(
        ...
        # Mock the API response
        mock_get.return_value = MockResponse(
            self.mock_recalls_data, 200, {"Content-Type": "application/json"}
        )
