...
def test_openai_model(model_name="gpt-3.5-turbo", api_key=None):
    ...
    if not api_key:
        ...
        if not api_key:
            logger.error(
                "No OpenAI API key provided or found in environment variables."
            )
            assert False, (
    ...
    try:
        ...
        response = model.generate_response(
            "What is the capital of France?", max_new_tokens=50, temperature=0.7
        )
        ...
        start_time = time.time()
        response = model.generate_response(
            system_prompt, max_new_tokens=100, temperature=0.7
        )
        elapsed_time = time.time() - start_time
...
def main():
    ...
    parser.add_argument(
        "--model", default="gpt-3.5-turbo", help="OpenAI model name to test"
    )
    parser.add_argument(
        "--key", help="OpenAI API key (will use environment variable if not provided)"
    )
    parser.add_argument(
        "--mock-only", action="store_true", help="Only test the mock implementation"
    )
