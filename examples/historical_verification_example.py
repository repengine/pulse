...
def create_synthetic_dataset(variable_name, start_date, end_date, with_issues=True):
    ...
    if with_issues:
        ...
        for i in range(5):
            ...
            series.iloc[anomaly_idx] += np.random.choice([1, -1]) * np.random.uniform(15, 30)
...
def create_multiple_sources(variable_name, base_series):
    ...
    drop_indices = np.random.choice(range(len(source1_data)), size=len(source1_data)//10, replace=False)
...
def run_verification_demo():
    ...
    if quality_result.anomalies:
        ...
        for i, anomaly in enumerate(quality_result.anomalies[:5]):  # Show first 5
            print(f"  {i+1}. {anomaly.timestamp.date()}: value={anomaly.value:.2f}, " +
    ...
    if quality_result.gaps:
        ...
        for i, gap in enumerate(quality_result.gaps[:5]):  # Show first 5
            print(f"  {i+1}. {gap.start_time.date()} to {gap.end_time.date()}: " +
    ...
    if quality_result.trend_breaks:
        ...
        for i, break_ in enumerate(quality_result.trend_breaks):
            print(f"  {i+1}. {break_.timestamp.date()}: Before={break_.before_trend:.2f}, " +
    ...
    for method in methods:
        try:
            ...
            for i, anomaly in enumerate(sorted(anomalies, key=lambda a: a.severity, reverse=True)[:3]):
                print(f"  {i+1}. {anomaly.timestamp.date()}: value={anomaly.value:.2f}, " +
    ...
    try:
        gap_result = analyze_gaps(variable_name, freq='D')  # Explicitly use 'D' for daily frequency
        ...
        if 'error' not in gap_result:
            ...
            if 'gap_patterns' in gap_result and 'possible_pattern' in gap_result['gap_patterns']:
