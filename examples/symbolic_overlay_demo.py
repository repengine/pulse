...
from symbolic_system.context import symbolic_context, enter_retrodiction_mode, enter_simulation_mode
...
def print_overlays(state, title=None):
...
def demo_isolation_and_gating():
    ...
    with enter_simulation_mode(enable_symbolic=True):
        ...
        print(f"  hope: {state.overlays.hope:.2f}")
        print(f"  trust: {state.overlays.trust:.2f}")  # Trust should increase due to high hope
...
def demo_performance_optimization():
    ...
    print(f"Speed improvement: {first_call_time / max(second_call_time, 0.0001):.1f}x faster")
...
def demo_enhanced_configurability():
    ...
    print(f"  Hope->Trust interaction: {config.get_value('interaction_strengths.hope->trust')}")
    ...
    print(f"  Dominance threshold: {config.get_value('overlay_thresholds.dominance')}")
    print(f"  Despair->Fatigue interaction: {config.get_value('interaction_strengths.despair->fatigue')}")
    ...
    bullish_variables = {"volatility_index": 12, "gdp_growth": 2.5, "unemployment_rate": 3.8}
    ...
    volatile_variables = {"volatility_index": 35, "gdp_growth": 1.0, "unemployment_rate": 4.5}
...
def demo_overlay_sophistication():
    ...
    for name, value in state.overlays.get_children("hope").items():
        metadata = state.overlays.get_overlay_metadata(name)
        description = metadata.description if metadata else ""
        print(f"  {name:<10}: {value:.2f} - {description}")
    
    # Set up relationships between overlays
    print("\nSetting relationships between overlays...")
    state.overlays.set_relationship("optimism", "confidence", 0.5)  # optimism boosts confidence
    state.overlays.set_relationship("anxiety", "uncertainty", 0.7)  # anxiety strongly boosts uncertainty
    state.overlays.set_relationship("confidence", "anxiety", -0.6)  # confidence reduces anxiety
    
    print("Relationship strengths:")
    print(f"  optimism → confidence: {state.overlays.get_relationship('optimism', 'confidence'):.2f}")
    print(f"  anxiety → uncertainty: {state.overlays.get_relationship('anxiety', 'uncertainty'):.2f}")
    print(f"  confidence → anxiety: {state.overlays.get_relationship('confidence', 'anxiety'):.2f}")

def demo_symbolic_numeric_integration():
    ...
    for t in transformations:
        print(f"  {t['indicator']:<15} → {t['overlay']:<10}: {t['from_value']:.2f} → {t['to_value']:.2f} "
...
def full_demonstration():
    ...
    state.overlays.add_overlay("concentration", 0.5, "secondary", "trust", 
                             "Focus on specific market segments")
    state.overlays.add_overlay("anticipation", 0.7, "secondary", "hope",
    ...
    with enter_retrodiction_mode(enable_symbolic=False):
        ...
        if state.overlays.hope == original_hope and state.overlays.despair == original_despair:
