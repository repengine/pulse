...
class CausalDiscovery:
    ...
    def __init__(self, data: pd.DataFrame, max_workers: Optional[int] = None):
        ...
        try:
            ...
            logger.warning(
                "Optimized causal discovery not available, using standard implementation"
            )
    ...
    def run_pc(self, alpha: float = 0.05) -> StructuralCausalModel:
        ...
        try:
            ...
            logger.warning(
                f"Vectorized edge detection failed: {e}, falling back to pairwise"
            )
    ...
    def run_fci(self, alpha: float = 0.05) -> StructuralCausalModel:
        ...
        """
        if self.use_optimized and hasattr(
            self.optimized_discovery, "vectorized_fci_algorithm"
        ):
        ...
        try:
            ...
            logger.warning(
                f"Vectorized correlation failed: {e}, falling back to pairwise"
            )
            # Fall back to pairwise correlation
            for var1, var2 in itertools.combinations(self.data.columns, 2):
                corr_val = self.data[var1].corr(self.data[var2])
