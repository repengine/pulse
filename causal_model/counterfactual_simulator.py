...
class InterventionScenario:
    ...
    def from_dict(cls, data: Dict[str, Any]) -> 'InterventionScenario':
        ...
        if creation_time_str:
            try:
                scenario.creation_time = datetime.fromisoformat(creation_time_str.replace('Z', '+00:00'))
            except ValueError:
...
class CounterfactualSimulator:
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        ...
        self.storage_path = self.config.get('storage_path', 'data/counterfactual_scenarios')
    ...
    def register_data_source(self, source_name: str, data_loader: Callable[[], pd.DataFrame]):
        """
    ...
    def build_scm(self, data: Optional[pd.DataFrame] = None, method: str = "pc"):
        ...
        if data is None:
            ...
            for source_name, data_loader in self.data_sources.items():
                try:
                    ...
                    logger.info(f"Loaded data from source: {source_name}, shape: {df.shape}")
                except Exception as e:
        ...
        if method == "pc":
            ...
            logger.warning(f"Unknown causal discovery method: {method}, using default approach")
    ...
    def run_scenario(self, scenario_input: Union[str, InterventionScenario]) -> Dict[str, Any]:
        """
        ...
        if self.scm is None or self.counterfactual_engine is None:
            raise ValueError("No structural causal model available. Call build_scm() first.")
        ...
        try:
            ...
            if scenario.target_variables:
                filtered_results = {var: cf_results.get(var) for var in scenario.target_variables if var in cf_results}
            else:
            ...
            logger.info(f"Completed counterfactual scenario in {processing_time}: {scenario}")
    ...
    def run_batch_scenarios(self, scenario_inputs: List[Union[str, InterventionScenario]]) -> List[Dict[str, Any]]:
        """
        ...
        logger.info(f"Running batch of {len(scenario_objects)} counterfactual scenarios")
        ...
        try:
            batch_results = self.counterfactual_engine.predict_counterfactuals_batch(batch_queries)
            
            results = []
            for i, (scenario, cf_result) in enumerate(zip(scenario_objects, batch_results)):
                # Filter results to target variables if specified
                if scenario.target_variables:
                    filtered_results = {var: cf_result.get(var) for var in scenario.target_variables if var in cf_result}
                else:
            ...
            logger.info(f"Completed batch of {len(scenario_objects)} counterfactual scenarios in {batch_time:.2f}s")
    ...
    def compare_scenarios(self, scenario_ids: List[str], variables: Optional[List[str]] = None) -> Dict[str, Any]:
        """
        ...
        comparison = {
            "scenario_info": {scenario.scenario_id: {"name": scenario.name, "description": scenario.description} for scenario in scenarios},
            "variables": {},
    ...
    def load_scenarios(self):
        ...
        scenario_files = [f for f in os.listdir(self.storage_path) if f.endswith('.json')]
