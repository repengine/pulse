...
class OptimizedCausalDiscovery:
    ...
    def __init__(self, data, max_workers: Optional[int] = None):
        ...
        self.max_workers = max_workers if max_workers is not None else max(1, (os.cpu_count() or 4) - 1)
        ...
        if PANDAS_AVAILABLE and isinstance(data, pd.DataFrame):
            ...
            self.logger.warning("Data not in pandas DataFrame format. Some optimizations unavailable.")
    ...
    def vectorized_pc_algorithm(self, alpha: float = 0.05) -> StructuralCausalModel:
        ...
        # Add variables
        if PANDAS_AVAILABLE and isinstance(self.data, pd.DataFrame):
            ...
            for i, var1 in enumerate(self.data.columns):
                for j, var2 in enumerate(self.data.columns):
                    if i != j and self.corr_matrix is not None and self.corr_matrix.at[var1, var2] >= alpha:
    ...
    def _run_conditional_independence_tests(self, scm: StructuralCausalModel, alpha: float) -> None:
        """
                ...
                for edge in edge_batch:
                    ...
                    futures.append(
                        executor.submit(
                            ...
                            var1, var2, list(scm.graph.nodes()), alpha
                        )
                    )
    ...
    def _test_conditional_independence(self, var1: str, var2: str, all_vars: List[str], alpha: float) -> bool:
        """
        ...
        for size in range(min(3, len(conditioning_vars))):  # Limit to sets of size 3 for performance
            for cond_set in itertools.combinations(conditioning_vars, size):
                ...
                try:
                    ...
                    self.logger.warning(f"Error in independence test for {var1},{var2}: {e}")
...
def get_optimized_causal_discovery(data, max_workers: Optional[int] = None) -> OptimizedCausalDiscovery:
    """
