...
class TestForecastRetrospector(unittest.TestCase):
    def setUp(self):
        self.forecast = {
            ...
            "forecast": {"start_capital": {"nvda": 1000, "msft": 1200}},
        }
        self.current_state = {
            "hope": 0.6,
            "despair": 0.3,
    ...
    def test_retrospective_analysis_batch(self):
        ...
        results = retrospective_analysis_batch(
            forecasts, self.current_state, threshold=0.5
        )
        for res in results:
            ...
            if res["retrodiction_error"] > 0.5:
                self.assertEqual(
                    res.get("retrodiction_flag"), "⚠️ Symbolic misalignment"
                )
