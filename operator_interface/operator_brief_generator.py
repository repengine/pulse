...
def generate_operator_brief(
    ...
    if previous_episode_log and os.path.exists(previous_episode_log) and episode_log_file:
        ...
        arcs_curr = {k.replace("arc_", ""): v for k, v in current.items() if k.startswith("arc_")}
    ...
    try:
        with open(output_md_path, "w", encoding="utf-8") as f:
            f.write(f"# 🧠 Pulse Operator Brief\n")
            f.write(f"_Generated: {datetime.utcnow().isoformat()} UTC_\n\n")

            # Top N Forecasts
            if forecasts:
                top = sorted(forecasts, key=lambda x: x.get("alignment_score", 0), reverse=True)[:top_k]
                ...
                for fc in top:
                    ...
                    f.write(f"- `{fc.get('trace_id', 'unknown')}` | Score: {score} | Tag: {tag} | Arc: {arc} | Confidence: {conf}\n")
                f.write("\n")
            ...
            if forecasts and any("alignment_components" in f for f in forecasts):
                ...
                for fc in forecasts:
                    ...
                    if components.get("arc_stability", 1.0) < 0.6:
                        f.write(f"- `Volatile Arc`: {fc.get('trace_id')} → Stability: {components['arc_stability']}\n")
                    if components.get("novelty", 0.0) < 0.5:
                        f.write(f"- `Low Novelty`: {fc.get('trace_id')} → Novelty: {components['novelty']}\n")
                f.write("\n")
