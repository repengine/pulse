...
from utils.file_utils import load_jsonl, save_jsonl  # You must implement these in utils/file_utils.py
...
def validate_regret_schema(event: dict) -> bool:
...
def log_regret_event(trace_id: str, reason: str, arc_label: str = None, rule_id: str = None,
    ...
    if not validate_regret_schema(event):
        raise ValueError("Regret event missing required fields")
    save_jsonl(REGRET_LOG, [event], append=True)
    # Auto-trigger rule expansion if enabled
    try:
        from simulation_engine.rules.pulse_rule_expander import expand_rules_from_regret
        expand_rules_from_regret()
    except Exception as e:
        print(f"[RegretChain] Rule expansion failed: {e}")
    return event
