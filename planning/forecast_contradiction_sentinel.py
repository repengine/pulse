...
def symbolic_tag_conflicts(forecasts: List[Dict]) -> List[Tuple[str, str, str]]:
    """
    Detects symbolic tag conflicts (e.g., Hope vs Despair) between forecasts.
    Returns a list of (trace_id1, trace_id2, conflict_description).
    """
    conflicts = []
...
def arc_conflicts(forecasts: List[Dict]) -> List[Tuple[str, str, str]]:
    """
    Detects symbolic arc conflicts (e.g., Hope arc vs Despair arc).
    Returns a list of (trace_id1, trace_id2, conflict_description).
    """
    conflicts = []
...
def capital_conflicts(forecasts: List[Dict], threshold: float = 1000.0) -> List[Tuple[str, str, str]]:
    """
    Detects capital outcome conflicts between forecasts.
    Returns a list of (trace_id1, trace_id2, conflict_description).
    """
    conflicts = []
    for i in range(len(forecasts)):
...
def compute_symbolic_drift_score(f1: Dict, f2: Dict) -> float:
    """
    Computes the Euclidean distance between overlay vectors of two forecasts.
    Returns a float drift score.
    """
    o1 = f1.get("overlays", {})
    o2 = f2.get("overlays", {})
...
def symbolic_drift_clusters(forecasts: List[Dict], threshold: float = 0.3) -> List[Tuple[str, str, float]]:
    """
    Detects pairs of forecasts with symbolic drift above the threshold.
    Returns a list of (trace_id1, trace_id2, drift_score).
    """
    drifts = []
    for i in range(len(forecasts)):
        for j in range(i+1, len(forecasts)):
...
def validate_forecast_schema(f: dict) -> bool:
    required = {"trace_id", "symbolic_tag"}
    return all(k in f for k in required)

def contradiction_summary(forecasts: List[Dict]) -> Dict:
    valid = [f for f in forecasts if validate_forecast_schema(f)]
    return {
        "symbolic_tag_conflicts": symbolic_tag_conflicts(valid),
        "symbolic_arc_conflicts": arc_conflicts(valid),
        "capital_conflicts": capital_conflicts(valid),
        "symbolic_drift_clusters": symbolic_drift_clusters(valid),
        "total_forecasts": len(valid)
    }
