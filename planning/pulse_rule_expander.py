...
import os
from utils.file_utils import load_jsonl, save_json, load_json  # You must implement these in utils/file_utils.py
...
def load_rules(path: str = FINGERPRINT_FILE) -> Dict[str, Dict]:
    """Load rule fingerprints from JSON file."""
...
def validate_candidate_rule(candidate: dict) -> bool:
    """Validate candidate rule schema."""
...
def export_candidate_rules(candidates: List[Dict], path: str = SUGGESTED_FILE):
    """Export validated candidate rules to JSON."""
    valid_candidates = [c for c in candidates if validate_candidate_rule(c)]
    save_json(valid_candidates, path)
    print(f"[RuleExpander] Exported {len(valid_candidates)} candidate rules to {path}")
