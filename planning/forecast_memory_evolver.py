...
logger = logging.getLogger("forecast_memory_evolver")
        ...
        with open(path, "r") as f:
            for line in f:
                try:
                    ...
                    logger.error(f"Failed to parse regret line: {e}")
                    continue
    return regrets
...
def adjust_rule_trust_weights(rule_stats: Dict, threshold: int = 2) -> Dict:
    ...
    Returns a dict mapping rule_id to new trust_weight.
    """
    if not os.path.exists(FINGERPRINT_FILE):
        logger.warning("Fingerprint file not found.")
        return {}
    try:
        with open(FINGERPRINT_FILE, "r") as f:
            rules = json.load(f)
    except Exception as e:
        logger.error(f"Failed to load fingerprint file: {e}")
        return {}
    adjusted = {}
    ...
    try:
        ...
        logger.error(f"Failed to save fingerprint file: {e}")
    return adjusted
...
def evolve_memory_from_regrets(regret_path: str = REGRET_LOG, rule_file: str = FINGERPRINT_FILE) -> Dict:
    ...
    Returns a summary dict of evolution results.
    """
