...
_submissions_index = {"forecast": {}, "retrodiction": {}}
...
def store_training_submission(submission_type, submission_id, data, metadata=None):
    ...
    try:
        ...
        storage_dir = (
            FORECAST_REVIEWS_DIR
            if submission_type == "forecast"
            else RETRODICTION_REVIEWS_DIR
        )
    ...
    try:
        ...
        if submission_id not in _submissions_index[submission_type]:
            ...
            if not file_path.exists():
                logger.warning(
                    f"{submission_type.capitalize()} submission {submission_id} not found"
                )
                return None
        ...
        logger.error(
            f"Error retrieving {submission_type} submission {submission_id}: {str(e)}"
        )
        return None


def list_training_submissions(
    submission_type, limit=50, status=None, sort_by="created_at", reverse=True
):
    """
...
def update_training_submission_status(
    submission_type, submission_id, status, reviewer=None, notes=None
):
    """
    ...
    try:
        ...
        logger.info(
            f"Updated {submission_type} submission {submission_id} status to {status}"
        )
        return True

    except Exception as e:
        logger.error(
            f"Error updating {submission_type} submission {submission_id}: {str(e)}"
        )
        return False
