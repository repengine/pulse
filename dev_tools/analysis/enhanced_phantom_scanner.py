...
class EnhancedPhantomScanner:
    ...
    def __init__(self, root_dir: str):
        ...
        self.function_contexts = defaultdict(list)  # Function -> (file, line number) contexts
    ...
    def scan(self) -> None:
        ...
        for dirpath, dirnames, filenames in os.walk(self.root_dir):
            ...
            dirnames[:] = [d for d in dirnames if not d.startswith('.') and 
                        d not in {'__pycache__', 'venv', '.git', 'node_modules'}]
    ...
    def find_true_phantoms(self) -> Dict[str, List[Tuple[str, int]]]:
        ...
        for func_name, files in self.called_functions.items():
            if func_name not in self.defined_functions and func_name not in all_imported:
    ...
    def categorize_phantom_functions(self) -> Dict[str, List[str]]:
        ...
        std_lib_prefixes = {
            'os_', 'sys_', 'math_', 'json_', 're_', 'random_', 'datetime_',
            'time_', 'file_', 'path_', 'str_', 'list_', 'dict_', 'set_', 'get_'
        }
        
        ml_lib_prefixes = {
            'np_', 'pd_', 'plt_', 'torch_', 'tf_', 'keras_', 'sklearn_',
            'model_', 'layer_', 'optimizer_', 'loss_', 'activation_'
        }
        
        # Project-specific prefixes (customize based on your project)
        project_prefixes = {
            'pulse_', 'forecast_', 'sim_', 'causal_', 'world_', 'trust_',
            'symbolic_', 'capital_', 'rule_', 'state_', 'memory_'
        }
    ...
    def report(self, max_contexts: int = 3) -> None:
        ...
        for func in sorted(categories['project_specific']):
            ...
            more = "" if len(contexts) <= max_contexts else f" (and {len(true_phantoms[func]) - max_contexts} more...)"
