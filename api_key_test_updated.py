...
def test_fred_api(api_key):
    ...
    try:
        ...
        try:
            ...
            content = {"text": response.text[:100] + "..." if len(response.text) > 100 else response.text}
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    if 'dataset_data' in content:
                        display_content = {"dataset_data": "Found", "columns": len(content.get('dataset_data', {}).get('column_names', []))}
                    elif 'dataset' in content:
                        display_content = {"dataset": "Found", "id": content.get('dataset', {}).get('id', 'N/A')}
                    else:
