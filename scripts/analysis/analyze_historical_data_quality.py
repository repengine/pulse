...
def get_latest_transformation(variable_name: str) -> Optional[str]:
    ...
    try:
        ...
        files = list(Path(transform_dir).glob(f"*_transform_result.json"))
        if not files:
...
def analyze_variable_data(variable_name: str) -> Dict[str, Any]:
    ...
    try:
        ...
        results.update({
            ...
            "data_points": len(df),
            "completeness_pct": 100.0 * (len(df) - missing_count) / len(df) if len(df) > 0 else 0,
            "has_missing_values": missing_count > 0,
            "missing_value_count": missing_count,
            "date_range": f"{min_date.date()} to {max_date.date()}",
            "min_value": None if df['value'].isna().all() else float(df['value'].min()),
            "max_value": None if df['value'].isna().all() else float(df['value'].max()),
            "mean_value": None if df['value'].isna().all() else float(df['value'].mean()),
        })
...
def analyze_temporal_alignment(variable_results: Dict[str, Dict[str, Any]]) -> Dict[str, Any]:
    """Analyze temporal alignment across variables."""
    results = {
        "variables_analyzed": len(variable_results),
        "variables_with_data": sum(1 for v in variable_results.values() if v["has_data"]),
        "min_years": min([v["historical_years"] for v in variable_results.values() if v["has_data"]] or [0]),
        "max_years": max([v["historical_years"] for v in variable_results.values() if v["has_data"]] or [0]),
        "avg_years": sum([v["historical_years"] for v in variable_results.values() if v["has_data"]]) / 
                    sum(1 for v in variable_results.values() if v["has_data"]) 
                    if sum(1 for v in variable_results.values() if v["has_data"]) > 0 else 0,
        "variables_meeting_min_years": sum(1 for v in variable_results.values() if v["historical_years"] >= MIN_YEARS),
        "percent_meeting_min_years": 100.0 * sum(1 for v in variable_results.values() if v["historical_years"] >= MIN_YEARS) / 
                                   sum(1 for v in variable_results.values() if v["has_data"])
                                   if sum(1 for v in variable_results.values() if v["has_data"]) > 0 else 0,
        "variables_meeting_target_years": sum(1 for v in variable_results.values() if v["historical_years"] >= TARGET_YEARS),
        "percent_meeting_target_years": 100.0 * sum(1 for v in variable_results.values() if v["historical_years"] >= TARGET_YEARS) / 
                                      sum(1 for v in variable_results.values() if v["has_data"])
                                      if sum(1 for v in variable_results.values() if v["has_data"]) > 0 else 0,
    }
...
def generate_visualizations(variable_results: Dict[str, Dict[str, Any]], alignment_results: Dict[str, Any]) -> Dict[str, str]:
    ...
    try:
        ...
        sorted_vars = sorted(data_vars.items(), key=lambda x: x[1]["historical_years"], reverse=True)
        var_names = [x[0] for x in sorted_vars]
        years = [x[1]["historical_years"] for x in sorted_vars]
        
        bars = plt.barh(var_names, years)
        
        # Add a line for MIN_YEARS
        plt.axvline(x=MIN_YEARS, color='r', linestyle='--', label=f'Minimum Target ({MIN_YEARS} years)')
        
        # Add a line for TARGET_YEARS
        plt.axvline(x=TARGET_YEARS, color='g', linestyle='--', label=f'Ideal Target ({TARGET_YEARS} years)')
        ...
        sorted_vars = sorted(data_vars.items(), key=lambda x: x[1]["completeness_pct"], reverse=True)
        var_names = [x[0] for x in sorted_vars]
        completeness = [x[1]["completeness_pct"] for x in sorted_vars]
        
        bars = plt.barh(var_names, completeness)
...
def generate_report(variable_results: Dict[str, Dict[str, Any]], 
    ...
    try:
        ...
        with open(report_path, 'w') as f:
            ...
            sorted_vars = sorted(variable_results.items(), key=lambda x: x[1]["historical_years"] if x[1]["has_data"] else 0, reverse=True)
            
            for var_name, var_results in sorted_vars:
                completeness_class = 'good' if var_results["completeness_pct"] >= 95 else 'warning' if var_results["completeness_pct"] >= 80 else 'bad'
                years_class = 'good' if var_results["historical_years"] >= TARGET_YEARS else 'warning' if var_results["historical_years"] >= MIN_YEARS else 'bad'
            ...
            if alignment_results['percent_meeting_min_years'] < 80:
                f.write(f"<li>Increase historical depth for variables below the {MIN_YEARS}-year minimum threshold.</li>")
            
            incomplete_vars = [v for v in variable_results.values() if v["has_data"] and v["completeness_pct"] < 95]
            if incomplete_vars:
                f.write("<li>Improve data completeness for the following variables:</li>")
                f.write("<ul>")
                for v in sorted(incomplete_vars, key=lambda x: x["completeness_pct"]):
                    f.write(f"<li>{v['variable_name']} ({v['completeness_pct']:.2f}% complete)</li>")
                f.write("</ul>")
            
            missing_vars = [v["variable_name"] for v in variable_results.values() if not v["has_data"]]
            if missing_vars:
...
def main():
    ...
    try:
        ...
        visualization_paths = generate_visualizations(variable_results, alignment_results)
        
        # Generate report
        report_path = generate_report(variable_results, alignment_results, visualization_paths)
