...
def ensure_module_md(name: str, data: Dict[str, str]) -> None:
    ...
    post["Module Intent/Purpose"] = data["purpose"] or post.get(
        "Module Intent/Purpose", ""
    )
    post["Operational Status/Completeness"] = data["status"] or post.get(
        "Operational Status/Completeness", ""
    )
    post["Implementation Gaps / Unfinished Next Steps"] = data["gaps"] or post.get(
...
def ensure_inventory(modules: list[str]) -> None:
    lines = (
        INV_FILE.read_text().splitlines()
        if INV_FILE.exists()
        else ["# Pulse Module Inventory\n"]
    )
    existing = {
        line.split("**")[1].split(".py")[0] for line in lines if line.startswith("- **")
    }
