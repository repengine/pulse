...
def test_fred_api(api_key):
    ...
    try:
        ...
        return {
            ...
            'details': "API connection successful" if response.status_code == 200 else "See response for details"
        }
...
def test_nasdaq_api(api_key):
    ...
    endpoints = [
        {"name": "LBMA Gold Dataset", "url": f"https://data.nasdaq.com/api/v3/datasets/LBMA/GOLD.json?api_key={api_key}&limit=1"},
        {"name": "FRED GDP Dataset", "url": f"https://data.nasdaq.com/api/v3/datasets/FRED/GDP.json?api_key={api_key}&limit=1"},
        {"name": "ODA Aluminum Price", "url": f"https://data.nasdaq.com/api/v3/datasets/ODA/PALUM_USD.json?api_key={api_key}&limit=1"},
        {"name": "OPEC Reference Basket", "url": f"https://data.nasdaq.com/api/v3/datasets/OPEC/ORB.json?api_key={api_key}&limit=1"},
        {"name": "World Bank Population", "url": f"https://data.nasdaq.com/api/v3/datasets/WORLDBANK/WLD_SP_POP_TOTL.json?api_key={api_key}&limit=1"}
    ]
...
def main():
    ...
    apis = [
        {'name': 'FRED', 'keys': ['FRED_API_KEY', 'FRED_KEY'], 'test_func': test_fred_api},
        {'name': 'Finnhub', 'keys': ['FINNHUB_API_KEY', 'FINNHUB_KEY'], 'test_func': test_finnhub_api},
        {'name': 'NASDAQ', 'keys': ['NASDAQ_API_KEY', 'NASDAQ_KEY'], 'test_func': test_nasdaq_api}
    ]
    ...
    for api in apis:
        ...
        all_connections_successful = all_connections_successful and api_connections_successful
    ...
    if all_keys_found and all_connections_successful:
        print("✅ All API keys found and working correctly with both naming conventions")
    ...
    if not all_connections_successful:
        print("\nNOTES:")
        if any(r['api'] == 'NASDAQ' and not r['connections_successful'] for r in results):
            print("- NASDAQ API: Connection test failed. Your API key may have limited access.")
            print("  Try using free tier datasets like 'LBMA/GOLD', 'FRED/GDP', 'ODA/PALUM_USD', or 'OPEC/ORB'.")
            print("  Access to premium datasets like 'EOD/AAPL' may require a paid subscription.")
