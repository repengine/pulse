...
DEFAULT_OUTPUT_DIR = PATHS.get("CAUSAL_BENCHMARKS_OUTPUT", "forecasts/causal_only_benchmarks")
DEFAULT_SCENARIOS_FILE = PATHS.get("CAUSAL_BENCHMARKS_CONFIG", "config/causal_benchmark_scenarios.yaml")
...
def load_scenarios_from_file(file_path: str) -> List[Dict[str, Any]]:
    ...
    if not os.path.exists(file_path):
        print(f"Warning: Scenarios file {file_path} not found. Using default scenarios.")
...
def run_scenario_programmatically(scenario: Dict[str, Any], output_path: str) -> Dict[str, Any]:
    """
...
def run_scenario_subprocess(scenario: Dict[str, Any], output_path: str) -> Dict[str, Any]:
    """
...
def calculate_metrics(results: List[Dict[str, Any]], scenario: Dict[str, Any]) -> Dict[str, Any]:
    """
    ...
    if forecasts:
        ...
        for var in all_variables:
            var_accuracies = [f.get("variables", {}).get(var, {}).get("accuracy", 0.0) 
                            for f in forecasts 
                            if var in f.get("variables", {})]
            
            var_drifts = [f.get("variables", {}).get(var, {}).get("drift", 0.0) 
                        for f in forecasts 
                        if var in f.get("variables", {})]
            
            if var_accuracies:
                metrics["accuracy"]["by_variable"][var] = sum(var_accuracies) / len(var_accuracies)
    ...
    metrics["summary"] = {
        ...
        "best_variable": max(metrics["accuracy"]["by_variable"].items(), key=lambda x: x[1])[0] if metrics["accuracy"]["by_variable"] else None,
        "worst_variable": min(metrics["accuracy"]["by_variable"].items(), key=lambda x: x[1])[0] if metrics["accuracy"]["by_variable"] else None
    }
            ...
            "scenario_summaries": [
                {
                    ...
                    "metrics": r.get("metrics", {}).get("summary", {}) if "metrics" in r else {}
                }
...
def main():
    parser = argparse.ArgumentParser(description="Run causal-only benchmarks with gravity disabled")
    parser.add_argument(
        "--config", 
