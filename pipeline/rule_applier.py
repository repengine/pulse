...
def load_proposed_rule_changes(file_path="pipeline/rule_proposals/proposed_rule_changes.json"):
...
def apply_rule_changes(proposed_changes, active_rules):
    ...
    print(f"Applying {len(modifications)} modifications, {len(generations)} generations, {len(prunings)} prunings.")
    ...
    for new_rule in generations:
        ...
        if 'id' in new_rule and any(rule.get('id') == new_rule['id'] for rule in updated_rules):
             print(f"Warning: Rule with ID {new_rule.get('id')} already exists. Skipping generation.")
             continue
