...
import pandas as pd
import logging
logger = logging.getLogger("pulse.retrodiction")
...
def _ensure_truth_parquet():
    if not os.path.exists(HIST_PARQ_PATH):
        try:
            ...
            logger.info("[Retrodiction] Migrated ground-truth to Parquet for performance")
        except Exception as e:
            logger.warning(f"[Retrodiction] Failed migrating to Parquet: {e}")

def _impute_missing_variables(vars_raw: Dict[str, float], defaults: Dict[str, float]) -> Dict[str, float]:
    """Fill missing variables and drop unexpected ones."""
    valid, missing, unexpected = validate_variables(vars_raw)
    if unexpected:
        logger.warning(f"[Retrodiction] Unexpected variables {unexpected}; dropping them")
        for k in unexpected:
            vars_raw.pop(k, None)
    for m in missing:
        vars_raw[m] = defaults.get(m)
        logger.info(f"[Retrodiction] Imputed missing variable {m} with default {defaults.get(m)}")
    return vars_raw
...
def load_historical_baseline(date_str: str) -> Dict[str, float]:
    """Load historical baseline for a given date, filling missing variables."""
    ...
    if os.path.exists(HIST_PARQ_PATH):
        try:
            ...
            if not sel.empty:
                ...
                return _impute_missing_variables(vars_raw, defaults)
        except Exception as e:
            logger.warning(f"[Retrodiction] Parquet read failed: {e}")
    try:
        with open(TRUTH_PATH, "r") as f:
            for line in f:
                try:
                    ...
                    logger.warning(f"[Retrodiction] Skipping invalid JSON line: {err}")
                    continue
                if entry.get("date") == date_str:
                    vars_raw = entry.get("variables", {})
                    return _impute_missing_variables(vars_raw, defaults)
    except Exception as e:
        logger.warning(f"[Retrodiction] Failed to load baseline: {e}")
    logger.warning(f"[Retrodiction] Baseline for {date_str} not found; returning default state")
...
def simulate_forward(state: WorldState, steps: int = 30, cache_key: Optional[str] = None) -> List[Dict[str, Any]]:
    ...
    if cache_key:
        ...
        if cache_path.exists():
            ...
            logger.info(f"[Sim] Loaded {len(history)} cached snapshots for {cache_key}")
            return history
        else:
    ...
    for i in range(steps):
        ...
        logger.info(f"[Sim] Step {i+1}/{steps} | State snapshot: {snapshot.get('variables', {})}")
...
def compare_to_actual(sim_steps: List[Dict], truth_steps: List[Dict]) -> List[Dict]:
    ...
    for sim, truth in zip(sim_steps, truth_steps):
        if not truth["variables"]:
            logger.warning(f"⚠️ Missing truth data for sim date {sim.get('date')}")
            continue
        ...
        logger.info(f"[Compare] Error score: {delta}")
    return comparisons

# New functions for historical timeline handling

def load_historical_timeline() -> Dict[str, Any]:
    """Load the historical timeline from config path."""
    timeline_path = PATHS.get("HISTORICAL_TIMELINE", "historical_timeline.json")
    try:
        with open(timeline_path, "r") as f:
            timeline = json.load(f)
        return timeline
    except Exception as e:
        logger.warning(f"[Retrodiction] Failed to load timeline: {e}")
        return {"eras": [], "events": []}

def load_modern_baseline(date_str: str) -> Dict[str, float]:
    """Load modern baseline from API or fallback."""
    import requests
    url = os.getenv("PULSE_MODERN_BASELINE_URL", "https://api.example.com/modern_historical_data")
    if url == "https://api.example.com/modern_historical_data":
        logger.warning("[Retrodiction] Modern baseline URL is not set. Using placeholder.")
    try:
        ...
        defaults = get_default_variable_state()
        return _impute_missing_variables(variables, defaults)
    except Exception as e:
        logger.warning(f"[Retrodiction] Failed to load modern baseline from internet for {date_str}: {e}")
        return {}
...
def get_era_config_for_date(date_str: str, timeline: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    try:
        ...
        logger.warning(f"[Retrodiction] Invalid date format: {date_str}")
        return None
    for era in timeline.get("eras", []):
        try:
            ...
            logger.warning(f"[Retrodiction] Error processing era dates: {e}")
            continue
    return None

def run_retrodiction_test(start_date: str, days: int = 30):
    """Run a retrodiction simulation from a given start date for N days."""
    if start_date >= "2001-01-01":
        logger.info("[Retrodiction] Modern date detected, loading baseline from internet")
        baseline = load_modern_baseline(start_date)
        if not baseline:
            logger.warning("[Retrodiction] Failed to load modern data, falling back to historical baseline")
    ...
    if era_config:
        logger.info(f"[Retrodiction] Era config for {start_date}: {era_config}")
        baseline["retrodiction_modifier"] = era_config.get("retrodiction_modifier", 1.0)
    else:
        logger.info(f"[Retrodiction] No era config found for {start_date}")
    if not baseline:
        logger.warning("[Retrodiction] Baseline state not found.")
        return
    # Use WorldState defaults for overlays/capital if None
    state = WorldState(
        variables=baseline,
        turn=0,
        sim_id="retrodict",
    ...
    logger.info(f"✅ Retrodiction complete for {start_date} → {days} days simulated ({steps} steps). Results written to log.")

def run_retrodiction_tests(start_dates: List[str], days: int = 30, max_workers: int = 4):
    """Parallel retrodiction for multiple start dates with progress reporting."""
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as executor:
        ...
        for i, future in enumerate(concurrent.futures.as_completed(futures), 1):
            ...
            try:
                future.result()
                logger.info(f"[Retrodiction] Completed {i}/{total} for {sd}")
            except Exception as e:
                logger.error(f"[Retrodiction] Error for {sd}: {e}")
