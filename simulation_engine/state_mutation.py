...
import core.pulse_config # Import core.pulse_config
...
def decay_overlay(
    state: WorldState, overlay: str, rate: float = 0.01
) -> Optional[float]:
    """
    ...
    if current_value is not None:
        ...
        state.log_event(
            f"Overlay '{overlay}' decayed from {current_value:.3f} to {new_value:.3f}"
        )
        log_learning_event(
            "overlay_shift",
            {
...
def adjust_overlay(state: WorldState, overlay: str, delta: float) -> Optional[float]:
    ...
    if current_value is not None:
        ...
        if not (core.pulse_config.CURRENT_SYSTEM_MODE == "retrodiction" and core.pulse_config.SYMBOLIC_PROCESSING_MODES.get("retrodiction", False)):
            state.log_event(
                f"Overlay '{overlay}' adjusted by {delta:.3f} to {new_value:.3f}"
            )
            log_learning_event(
                "overlay_shift",
                {
...
def adjust_capital(state: WorldState, asset: str, delta: float) -> Optional[float]:
    ...
    if hasattr(state.capital, asset):
        ...
        state.log_event(
            f"Capital exposure for '{asset}' changed by {delta:.2f} to {new_value:.2f}"
        )
        log_learning_event(
            "capital_shift",
            {
